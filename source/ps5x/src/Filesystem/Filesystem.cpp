...
static constexpr std::array<const char*, 6> kMountPrefix = {"/app0", "/savedata", "/system",
															"/temp", "/user",     "/hostapp"};
...
std::optional<std::filesystem::path> ResolveImpl(std::string_view guestRaw, const VfsState& st)
{
	...
	for (const auto& m : st.mounts)
	{
		if (!m.valid)
			continue;
		if (guest == m.prefix || guest.starts_with(m.prefix + "/"))
		{
...
bool Mount(MountPoint mp, const std::filesystem::path& hostPath, bool readOnly)
{
	...
	if (!std::filesystem::exists(hostPath))
	{
		...
		if (ec)
		{
			PS5X_ERROR("[FS] Mount: host path absent and can't create: %s", hostPath.string().c_str());
			return false;
		}
	}
	std::lock_guard lk(st.mtx);
	st.mounts[idx] = MountEntry{kMountPrefix[idx], hostPath, true, readOnly};
	PS5X_INFO("[FS] Mount %s → %s%s", kMountPrefix[idx], hostPath.string().c_str(), readOnly ? " [ro]" : "");
	return true;
}
...
std::optional<std::filesystem::path> Resolve(std::string_view guestPath)
{
	...
	if (!r)
		PS5X_WARN("[FS] Unresolvable: %.*s", static_cast<int>(guestPath.size()), guestPath.data());
	return r;
}
...
FileHandle Open(std::string_view guestPath, OpenFlags flags)
{
	...
	{
		PS5X_ERROR("[FS] Open: write denied on read-only mount: %.*s", static_cast<int>(guestPath.size()),
				   guestPath.data());
		return INVALID_FD;
	}
	...
	if (!fp)
	{
		PS5X_ERROR("[FS] Open failed: %s (%s)", hostOpt->string().c_str(), std::strerror(errno));
		return INVALID_FD;
	}
...
int64_t Read(FileHandle fd, void* buf, uint64_t size)
{
	...
	if (it == st.files.end())
		return -1;
	return static_cast<int64_t>(std::fread(buf, 1, static_cast<size_t>(size), it->second.fp));
}
...
int64_t Write(FileHandle fd, const void* buf, uint64_t size)
{
	...
	if (it->second.rdOnly)
		return -1;
	return static_cast<int64_t>(std::fwrite(buf, 1, static_cast<size_t>(size), it->second.fp));
}
...
int64_t Seek(FileHandle fd, int64_t offset, SeekOrigin origin)
{
	...
	if (it == st.files.end())
		return -1;
	int w = (origin == SeekOrigin::Set) ? SEEK_SET : (origin == SeekOrigin::Cur) ? SEEK_CUR : SEEK_END;
...
bool MakeDir(std::string_view guestPath, bool recursive)
{
	...
	if (!h)
		return false;
	std::error_code ec;
	return recursive ? std::filesystem::create_directories(*h, ec) : std::filesystem::create_directory(*h, ec);
}
...
FileStat Stat(std::string_view guestPath)
{
	...
	if (!ec)
	{
		auto sctp = std::chrono::clock_cast<std::chrono::system_clock>(wt);
		s.modTime =
			static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::seconds>(sctp.time_since_epoch()).count());
	}
	return s;
}
...
void DumpMounts()
{
	...
	for (size_t i = 0; i < st.mounts.size(); ++i)
	{
		...
		if (m.valid)
			PS5X_INFO("[FS]  %-12s → %s%s", kMountPrefix[i], m.hostPath.string().c_str(), m.readOnly ? " [ro]" : "");
		else
...
struct FsTraceState
{
	...
	static FsTraceState& Get()
	{
		static FsTraceState s;
		return s;
	}
};
...
void TraceEvent(FsEvent ev, std::string_view path, int64_t bytes = 0, bool ok = true)
{
	...
	if (!ts.enabled.load(std::memory_order_relaxed))
		return;
	std::lock_guard lk(ts.mtx);
	if (ts.entries.size() >= FsTraceState::MAX)
		ts.entries.pop_front();
...
std::vector<FsTraceEntry> GetTrace(size_t maxEntries)
{
	...
	return std::vector<FsTraceEntry>(ts.entries.end() - static_cast<ptrdiff_t>(n), ts.entries.end());
}
