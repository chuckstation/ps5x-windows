// PS5x – Debugger (Phase 8 polished implementation)
...
std::vector<FlagEntry> GetFlagsView()
{
    auto& ctx = Cpu::GetContextConst();
    return {
        {"CF", ctx.flag(Cpu::Flags::CF)},
        {"PF", ctx.flag(Cpu::Flags::PF)},
        {"AF", ctx.flag(Cpu::Flags::AF)},
        {"ZF", ctx.flag(Cpu::Flags::ZF)},
        {"SF", ctx.flag(Cpu::Flags::SF)},
        {"TF", ctx.flag(Cpu::Flags::TF)},
        {"IF", ctx.flag(Cpu::Flags::IF)},
        {"DF", ctx.flag(Cpu::Flags::DF)},
        {"OF", ctx.flag(Cpu::Flags::OF)},
    };
}
...
std::optional<std::string> LookupSymbol(uint64_t addr)
{
    ...
    if (it != st.symbols.end()) return it->second;
    return std::nullopt;
}
...
std::optional<NearestSymbolResult> NearestSymbol(uint64_t addr)
{
    auto& st = State::Get();
    std::lock_guard lk(st.mtx);
    if (st.symbols.empty()) return std::nullopt;

    // Find the largest key ≤ addr
    auto it = st.symbols.upper_bound(addr);
    if (it == st.symbols.begin()) return std::nullopt;
    --it;
...
size_t MaxTimelineEvents() { return State::kMaxTimeline; }
...
std::vector<EventLogEntry> GetEventLog(RuntimeEvents::EventType filter)
{
    ...
    for (auto& e : st.eventLog) {
        if (e.type == filter) out.push_back(e);
    }
    return out;
}
...
bool EnableBreakpoint(uint32_t id, bool enable)
{
    ...
    std::lock_guard lk(st.mtx);
    for (auto& bp : st.breakpoints) {
        if (bp.id == id) { bp.enabled = enable; return true; }
    }
    return false;
}

void ClearBreakpoints() { ClearAllBreakpoints(); }
...
bool WriteCrashDump(const std::string& dumpDir)
{
    ...
    if (ec) {
        PS5X_ERROR("[Debugger] WriteCrashDump: cannot create dir '%s': %s",
                   dumpDir.c_str(), ec.message().c_str());
        return false;
    }
    ...
    if (hDbgHelp) {
        ...
        if (pMiniDumpWriteDump) {
            ...
            if (hFile != INVALID_HANDLE_VALUE) {
                ...
                ::CloseHandle(hFile);
                if (ok) {
                    PS5X_INFO("[Debugger] Crash dump written: '%s'", dumpPath.c_str());
                    ::FreeLibrary(hDbgHelp);
                    return true;
                }
                PS5X_ERROR("[Debugger] MiniDumpWriteDump failed (err=%lu).", ::GetLastError());
            }
        }
        ::FreeLibrary(hDbgHelp);
    }
    PS5X_WARN("[Debugger] dbghelp.dll unavailable — writing text crash report.");
#endif

    // Fallback: plain-text crash report with register state + call stack
    std::ofstream f(dumpPath);
    if (!f.is_open()) {
        PS5X_ERROR("[Debugger] Cannot write crash report to '%s'.", dumpPath.c_str());
        return false;
    }
    f << "PS5x Crash Report\nTimestamp: " << ts << "\n\n";
    f << "=== Registers ===\n";
    ...
    for (size_t i = 0; i < frames.size(); ++i) {
        ...
        if (!frames[i].symbol.empty()) f << "  " << frames[i].symbol;
        f << "\n";
    }
    f << "\n=== Timeline (last 20) ===\n";
    auto tl = GetTimeline();
    ...
    for (size_t i = start; i < tl.size(); ++i) {
        f << "  [" << tl[i].timestampNs << "] " << tl[i].category
          << " @ 0x" << std::hex << tl[i].address << "\n";
    }
    PS5X_INFO("[Debugger] Text crash report written: '%s'", dumpPath.c_str());
    return true;
}
...
bool RemoveWatch(uint32_t id)
{
    auto& st = State::Get();
    std::lock_guard lk(st.mtx);
    auto it = std::find_if(st.watches.begin(), st.watches.end(),
                           [id](const WatchExpression& w){ return w.id == id; });
    if (it == st.watches.end()) return false;
    st.watches.erase(it);
    return true;
}
