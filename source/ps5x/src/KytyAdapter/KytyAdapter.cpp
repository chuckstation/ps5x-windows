...
struct NativeVMState
{
	...
	std::mutex mtx;
	static NativeVMState& Get()
	{
		static NativeVMState s;
		return s;
	}
};
...
struct NativeAudioState
{
	...
	uint16_t channels = 2;
	static NativeAudioState& Get()
	{
		static NativeAudioState s;
		return s;
	}
};
...
void* VirtualAlloc(void* hint, uint64_t size, int prot)
{
	...
	void* p = ::VirtualAlloc(hint, static_cast<SIZE_T>(size), MEM_RESERVE | MEM_COMMIT, winProt);
	if (!p)
	{
		// Hint failed — try anywhere
		p = ::VirtualAlloc(nullptr, static_cast<SIZE_T>(size), MEM_RESERVE | MEM_COMMIT, winProt);
	}
	if (p)
	{
	...
	{
		PS5X_ERROR("[KytyAdapter] VirtualAlloc failed size=%llu err=%lu", size, ::GetLastError());
	}
	return p;
#else
	...
	uintptr_t base = Memory::Map(reinterpret_cast<uintptr_t>(hint), static_cast<size_t>(size), p,
								 Memory::AllocType::Unknown, "KytyAdapter");
...
void InstallLogBridge()
{
	...
	Kyty::Core::Log::SetCallback([](const char* msg) { PS5X_INFO("[Kyty] %s", msg); });
