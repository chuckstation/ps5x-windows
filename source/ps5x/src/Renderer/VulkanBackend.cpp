  ...
  public:
	bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
	{
		...
		for (UINT i = 0; factory_->EnumAdapters1(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i)
		{
			...
			hr = D3D12CreateDevice(adapter, D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&device_));
			if (SUCCEEDED(hr))
			{
		...
		if (FAILED(hr))
		{
			PS5X_ERROR("[DX12] CreateCommandQueue failed.");
			return false;
		}
		...
		if (hwnd_)
		{
			...
			{
				hr = f4->CreateSwapChainForHwnd(commandQueue_, hwnd_, &scDesc, nullptr, nullptr, &sc1);
				f4->Release();
				if (SUCCEEDED(hr))
				{
		...
		device_->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator_, nullptr,
	...
	bool Resize(uint32_t w, uint32_t h) override
	{
		...
		for (auto& rt : renderTargets_)
			SafeRelease(rt);
		HRESULT hr = swapChain_->ResizeBuffers(kFrameCount, w, h, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
		if (FAILED(hr))
		{
			PS5X_ERROR("[DX12] ResizeBuffers failed.");
			return false;
		}
		D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle = rtvHeap_->GetCPUDescriptorHandleForHeapStart();
		for (UINT f = 0; f < kFrameCount; ++f)
	...
	Config::GraphicsBackend BackendType() const override
	{
		return Config::GraphicsBackend::DirectX12;
	}
	const FrameStats& Stats() const override
	{
		return stats_;
	}
  ...
  public:
	bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
	{
		...
		if (hwnd)
		{
			...
			hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, flags, featureLevels,
											   _countof(featureLevels), D3D11_SDK_VERSION, &scDesc, &swapChain_,
											   &device_, &achieved, &context_);
		}
		else
		{
			hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, flags, featureLevels,
								   _countof(featureLevels), D3D11_SDK_VERSION, &device_, &achieved, &context_);
		}
		...
		PS5X_INFO("[DX11] Backend ready (feature level 0x%x).", static_cast<unsigned>(achieved));
		ready_ = true;
		return true;
#else
		ready_ = true;
	...
	Config::GraphicsBackend BackendType() const override
	{
		return Config::GraphicsBackend::DirectX11;
	}
	const FrameStats& Stats() const override
	{
		return stats_;
  ...
  public:
	bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
	{
		...
		auto vkGetInstanceProcAddr =
			reinterpret_cast<PFN_vkGetInstanceProcAddr_t>(::GetProcAddress(hVulkan_, "vkGetInstanceProcAddr"));
	...
	Config::GraphicsBackend BackendType() const override
	{
		return Config::GraphicsBackend::Vulkan;
	}
	const FrameStats& Stats() const override
	{
		return stats_;
...
class NullBackend final : public IRendererBackend
{
  public:
	...
	void Shutdown() override
	{
		PS5X_INFO("[NullRenderer] Shut down.");
	}
	void BeginFrame() override
	{
		stats_.frameIndex++;
	...
	Config::GraphicsBackend BackendType() const override
	{
		return Config::GraphicsBackend::Null;
	}
	const FrameStats& Stats() const override
	{
		return stats_;
...
std::unique_ptr<IRendererBackend> CreateBackend(Config::GraphicsBackend type)
{
	switch (type)
	{
	case Config::GraphicsBackend::DirectX12:
		return std::make_unique<DX12Backend>();
	case Config::GraphicsBackend::DirectX11:
		return std::make_unique<DX11Backend>();
	case Config::GraphicsBackend::Vulkan:
		return std::make_unique<VulkanBackend>();
	case Config::GraphicsBackend::Null:
		return std::make_unique<NullBackend>();
	case Config::GraphicsBackend::OpenGL:
		return std::make_unique<NullBackend>();
	default:
		break;
	}
