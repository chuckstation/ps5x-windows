...
void Shutdown() { Reset(); PS5X_INFO("[DynLink] Shutdown."); }
...
bool ApplyReloc(const RelocEntry& reloc)
{
    ...
    switch (reloc.type) {
        ...
        default:
            PS5X_WARN("[DynLink] Unsupported reloc type %s for %s",
...
LinkResult LinkModule(ModuleRegistry::ModuleId id)
{
    ...
    PS5X_INFO("[DynLink] LinkModule %u: resolved=%u unresolved=%u lazy=%u",
...
LinkResult LinkAll()
{
    ...
    PS5X_INFO("[DynLink] LinkAll: resolved=%u unresolved=%u lazy=%u missing=%zu",
...
void DumpRelocations(ModuleRegistry::ModuleId id)
{
    ...
    for (const auto& r : it->second) {
        PS5X_INFO("[DynLink]   0x%llx %-20s %-30s addend=%lld %s%s",
...
void DumpSymbolCache()
{
    ...
    for (const auto& [name, sym] : ls.cache)
        PS5X_INFO("[DynLink]   %-40s 0x%llx mod=%u hits=%llu",
                  name.c_str(),
...
{
    PS5X_INFO("[DynLink] Link result: resolved=%u unresolved=%u lazy=%u",
