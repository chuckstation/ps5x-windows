// PS5x – Dynamic Linker implementation
...
const char* RelocTypeName(RelocType t)
{
	...
	case RelocType::GotPcRel:
		return "R_X86_64_GOTPCREL";
	case RelocType::Size32:
		return "R_X86_64_SIZE32";
	case RelocType::Size64:
...
struct LinkerState
{
	...
	static LinkerState& Get()
	{
		static LinkerState s;
		return s;
	}
};
...
bool Patch64(uintptr_t hostAddr, uint64_t value)
{
	if (!Memory::IsWritable(hostAddr, 8))
	{
		...
		if (!Memory::Protect(hostAddr, 8, Memory::Prot::RW))
			return false;
...
bool Patch32(uintptr_t hostAddr, uint32_t value)
{
	...
	{
		if (!Memory::Protect(hostAddr, 4, Memory::Prot::RW))
			return false;
...
bool Init()
{
	...
	ls.cacheHits.store(0);
	ls.cacheMisses.store(0);
	ls.totalRelocs.store(0);
	ls.resolvedRelocs.store(0);
	ls.unresolvedRelocs.store(0);
	ls.lazyRelocs.store(0);
	PS5X_INFO("[DynLink] Initialised.");
	return true;
}
...
std::optional<CachedSymbol> LookupSymbol(const std::string& name)
{
	...
	if (!sym)
	{
		PS5X_DEBUG("[DynLink] Unresolved symbol: %s", name.c_str());
...
void InvalidateCache(ModuleRegistry::ModuleId id)
{
	...
	{
		if (it->second.moduleId == id)
			it = ls.cache.erase(it);
		else
			++it;
	}
	PS5X_DEBUG("[DynLink] Cache invalidated for module %u", id);
}
...
bool ApplyReloc(const RelocEntry& reloc)
{
	if (reloc.type == RelocType::None)
		return true;
	if (reloc.lazy)
		return true; // deferred until first call
	...
	default:
		PS5X_WARN("[DynLink] Unsupported reloc type %s for %s", RelocTypeName(reloc.type), reloc.symbolName.c_str());
		return false;
	}
}
...
LinkResult LinkModule(ModuleRegistry::ModuleId id)
{
	...
	{
		PS5X_DEBUG("[DynLink] LinkModule %u: no relocations registered", id);
		return result;
	}
	...
	PS5X_INFO("[DynLink] LinkModule %u: resolved=%u unresolved=%u lazy=%u", id, result.resolved, result.unresolved,
			  result.lazy);
	return result;
}
...
LinkResult LinkAll()
{
	...
	PS5X_INFO("[DynLink] LinkAll: resolved=%u unresolved=%u lazy=%u missing=%zu", total.resolved, total.unresolved,
			  total.lazy, total.missingSymbols.size());
	return total;
}
...
uint32_t EagerBind(ModuleRegistry::ModuleId id)
{
	...
	PS5X_INFO("[DynLink] EagerBind %u: bound %u PLT entries", id, count);
	return count;
}
...
std::optional<std::vector<ModuleRegistry::ModuleId>> DetectCycles()
{
	...
	for (const auto& m : modules)
	{
		if (!visited.count(m.id))
		{
			...
			if (dfs(m.id))
			{
				PS5X_WARN("[DynLink] Circular dependency detected (len=%zu)", path.size());
				return path;
			}
		}
	}
...
bool CanUnload(ModuleRegistry::ModuleId id)
{
	...
	if (!deps.empty())
	{
		PS5X_WARN("[DynLink] CanUnload %u: %zu module(s) still depend on it", id, deps.size());
		return false;
	}
	return true;
}
...
void DumpRelocations(ModuleRegistry::ModuleId id)
{
	...
	if (it == ls.relocations.end())
	{
		PS5X_INFO("[DynLink] No relocations for module %u", id);
		return;
	}
	PS5X_INFO("[DynLink] Relocations for module %u (%zu):", id, it->second.size());
	for (const auto& r : it->second)
	{
		PS5X_INFO("[DynLink]   0x%llx %-20s %-30s addend=%lld %s%s", static_cast<unsigned long long>(r.offset),
				  RelocTypeName(r.type), r.symbolName.c_str(), static_cast<long long>(r.addend),
				  r.resolved ? "[OK]" : "[?]", r.lazy ? "[lazy]" : "");
	}
}
...
void DumpSymbolCache()
{
	auto& ls = LinkerState::Get();
	std::lock_guard lk(ls.cacheMtx);
	PS5X_INFO("[DynLink] Symbol cache (%zu entries):", ls.cache.size());
	for (const auto& [name, sym] : ls.cache)
		PS5X_INFO("[DynLink]   %-40s 0x%llx mod=%u hits=%llu", name.c_str(),
				  static_cast<unsigned long long>(sym.address), sym.moduleId,
				  static_cast<unsigned long long>(sym.hitCount));
}

void DumpLinkResult(const LinkResult& r)
{
	PS5X_INFO("[DynLink] Link result: resolved=%u unresolved=%u lazy=%u", r.resolved, r.unresolved, r.lazy);
	for (const auto& s : r.missingSymbols)
		PS5X_WARN("[DynLink]   missing: %s", s.c_str());
}
