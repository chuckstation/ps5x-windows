...
static const std::vector<double> kDefaultBounds = {1.0,   2.0,   5.0,    10.0,   20.0,   50.0,   100.0,
												   250.0, 500.0, 1000.0, 2500.0, 5000.0, 10000.0};

// ── Histogram implementation ────────────────────────────────────────────────

Histogram::Histogram(const std::vector<double>& bounds) : bounds_(bounds.empty() ? kDefaultBounds : bounds)
{
...
void Histogram::Record(double value)
{
	...
	while (value < curMin &&
		   !min_.compare_exchange_weak(curMin, value, std::memory_order_relaxed, std::memory_order_relaxed))
	{
	...
	while (value > curMax &&
		   !max_.compare_exchange_weak(curMax, value, std::memory_order_relaxed, std::memory_order_relaxed))
	{
...
Histogram::Snapshot Histogram::GetSnapshot() const
{
	...
	{
		uint64_t target = static_cast<uint64_t>(std::ceil(pct / 100.0 * static_cast<double>(snap.count)));
		if (target == 0)
			target = 1;
...
Histogram& GetHistogram(const std::string& name, const std::vector<double>& bounds)
{
	...
	if (it == reg.histograms.end())
	{
		...
		auto [insIt, inserted] = reg.histograms.emplace(std::piecewise_construct, std::forward_as_tuple(name),
...
void LogAll()
{
	...
	if (!reg.counters.empty())
	{
		...
		for (const auto& [name, ctr] : reg.counters)
			PS5X_INFO("    %-40s : %lld", name.c_str(), static_cast<long long>(ctr.Get()));
	}
	...
	if (!reg.histograms.empty())
	{
		...
		for (const auto& [name, h] : reg.histograms)
		{
			...
			PS5X_INFO("    %-40s : count=%llu min=%.2f max=%.2f mean=%.2f "
					  ...
					  name.c_str(), static_cast<unsigned long long>(snap.count), snap.min, snap.max, snap.mean,
