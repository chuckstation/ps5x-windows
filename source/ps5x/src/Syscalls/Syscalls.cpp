...
static uintptr_t Brk(uintptr_t addr)
{
	...
	if (toCommit > 0)
	{
		...
		if (!commit)
		{
			PS5X_ERROR("[Syscall] brk: VirtualAlloc commit failed (err=%lu).", ::GetLastError());
			return brkCurrent;
		}
		brkSize = newSize;
	}
...
static void* Mmap(void* hint, size_t length, int prot, int flags)
{
	...
	if (!p)
	{
		PS5X_WARN("[Syscall] mmap: VirtualAlloc failed len=%zu err=%lu.", length, ::GetLastError());
		return reinterpret_cast<void*>(~0ULL); // MAP_FAILED
	}
	return p;
#else
...
static int64_t Builtin_write(const SyscallArgs& a)
{
	...
	if (fd == 1 || fd == 2)
	{
		if (buf && count > 0)
		{
			PS5X_INFO("[Guest %s] %.*s", fd == 1 ? "stdout" : "stderr", static_cast<int>(count),
...
static int64_t Builtin_brk(const SyscallArgs& a)
{
	...
	PS5X_DEBUG("[Syscall] brk(0x%zx) → 0x%zx", static_cast<size_t>(addr), static_cast<size_t>(result));
...
static int64_t Builtin_ioctl(const SyscallArgs& a)
{
	...
	if (fd >= 3)
	{
		...
		if (ok)
		{
			PS5X_DEBUG("[Syscall] ioctl fd=%llu req=0x%llx → ok (%lu bytes)", static_cast<unsigned long long>(fd),
	...
	PS5X_WARN("[Syscall] ioctl fd=%llu req=0x%llx argp=0x%llx — ENOTTY", static_cast<unsigned long long>(fd),
...
{
	PS5X_WARN("[Syscall] UNKNOWN nr=%llu — ENOSYS", static_cast<unsigned long long>(a.number));
	return -38; // ENOSYS
}
...
void RegisterSyscall(const SyscallDesc& desc)
{
	...
	PS5X_DEBUG("[Syscall] Registered %llu (%s)", static_cast<unsigned long long>(desc.number), desc.name.c_str());
}
...
bool Dispatch(Cpu::CpuContext& ctx)
{
	...
	PS5X_TRACE("[Syscall] %llu (%s) → %lld", static_cast<unsigned long long>(args.number),
