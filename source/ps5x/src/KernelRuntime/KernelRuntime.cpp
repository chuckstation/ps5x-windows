...
KHandle CreateThread(ThreadEntryFn fn, void* arg, const ThreadAttr& attr)
{
	...
	PS5X_INFO("[KernelRuntime] CreateThread '%s' h=%d stack=0x%llx+%zu", attr.name.c_str(), h,
...
bool SetThreadAffinity(KHandle h, uint64_t mask)
{
	...
	if (kt->osThread.native_handle())
	{
		...
		{
			PS5X_WARN("[KernelRuntime] SetThreadAffinityMask failed (err=%lu).", ::GetLastError());
			return false;
		}
		PS5X_DEBUG("[KernelRuntime] Thread h=%d affinity set to 0x%llx.", h, static_cast<unsigned long long>(mask));
	}
#endif
	return true;
}
...
KHandle CreateMutex(const MutexAttr& attr, std::string_view name)
{
	...
	PS5X_TRACE("[KernelRuntime] CreateMutex '%.*s' h=%d", static_cast<int>(name.size()), name.data(), h);
	return h;
}
...
KHandle CreateSemaphore(const SemaphoreAttr& attr, std::string_view name)
{
	...
	PS5X_TRACE("[KernelRuntime] CreateSemaphore '%.*s' init=%d h=%d", static_cast<int>(name.size()), name.data(),
			   attr.initialValue, h);
	return h;
}
...
KHandle CreateEvent(const EventAttr& attr, std::string_view name)
{
	...
	PS5X_TRACE("[KernelRuntime] CreateEvent '%.*s' h=%d", static_cast<int>(name.size()), name.data(), h);
	return h;
}
...
bool RegisterName(std::string_view name, KHandle handle, NamespaceId ns)
{
	...
	PS5X_DEBUG("[KR] RegisterName '%s' ns=%u h=%d", std::string(name).c_str(), ns.value, handle);
	return true;
}
...
void SetResourceLimits(const ResourceLimits& limits)
{
	...
	st.limits = limits;
	PS5X_INFO("[KR] ResourceLimits: maxMem=%zu maxThreads=%u maxHandles=%u", limits.maxMemoryBytes, limits.maxThreads,
			  limits.maxHandles);
}
