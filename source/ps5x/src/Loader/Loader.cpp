...
Memory::Prot FlagsToProt(uint32_t fl)
{
	...
	if (fl & Elf64::PF_R)
		p = p | Memory::Prot::Read;
	if (fl & Elf64::PF_W)
		p = p | Memory::Prot::Write;
	if (fl & Elf64::PF_X)
		p = p | Memory::Prot::Exec;
	return p;
}
...
const char* LoadResultStr(LoadResult r)
{
	...
	case LoadResult::FileNotFound:
		return "FileNotFound";
	case LoadResult::InvalidElf:
		return "InvalidElf";
	case LoadResult::UnsupportedArch:
		return "UnsupportedArch";
	case LoadResult::MissingSymbol:
		return "MissingSymbol";
	case LoadResult::MemoryError:
		return "MemoryError";
	case LoadResult::FirmwareRequired:
		return "FirmwareRequired";
	case LoadResult::InvalidProgramHeader:
		return "InvalidProgramHeader";
	case LoadResult::RelocationFailed:
		return "RelocationFailed";
	case LoadResult::AlreadyLoaded:
		return "AlreadyLoaded";
	case LoadResult::NotLoaded:
		return "NotLoaded";
...
void Init()
{
	PS5X_INFO("[Loader] Initialised.");
}
void Shutdown()
{
	Reset();
	PS5X_INFO("[Loader] Shutdown.");
}
...
LoadResult InspectElf(const std::filesystem::path& path, ExecutableInfo& out)
{
	...
	if (eh.e_ident[4] != Elf64::ELFCLASS64)
		return LoadResult::InvalidElf;
	if (eh.e_ident[5] != Elf64::ELFLSB)
		return LoadResult::InvalidElf;
	if (eh.e_machine != Elf64::EM_X86_64)
		return LoadResult::UnsupportedArch;

	bool isSce =
		(eh.e_type == Elf64::ET_SCE_EXEC || eh.e_type == Elf64::ET_SCE_DYNEXEC || eh.e_type == Elf64::ET_SCE_DYNAMIC);
	...
	out.isPic =
		(eh.e_type == Elf64::ET_DYN || eh.e_type == Elf64::ET_SCE_DYNEXEC || eh.e_type == Elf64::ET_SCE_DYNAMIC);
	...
	{
		Elf64::Phdr ph{};
		if (!ReadAt(f, eh.e_phoff + i * sizeof(Elf64::Phdr), ph))
			continue;
	...
	PS5X_INFO("[Loader] Inspect OK: %s  entry=0x%llx segs=%zu PIC=%d symbols=%zu relas=%zu",
			  path.filename().string().c_str(), static_cast<unsigned long long>(out.entryPoint), out.segments.size(),
			  out.isPic ? 1 : 0, out.symbols.size(), out.relaSections.size());
...
LoadResult MapSegments(ExecutableInfo& info, const std::filesystem::path& path)
{
	...
	for (auto& seg : info.segments)
	{
		...
		uintptr_t base = Memory::Map(static_cast<uintptr_t>(loadBias + aVa), mapSz, Memory::Prot::RW,
		...
		PS5X_INFO("[Loader]  LOAD host=0x%zx va=0x%llx sz=0x%zx flags=%c%c%c", base,
				  static_cast<unsigned long long>(seg.vaddr), mapSz, (seg.flags & Elf64::PF_R) ? 'R' : '-',
				  (seg.flags & Elf64::PF_W) ? 'W' : '-', (seg.flags & Elf64::PF_X) ? 'X' : '-');
	}

	if (info.isPic)
		info.entryPoint += loadBias;
...
LoadResult LoadExecutable(const std::filesystem::path& path, ExecutableInfo& out)
{
	...
	if (auto r = InspectElf(path, out); r != LoadResult::Ok)
		return r;
	if (auto r = ValidateExecutable(out); r != LoadResult::Ok)
		return r;
	if (auto r = MapSegments(out, path); r != LoadResult::Ok)
		return r;
	auto& st = State::Get();
	std::lock_guard lk(st.mtx);
	st.current = out;
	PS5X_INFO("[Loader] Loaded. entry=0x%llx base=0x%llx", static_cast<unsigned long long>(out.entryPoint),
...
LoadResult LoadFromMemory(const uint8_t* data, size_t size)
{
	...
	bool isSce = (eh->e_type == Elf64::ET_SCE_EXEC || eh->e_type == Elf64::ET_SCE_DYNEXEC ||
	...
	for (auto& seg : info.segments)
	{
		...
		uintptr_t base =
			Memory::Map(static_cast<uintptr_t>(loadBias + aVa), mapSz, Memory::Prot::RW,
						(seg.flags & Elf64::PF_X) ? Memory::AllocType::Code : Memory::AllocType::Data, "MemoryElf");
...
{
	return State::Get().current.entryPoint;
}
...
LoadResult Execute()
{
	auto& st = State::Get();
	std::lock_guard lk(st.mtx);
	if (!st.current.loaded)
		return LoadResult::NotLoaded;
	uint64_t entry = st.current.entryPoint;
	PS5X_INFO("[Loader] Launching guest entry 0x%llx on Win32 thread.", static_cast<unsigned long long>(entry));
#if defined(_WIN32)
...
LoadResult ValidateFirmware(const std::filesystem::path& p)
{
	...
	{
		PS5X_ERROR("[Loader] Firmware absent: '%s'. PS5x does NOT supply firmware.", p.string().c_str());
...
LoadResult LoadParamSfo(const std::filesystem::path& sfoPath, ExecutableInfo& info)
{
	...
	uint32_t magic = 0;
	if (!ReadAt(f, 0, magic) || magic != 0x46535000u)
		return LoadResult::InvalidElf;
...
#pragma pack(pop)
	...
	PS5X_INFO("[Loader] SFO title=%s ver=%s", info.titleId.c_str(), info.appVersion.c_str());
	return LoadResult::Ok;
}
