...
struct GpuHeap
{
	...
	bool Init()
	{
		...
		if (!base)
		{
			PS5X_ERROR("[GPU] GpuHeap: VirtualAlloc failed (err=%lu).", ::GetLastError());
			return false;
		}
		PS5X_INFO("[GPU] GPU heap: %zu MB reserved via VirtualAlloc.", kSize / (1024 * 1024));
		return true;
#else
...
bool SetRenderTarget(uint32_t /*slot*/, const RenderTarget& rt)
{
	auto& st = GpuState::Get();
	st.currentRT = rt;
	PS5X_TRACE("[GPU] SetRenderTarget %ux%u fmt=%u", rt.width, rt.height, static_cast<uint32_t>(rt.format));
	return true;
}
...
bool SetDepthTarget(const DepthTarget& dt)
{
	...
	PS5X_TRACE("[GPU] SetDepthTarget %ux%u fmt=%u", dt.width, dt.height, static_cast<uint32_t>(dt.format));
	return true;
}
...
{
	auto& st = GpuState::Get();
	PS5X_TRACE("[GPU] Flip buf=%d arg=%lld", args.bufferIndex, static_cast<long long>(args.flipArg));
	if (st.backend)
		st.backend->Present();
...
{
	PS5X_TRACE("[GPU] Barrier gpuAddr=0x%llx %u→%u", static_cast<unsigned long long>(barrier.gpuAddr),
