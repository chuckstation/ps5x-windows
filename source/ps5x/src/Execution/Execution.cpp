...
struct ExecContext
{
	...
	static ExecContext& Get()
	{
		static ExecContext c;
		return c;
	}
};
...
void SetState(ExecState next)
{
	...
	PS5X_INFO("[Exec] State %s → %s", ExecStateName(prev), ExecStateName(next));

	std::lock_guard lk(ctx.mtx);
	for (auto& fn : ctx.onStateChange)
		fn(prev, next);
}
...
const char* ExecStateName(ExecState s)
{
	...
	case ExecState::Paused:
		return "Paused";
	case ExecState::Exiting:
		return "Exiting";
	case ExecState::Terminated:
		return "Terminated";
	case ExecState::Faulted:
		return "Faulted";
	}
	return "?";
}
...
bool LoadProgram(const std::filesystem::path& elfPath, const LoadOptions& opts)
{
	...
	if (ctx.state.load() != ExecState::Idle && ctx.state.load() != ExecState::Terminated)
	{
	...
	if (!opts.firmwarePath.empty())
	{
		...
		{
			PS5X_WARN("[Exec] Firmware validation: %s (continuing)", Loader::LoadResultStr(r));
		}
	}
	else
	{
	...
	if (!opts.contentPath.empty())
		Filesystem::Mount(Filesystem::MountPoint::App0, opts.contentPath, true);
	if (!opts.saveDataPath.empty())
		Filesystem::Mount(Filesystem::MountPoint::SaveData, opts.saveDataPath, false);
	if (!opts.firmwarePath.empty())
		Filesystem::Mount(Filesystem::MountPoint::System, opts.firmwarePath, true);
	...
	Process::RegisterExitCallback(
		[&ctx](uint32_t, int exitCode)
		{
			{
				std::lock_guard lk(ctx.mtx);
	...
	PS5X_INFO("[Exec] Program loaded in %.2f ms: %s  PID=%u", ctx.loadTimeMs, elfPath.filename().string().c_str(), pid);
	return true;
}
...
{
	auto& ctx = ExecContext::Get();
	if (ctx.state.load() != ExecState::Running && ctx.state.load() != ExecState::Paused)
		return false;
...
ExecState GetState()
{
	return ExecContext::Get().state.load();
}
bool IsRunning()
{
	return GetState() == ExecState::Running;
}
...
{
	ExecContext::Get().frames.fetch_add(1);
}
void NotifySyscall(uint32_t /*sysno*/)
{
	ExecContext::Get().syscalls.fetch_add(1);
}
...
{
	PS5X_WARN("[Exec] Guest exception vector=%u ec=0x%llx", vector, static_cast<unsigned long long>(errorCode));
	RuntimeEvents::PublishFault(errorCode, "Guest exception vector=" + std::to_string(vector));
	SetState(ExecState::Faulted);
}
...
const char* ExitReasonName(ExitReason r)
{
	...
	case ExitReason::Normal:
		return "Normal";
	case ExitReason::GuestPanic:
		return "GuestPanic";
	case ExitReason::Fault:
		return "Fault";
	case ExitReason::Timeout:
		return "Timeout";
	case ExitReason::Requested:
		return "Requested";
...
void ReportFault(uint64_t faultAddr, const std::string& description)
{
	...
	auto& ctx = ExecContext::Get();
	std::lock_guard lk(ctx.mtx);
	for (auto& fn : ctx.onFault)
		fn(faultAddr, description);
	SetState(ExecState::Faulted);
	RuntimeEvents::PublishFault(faultAddr, description);
	PS5X_ERROR("[Exec] Fault @ 0x%llx: %s", static_cast<unsigned long long>(faultAddr), description.c_str());
}
