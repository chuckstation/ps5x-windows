// PS5x – Input Mapping System implementation
...
static const char* Ps5ButtonName(Ps5Button b)
{
	switch (b)
	{
	case Ps5Button::Triangle:
		return "Triangle";
	case Ps5Button::Circle:
		return "Circle";
	case Ps5Button::Cross:
		return "Cross";
	case Ps5Button::Square:
		return "Square";
	case Ps5Button::L1:
		return "L1";
	case Ps5Button::R1:
	...
	case Ps5Button::R2:
		return "R2";
	case Ps5Button::Create:
		return "Create";
	case Ps5Button::Options:
		return "Options";
	case Ps5Button::PS:
		return "PS";
	case Ps5Button::TouchPad:
		return "TouchPad";
	case Ps5Button::L3:
		return "L3";
	case Ps5Button::R3:
		return "R3";
	case Ps5Button::DPadUp:
		return "DPadUp";
	case Ps5Button::DPadDown:
		return "DPadDown";
	case Ps5Button::DPadLeft:
		return "DPadLeft";
	case Ps5Button::DPadRight:
		return "DPadRight";
	case Ps5Button::GyroX:
		return "GyroX";
	case Ps5Button::GyroY:
		return "GyroY";
	case Ps5Button::GyroZ:
		return "GyroZ";
	default:
		return "Unknown";
	}
...
static const char* HostInputTypeName(HostInputType t)
{
	...
	case HostInputType::Mouse:
		return "Mouse";
	case HostInputType::Gamepad:
		return "Gamepad";
	case HostInputType::Touch:
		return "Touch";
	default:
		return "Unknown";
	}
...
{
	if (name == "Keyboard")
		return HostInputType::Keyboard;
	if (name == "Mouse")
		return HostInputType::Mouse;
	if (name == "Gamepad")
		return HostInputType::Gamepad;
	if (name == "Touch")
		return HostInputType::Touch;
...
struct InputMappingState
{
	...
	static InputMappingState& Get()
	{
		static InputMappingState s;
		return s;
	}
};
...
MappingProfile DefaultDualSenseProfile()
{
	...
	for (uint16_t i = 0; i < static_cast<uint16_t>(Ps5Button::Count); ++i)
	{
		...
		if (i >= static_cast<uint16_t>(Ps5Button::GyroX) && i <= static_cast<uint16_t>(Ps5Button::GyroZ))
			continue;
		...
		e.scale = 1.0f;
		e.deadzone =
			(i >= static_cast<uint16_t>(Ps5Button::L2) && i <= static_cast<uint16_t>(Ps5Button::R2)) ? 0.05f : 0.0f;
		e.inverted = false;
...
static bool WriteProfileIni(const MappingProfile& profile, const std::filesystem::path& path)
{
	...
	if (!f.is_open())
	{
		PS5X_ERROR("[InputMapping] Cannot write profile to '%s'.", path.string().c_str());
		return false;
	}
	...
	f.close();
	PS5X_INFO("[InputMapping] Saved profile '%s' to %s (%zu mappings).", profile.name.c_str(), path.string().c_str(),
...
static std::string GetKv(const IniSection& s, const std::string& key, const std::string& defaultVal = "")
{
...
static bool ReadProfileIni(const std::filesystem::path& path, MappingProfile& outProfile)
{
	...
	if (sections.empty())
	{
		PS5X_ERROR("[InputMapping] No sections in INI '%s'.", path.string().c_str());
		return false;
	}
	...
	PS5X_INFO("[InputMapping] Loaded profile '%s' from %s (%zu mappings).", outProfile.name.c_str(),
			  path.string().c_str(), outProfile.entries.size());
	return true;
}
...
bool Init()
{
	...
	if (ec)
	{
		PS5X_ERROR("[InputMapping] Failed to create profile directory: %s", ec.message().c_str());
	...
	PS5X_INFO("[InputMapping] Initialised. Profile dir: %s", st.profileDir.string().c_str());
	return true;
}
...
void Shutdown()
{
	...
	st.initialised = false;
	PS5X_INFO("[InputMapping] Shutdown.");
}
...
bool LoadProfile(const std::string& name)
{
	...
	if (!st.initialised)
	{
		PS5X_ERROR("[InputMapping] Not initialised.");
		return false;
	}
	...
	{
		PS5X_ERROR("[InputMapping] Profile '%s' not found at %s.", name.c_str(), path.string().c_str());
		return false;
	}

	MappingProfile loaded;
	if (!ReadProfileIni(path, loaded))
		return false;

	st.activeProfile = std::move(loaded);
	PS5X_INFO("[InputMapping] Active profile set to '%s'.", st.activeProfile.name.c_str());
	return true;
}
...
bool DeleteProfile(const std::string& name)
{
	...
	if (ok)
	{
		PS5X_INFO("[InputMapping] Deleted profile '%s'.", name.c_str());
	}
	else
	{
		PS5X_ERROR("[InputMapping] Failed to delete profile '%s': %s", name.c_str(), ec.message().c_str());
	}
	return ok;
}
...
void SetActiveProfile(const MappingProfile& profile)
{
	...
	PS5X_INFO("[InputMapping] Active profile changed to '%s' (%zu mappings).", profile.name.c_str(),
			  profile.entries.size());
}
