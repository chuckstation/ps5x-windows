...
static bool WriteProfileIni(const MappingProfile& profile, const std::filesystem::path& path)
{
    ...
    f.close();
    PS5X_INFO("[InputMapping] Saved profile '%s' to %s (%zu mappings).",
...
static bool ReadProfileIni(const std::filesystem::path& path, MappingProfile& outProfile)
{
    ...
    PS5X_INFO("[InputMapping] Loaded profile '%s' from %s (%zu mappings).",
...
bool DeleteProfile(const std::string& name)
{
    ...
    if (ok) {
        ...
        PS5X_ERROR("[InputMapping] Failed to delete profile '%s': %s",
...
void SetActiveProfile(const MappingProfile& profile)
{
    ...
    PS5X_INFO("[InputMapping] Active profile changed to '%s' (%zu mappings).",
