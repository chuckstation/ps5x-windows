				  ...
				  bool isMain)
{
	...
	if (byNameIt != rs.byName.end())
	{
		...
		existing.refCount++;
		PS5X_DEBUG("[ModReg] '%s' already registered (refCount=%u)", name.c_str(), existing.refCount);
	...
	PS5X_INFO("[ModReg] Registered '%s' id=%u exports=%zu", name.c_str(), id, rs.modules[id].exports.size());
	return id;
}
...
ModuleId Load(const std::filesystem::path& path)
{
	...
	{
		PS5X_ERROR("[ModReg] Load '%s' failed: %s", path.string().c_str(), Loader::LoadResultStr(r));
...
bool Unload(ModuleId id)
{
	...
	for (auto& [oid, om] : rs.modules)
	{
		...
		for (auto dep : om.deps)
			if (dep == id)
			{
				PS5X_WARN("[ModReg] Cannot unload '%s' – '%s' depends on it", m.name.c_str(), om.name.c_str());
				return false;
			}
	}
...
bool ApplyRelocations(ModuleId id)
{
	...
	{
		const uint8_t* data = reinterpret_cast<const uint8_t*>(base + rela_section.offset);
		...
		for (size_t i = 0; i < count; ++i)
		{
			...
			if (symId < m.elfInfo.symbols.size())
			{
				...
				else
				{
					...
					if (!S)
					{
						PS5X_WARN("[ModReg] Unresolved symbol '%s' in '%s'", sym.name.c_str(), m.name.c_str());
						++skipped;
						continue;
					}
				}
			...
			auto patch64 = [&](uint64_t val) { std::memcpy(reinterpret_cast<void*>(P), &val, 8); };
			auto patch32 = [&](uint32_t val) { std::memcpy(reinterpret_cast<void*>(P), &val, 4); };
			...
			case 2: /* R_X86_64_PC32 */
				patch32(static_cast<uint32_t>(S + A - P));
				break;
			...
			case 8: /* R_X86_64_RELATIVE */
				...
				break;
			case 10: /* R_X86_64_32 */
				patch32(static_cast<uint32_t>(S + A));
				break;
			case 11: /* R_X86_64_32S */
				patch32(static_cast<uint32_t>(static_cast<int64_t>(S) + r.addend));
				break;
			default:
				PS5X_WARN("[ModReg] Unhandled reloc type %u in '%s'", type, m.name.c_str());
				++skipped;
				continue;
			}
			++processed;
	...
	for (const auto& needed : m.elfInfo.needed)
	{
		if (rs.byName.find(needed) == rs.byName.end())
			PS5X_WARN("[ModReg] '%s' needs '%s' (not loaded)", m.name.c_str(), needed.c_str());
	}

	PS5X_INFO("[ModReg] Relocated '%s': %u applied, %u skipped.", m.name.c_str(), processed, skipped);
	return true;
}
...
void DumpModules()
{
	auto& rs = RegistryState::Get();
	std::lock_guard lk(rs.mtx);
	PS5X_INFO("[ModReg] === Module Table (%u modules) ===", static_cast<unsigned>(rs.modules.size()));
	for (auto id : rs.loadOrder)
	{
		auto it = rs.modules.find(id);
		if (it == rs.modules.end())
			continue;
		const auto& m = it->second;
		PS5X_INFO("[ModReg]  [%3u] %-30s  refs=%u  exports=%zu  %s", id, m.name.c_str(), m.refCount, m.exports.size(),
				  m.isMain ? "[MAIN]" : "");
	}
}
...
void DumpSymbols(ModuleId id)
{
	...
	for (const auto& sym : m.exports)
		PS5X_INFO("[ModReg]   0x%016llx  %s  sz=%zu", static_cast<unsigned long long>(sym.address), sym.name.c_str(),
				  sym.size);
}
