...
// ── ScopeTimer ────────────────────────────────────────────────────────────
ScopeTimer::ScopeTimer(const std::string& name) : name_(name), startNs_(NowNs()) {}
...
void RecordFrameTime(double ms)
{
	auto& st = PerfState::Get();
	std::lock_guard lk(st.mtx);
	if (st.frameTimes.size() >= kFrameWindow)
...
void ProfileEnd(uint64_t id)
{
	...
	{
		double ms =
			std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - it->second.start).count();
...
std::vector<ProfileReportEntry> GetProfileReport()
{
	...
	std::sort(out.begin(), out.end(),
			  [](const ProfileReportEntry& a, const ProfileReportEntry& b) { return a.totalMs > b.totalMs; });
	return out;
}
...
BenchmarkResult Benchmark(const std::string& label, WorkFn fn, uint32_t runs, uint32_t warmups)
{
	...
	PS5X_DEBUG("[PerfTools] Benchmark '%s': %.3f ms avg over %u runs.", label.c_str(), r.avgMs, runs);
	return r;
}
...
std::vector<LockStatEntry> GetLockReport()
{
	...
	std::sort(out.begin(), out.end(),
			  [](const LockStatEntry& a, const LockStatEntry& b) { return a.contentions > b.contentions; });
	return out;
}
...
{
	PS5X_INFO("[PerfTools] Startup Report:");
	for (const auto& mark : g_startupMarks)
	{
		PS5X_INFO("  %s", mark.first.c_str());
	}
}
