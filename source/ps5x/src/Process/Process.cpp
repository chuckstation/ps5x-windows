                ...
                const std::filesystem::path& firmwarePath)
{
    ...
    {
        PS5X_WARN("[Process] Firmware warning: %s",
    ...
    if (loadResult != Loader::LoadResult::Ok) {
        PS5X_ERROR("[Process] LoadExecutable failed: %s",
    ...
    KernelRuntime::KHandle mainTh = KernelRuntime::CreateThread(
        [entryPoint](void*) -> int {
            ...
            PS5X_INFO("[Process] Guest main thread ready, entry=0x%llx",
    ...
    PS5X_INFO("[Process] Created PID=%u title='%s' entry=0x%llx",
...
bool Start(uint32_t pid)
{
    ...
    if (ok) {
        PS5X_INFO("[Process] PID=%u started (main thread h=%d).",
...
bool LoadModule(uint32_t pid, const std::filesystem::path& path)
{
    ...
    if (auto r = Loader::LoadExecutable(path, info); r != Loader::LoadResult::Ok) {
        PS5X_ERROR("[Process] LoadModule '%s' failed: %s",
