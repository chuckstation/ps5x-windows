...
static constexpr std::array<const char*, 16> kRegNames = {"rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi",
...
{
	uint64_t mask = (bits == 64) ? UINT64_MAX : (bits == 32) ? UINT32_MAX : (bits == 16) ? 0xFFFF : 0xFF;
	result &= mask;
...
static void UpdateFlagsArith(CpuContext& ctx, uint64_t a, uint64_t b, uint64_t result, bool isSub, int bits)
{
	uint64_t mask = (bits == 64) ? UINT64_MAX : (bits == 32) ? UINT32_MAX : (bits == 16) ? 0xFFFF : 0xFF;
...
static StepResult ExecuteOne(CpuState& st)
{
	...
	if (!st.bpByAddr.empty())
	{
		...
		if (it != st.bpByAddr.end())
		{
			...
			PS5X_DEBUG("[Cpu] Breakpoint hit @ 0x%llx", static_cast<unsigned long long>(ctx.rip));
	...
	case 0x83:
	{
		...
		default:
			PS5X_WARN("[Cpu] 0x83 /%u not implemented @ 0x%llx", ext, static_cast<unsigned long long>(saved_rip));
	...
	case 0x03:
	{
		...
		else
		{
			...
			{
				int8_t d;
				std::memcpy(&d, ip + off - 1, 1);
				ctx.rip++;
				disp = d;
			}
			uint64_t ea = ctx.gpr[rm] + disp;
			uint64_t b = 0;
			if (!GuestRead<uint64_t>(ea, b))
			{
				++st.stats.faults;
				return StepResult::Fault;
			}
	...
	case 0x89:
	{
		...
		else
		{
			...
			if (rex_w)
			{
				if (!GuestWrite<uint64_t>(ea, ctx.gpr[reg]))
				{
					++st.stats.faults;
					return StepResult::Fault;
				}
			...
			{
				if (!GuestWrite<uint32_t>(ea, static_cast<uint32_t>(ctx.gpr[reg])))
				{
					++st.stats.faults;
					return StepResult::Fault;
				}
	...
	case 0x7F:
	{
		...
		bool taken = false;
		bool cf = ctx.flag(Flags::CF), zf = ctx.flag(Flags::ZF), sf = ctx.flag(Flags::SF), of = ctx.flag(Flags::OF),
	...
	case 0xCD:
	{
		...
		PS5X_WARN("[Cpu] INT 0x%02x @ 0x%llx", vec, static_cast<unsigned long long>(saved_rip));
		return StepResult::Unimplemented;
	}
	...
	case 0x0F:
	{
		...
		if (op2 >= 0x40 && op2 <= 0x4F)
		{
			...
			if ((modrm >> 6) == 3)
			{
				uint8_t cc = op2 & 0xF;
				bool taken = false;
				bool cf = ctx.flag(Flags::CF), zf = ctx.flag(Flags::ZF), sf = ctx.flag(Flags::SF),
					 of = ctx.flag(Flags::OF);
				switch (cc)
				{
				case 0x0:
					taken = of;
		...
		PS5X_WARN("[Cpu] 0x0F 0x%02x not implemented @ 0x%llx", op2, static_cast<unsigned long long>(saved_rip));
		++st.stats.unimplemented;
		return StepResult::Unimplemented;
	}
	...
	case 0xC7:
	{
		...
		if (ext == 0 && (modrm >> 6) == 3)
		{
			ctx.gpr[rm] = rex_w ? static_cast<uint64_t>(imm) : static_cast<uint64_t>(static_cast<uint32_t>(imm));
		}
		break;
	}
	...
	case 0xD3:
	{
		...
		{
			count = static_cast<uint8_t>(ctx.gpr[static_cast<size_t>(Reg::RCX)]) & 0x3F;
		}
		Advance();
		if ((modrm >> 6) == 3)
		{
			...
			{ // SAR
				...
				uint64_t r = (count < 64) ? static_cast<uint64_t>(sv >> count) : static_cast<uint64_t>(sv >> 63);
	...
	default:
		PS5X_WARN("[Cpu] Unimplemented opcode 0x%02x @ 0x%llx", opcode, static_cast<unsigned long long>(saved_rip));
		++st.stats.unimplemented;
		return StepResult::Unimplemented;
	}
...
const char* StepResultName(StepResult r)
{
	...
	case StepResult::Ok:
		return "Ok";
	case StepResult::Breakpoint:
		return "Breakpoint";
	case StepResult::Syscall:
		return "Syscall";
	case StepResult::Fault:
		return "Fault";
	case StepResult::Halt:
		return "Halt";
	case StepResult::Unimplemented:
...
void Pause()
{
	CpuState::Get().paused.store(true);
}
void Resume()
{
	CpuState::Get().paused.store(false);
}
void Stop()
{
	CpuState::Get().running.store(false);
}
bool IsRunning()
{
	return CpuState::Get().running.load();
}
...
std::optional<DecodedInsn> Decode(const uint8_t* bytes, size_t maxBytes)
{
	...
	static const std::unordered_map<uint8_t, const char*> mnemonics = {
		{0x03, "add"},          {0x0B, "or"},   {0x23, "and"}, {0x2B, "sub"},       {0x33, "xor"}, {0x39, "cmp"},
		{0x3B, "cmp"},          {0x50, "push"}, {0x58, "pop"}, {0x69, "imul"},      {0x81, "alu"}, {0x83, "alu"},
		{0x85, "test"},         {0x89, "mov"},  {0x8B, "mov"}, {0x8D, "lea"},       {0x90, "nop"}, {0x9E, "sahf"},
		{0x9F, "lahf"},         {0xA9, "test"}, {0xC1, "shl"}, {0xC2, "ret"},       {0xC3, "ret"}, {0xC7, "mov"},
		{0xD3, "shl"},          {0xE8, "call"}, {0xE9, "jmp"}, {0xEB, "jmp short"}, {0xF4, "hlt"}, {0xF7, "alu"},
		{0xFF, "call/jmp/inc"}, {0xCD, "int"},
	};
...
uint32_t AddBreakpoint(uint64_t addr, const std::string& label)
{
	...
	st.bpByAddr[addr] = id;
	PS5X_DEBUG("[Cpu] Breakpoint %u set @ 0x%llx (%s)", id, static_cast<unsigned long long>(addr), label.c_str());
	return id;
}
...
bool RemoveBreakpoint(uint32_t id)
{
	auto& st = CpuState::Get();
	std::lock_guard lk(st.mtx);
	auto it = std::find_if(st.breakpoints.begin(), st.breakpoints.end(), [id](const BpEntry& b) { return b.id == id; });
...
{
	return {
		0x03, 0x0B, 0x0F, 0x23, 0x2B, 0x33, 0x39, 0x3B, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57,
		0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F, 0x69, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76,
		0x77, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 0x7F, 0x81, 0x83, 0x85, 0x89, 0x8B, 0x8D, 0x90,
		0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x9E, 0x9F, 0xA9, 0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD,
		0xBE, 0xBF, 0xC1, 0xC2, 0xC3, 0xC7, 0xCD, 0xD3, 0xE8, 0xE9, 0xEB, 0xF4, 0xF7, 0xFF,
	};
}
