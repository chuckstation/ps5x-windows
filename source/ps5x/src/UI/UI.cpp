// PS5x – UI implementation (Windows-native, Phase 8)
...
const char* PanelName(Panel p)
{
	switch (p)
	{
	case Panel::ExecutionDashboard:
		return "Execution Dashboard";
	case Panel::RuntimeTimeline:
		return "Runtime Timeline";
	case Panel::MemoryInspector:
		return "Memory Inspector";
	case Panel::GpuStatistics:
		return "GPU Statistics";
	case Panel::EventViewer:
		return "Event Viewer";
	case Panel::ControllerMonitor:
		return "Controller Monitor";
	case Panel::FilesystemBrowser:
		return "Filesystem Browser";
	case Panel::PerformanceGraphs:
		return "Performance Graphs";
	case Panel::ThreadInspector:
		return "Thread Inspector";
	case Panel::ModuleViewer:
		return "Module Viewer";
	case Panel::ShaderCacheViewer:
		return "Shader Cache Viewer";
	default:
		return "Unknown";
	}
}
...
bool Init(void*)
{
	...
	PS5X_INFO("[UI] Initialised (Phase 8).");
	return true;
}
...
void Run()
{
	...
	PS5X_INFO("[UI] Run() started.");

#if defined(_WIN32)
	// Win32 message pump.  The ImGui render loop is layered on top of this
	// via the ImGui Win32 + DX12 backend (linked when PS5X_ENABLE_UI_WINDOW
	...
	PS5X_INFO("[UI] Win32 message pump exited.");
#else
	...
	st.running.store(false);
	PS5X_INFO("[UI] Run() returned.");
}

void RequestExit()
{
	UIState::Get().running.store(false);
}
...
void SetGameTitle(const std::string& t)
{
	std::lock_guard lk(UIState::Get().mtx);
	UIState::Get().gameTitle = t;
}
void SetEmulationActive(bool a)
{
	UIState::Get().emulationActive.store(a);
}
void SetFps(float f)
{
	UIState::Get().fps.store(f);
}
void SetBackendName(const std::string& n)
{
	std::lock_guard lk(UIState::Get().mtx);
	UIState::Get().backendName = n;
}
...
bool SaveWorkspace(const std::string& path)
{
	...
	if (!f.is_open())
	{
		PS5X_ERROR("[UI] SaveWorkspace: cannot write '%s'", path.c_str());
		return false;
	}
	auto& st = UIState::Get();
	std::lock_guard lk(st.mtx);
	...
	PS5X_INFO("[UI] Workspace saved to '%s'.", path.c_str());
	return true;
}
...
bool LoadWorkspace(const std::string& path)
{
	...
	if (!f.is_open())
	{
		PS5X_WARN("[UI] LoadWorkspace: '%s' not found — using defaults.", path.c_str());
		return false;
	}
	auto& st = UIState::Get();
	std::lock_guard lk(st.mtx);
	...
	while (std::getline(f, line))
	{
		...
		if (key.substr(0, 6) == "panel_")
		{
			...
			if (idx < 0 || idx >= static_cast<int>(st.panels.size()))
				continue;
			auto& p = st.panels[idx];
			std::string field = key.substr(sep1 + 1);
			if (field == "visible")
				p.visible = (value != "0");
			else if (field == "docked")
				p.docked = (value != "0");
			else if (field == "x")
				p.x = std::stof(value);
			else if (field == "y")
				...
				p.width = std::stof(value);
			else if (field == "h")
				p.height = std::stof(value);
		}
	}
	PS5X_INFO("[UI] Workspace loaded from '%s'.", path.c_str());
	return true;
}
...
void AddRecentEntry(const std::string& path, const std::string& label)
{
	...
	std::lock_guard lk(st.mtx);
	// Remove duplicate
	st.recentList.erase(std::remove_if(st.recentList.begin(), st.recentList.end(),
...
void SetTheme(const std::string& name)
{
	...
	PS5X_DEBUG("[UI] Theme set to '%s'.", name.c_str());
}
...
void LoadLayout()
{
	...
	if (!welcome.empty())
		st.welcomeEnabled = (welcome != "0");
	auto theme = Config::Get("ui.theme");
	if (!theme.empty())
		st.theme.name = theme;
	auto r = Config::Get("ui.accent_r");
	if (!r.empty())
		st.accentColor.r = static_cast<uint8_t>(std::stoi(r));
	auto g = Config::Get("ui.accent_g");
	if (!g.empty())
		st.accentColor.g = static_cast<uint8_t>(std::stoi(g));
	auto b = Config::Get("ui.accent_b");
	if (!b.empty())
		st.accentColor.b = static_cast<uint8_t>(std::stoi(b));
	...
	if (!countStr.empty())
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			if (!side.empty())
				p.side = static_cast<DockSide>(std::stoi(side));
			auto size = Config::Get(prefix + "size");
			if (!size.empty())
				p.size = std::stoi(size);
			if (!p.id.empty())
				st.dockPanels[p.id] = p;
		}
	}
	PS5X_DEBUG("[UI] Layout loaded.");
}
...
void IngestLogLine(const std::string& line)
{
	auto& st = UIState::Get();
	std::lock_guard lk(st.mtx);
	if (st.logViewer.size() >= kMaxLogLines)
		st.logViewer.pop_front();
...
std::vector<std::string> SearchLogViewer(const std::string& query, bool caseSensitive)
{
	...
	std::lock_guard lk(st.mtx);
	if (query.empty())
		return std::vector<std::string>(st.logViewer.begin(), st.logViewer.end());
...
std::vector<std::string> FilterLogByLevel(LogLevel level)
{
	...
	for (auto& line : st.logViewer)
	{
		if (prefix && line.find(prefix) != std::string::npos)
			out.push_back(line);
	}
	return out;
}
...
float GetDashboardFPS()
{
	...
	if (stats.samples == 0 || stats.avgMs <= 0.0)
		return 0.0f;
