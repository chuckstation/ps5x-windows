// PS5x – Input implementation (Phase 2 – SDL2 backend)
...
InputMode DetectMode(SDL_GameController* gc)
{
	...
	if (vid == VID_SONY)
	{
		if (pid == PID_DUALSENSE)
			return InputMode::DualSense;
		if (pid == PID_DS4_V1 || pid == PID_DS4_V2)
			return InputMode::DS4;
	}
...
void UpdateSlotFromGamepad(ControllerSlot& slot)
{
	...
	auto btn = [&](SDL_GameControllerButton b) -> bool { return SDL_GameControllerGetButton(gc, b) != 0; };

	if (btn(SDL_CONTROLLER_BUTTON_A))
		ps.buttons |= BTN_CROSS;
	if (btn(SDL_CONTROLLER_BUTTON_B))
		ps.buttons |= BTN_CIRCLE;
	if (btn(SDL_CONTROLLER_BUTTON_X))
		ps.buttons |= BTN_SQUARE;
	if (btn(SDL_CONTROLLER_BUTTON_Y))
		ps.buttons |= BTN_TRIANGLE;
	...
	if (ps.leftTrigger > 0.5f)
		ps.buttons |= BTN_L2;
	if (ps.rightTrigger > 0.5f)
		ps.buttons |= BTN_R2;
...
{
	if (!SDL_IsGameController(sdlIdx))
		return;
	int slot = FindFreeSlot(st);
	if (slot < 0)
	{
		PS5X_WARN("[Input] No free pad slot.");
		return;
	}
	...
	if (!gc)
	{
		PS5X_WARN("[Input] SDL_GameControllerOpen failed: %s", SDL_GetError());
		return;
	}
	...
	PS5X_INFO("[Input] Pad %d connected: %s (mode=%u)", slot, SDL_GameControllerName(gc),
...
void CloseController(InputState& st, SDL_JoystickID jid)
{
	...
	{
		auto& sl = st.slots[i];
		if (!sl.active || sl.jid != jid)
			continue;
		if (sl.haptic)
		{
			SDL_HapticClose(sl.haptic);
			sl.haptic = nullptr;
		}
		...
		sl.active = false;
		sl.state = PadState{};
		PS5X_INFO("[Input] Pad %d disconnected.", i);
		return;
	}
}
#endif // PS5X_HAVE_SDL2
...
void Init()
{
	...
	if (SDL_WasInit(SDL_INIT_GAMECONTROLLER) == 0)
	{
		if (SDL_InitSubSystem(SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC) == 0)
			...
			PS5X_WARN("[Input] SDL GameController init failed: %s", SDL_GetError());
	}
	...
	for (int i = 0; i < SDL_NumJoysticks(); ++i)
		OpenController(st, i);
#else
	PS5X_WARN("[Input] SDL2 not available – keyboard virtual pad only.");
#endif

	st.initialised = true;
	int pads = 0;
	for (const auto& sl : st.slots)
		if (sl.active)
			++pads;
	PS5X_INFO("[Input] Initialised. %d controller(s) connected.", pads - 1);
}
...
struct InputExtState
{
	...
	static InputExtState& Get()
	{
		static InputExtState s;
		return s;
	}
};
...
void SetMotionEnabled(int idx, bool enable)
{
	...
	PS5X_DEBUG("[Input] Motion sensor pad%d %s", idx, enable ? "on" : "off");
}
...
bool SetControllerProfile(int idx, const ControllerProfile& profile)
{
	...
	PS5X_INFO("[Input] Profile '%s' applied to pad%d", profile.name.c_str(), idx);
	return true;
}
...
void ResetControllerProfile(int idx)
{
	...
	PS5X_DEBUG("[Input] Profile reset for pad%d", idx);
}
...
void StartRecording()
{
	...
	PS5X_INFO("[Input] Recording started");
}
...
void StopRecording()
{
	...
	PS5X_INFO("[Input] Recording stopped (%zu frames)", s.recording_frames.size());
}

bool IsRecording()
{
	return InputExtState::Get().recording.load();
}
...
bool StartPlayback(const std::vector<InputFrame>& frames)
{
	...
	PS5X_INFO("[Input] Playback started (%zu frames)", frames.size());
	return true;
}
...
void StopPlayback()
{
	...
	PS5X_INFO("[Input] Playback stopped");
}

bool IsPlayingBack()
{
	return InputExtState::Get().playback.load();
}
