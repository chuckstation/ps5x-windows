...
void OpenController(InputState& st, int sdlIdx) {
    ...
    if (slot < 0) { PS5X_WARN("[Input] No free pad slot."); return; }
    ...
    PS5X_INFO("[Input] Pad %d connected: %s (mode=%u)",
...
bool SetControllerProfile(int idx, const ControllerProfile& profile)
{
    ...
    PS5X_INFO("[Input] Profile '%s' applied to pad%d",
...
void StopRecording()
{
    ...
    PS5X_INFO("[Input] Recording stopped (%zu frames)",
