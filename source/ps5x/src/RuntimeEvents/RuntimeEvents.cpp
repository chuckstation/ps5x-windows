// PS5x – Runtime Event Queue implementation
...
const char* EventTypeName(EventType t)
{
	...
	case EventType::ProcessFaulted:
		return "ProcessFaulted";
	case EventType::ThreadSpawned:
		return "ThreadSpawned";
	case EventType::ThreadExited:
		return "ThreadExited";
	case EventType::ThreadFaulted:
		return "ThreadFaulted";
	case EventType::ModuleLoaded:
		return "ModuleLoaded";
	case EventType::ModuleUnloaded:
		return "ModuleUnloaded";
	case EventType::SymbolResolved:
		return "SymbolResolved";
	case EventType::SymbolMissing:
		return "SymbolMissing";
	case EventType::PageFault:
		return "PageFault";
	case EventType::MemoryProtViolation:
		return "MemoryProtViolation";
	case EventType::MemoryLeak:
		return "MemoryLeak";
	case EventType::FrameBegin:
		return "FrameBegin";
	case EventType::FrameEnd:
		return "FrameEnd";
	case EventType::SwapchainResize:
		return "SwapchainResize";
	case EventType::ShaderCompiled:
		return "ShaderCompiled";
	case EventType::SyscallEnter:
		return "SyscallEnter";
	case EventType::SyscallReturn:
		return "SyscallReturn";
	case EventType::SyscallUnknown:
		return "SyscallUnknown";
	case EventType::WatchdogWarning:
		return "WatchdogWarning";
	case EventType::WatchdogTimeout:
		return "WatchdogTimeout";
	case EventType::ProfileMark:
		return "ProfileMark";
	case EventType::ProfileBegin:
		return "ProfileBegin";
...
bool Init(size_t ringCapacity)
{
	...
	PS5X_INFO("[Events] Initialised. Ring capacity=%zu", ringCapacity);
	return true;
}
...
void Shutdown()
{
	...
	PS5X_INFO("[Events] Shutdown. Total events published: %llu", static_cast<unsigned long long>(es.totalCount.load()));
}
...
{
	PS5X_INFO("[Events] Process exit: code=%d %s", exitCode, reason.c_str());
	Publish(EventType::ProcessExited, ProcessExitedPayload{exitCode, std::move(reason)});
}

void PublishFault(uint64_t faultAddr, std::string desc)
{
	PS5X_ERROR("[Events] Fault @ 0x%llx: %s", static_cast<unsigned long long>(faultAddr), desc.c_str());
	Publish(EventType::ProcessFaulted, ProcessFaultedPayload{faultAddr, std::move(desc)});
}

void PublishSyscall(uint32_t sysno, const std::string& name, int64_t result)
{
	Publish(EventType::SyscallReturn, SyscallPayload{sysno, name, result});
}

void PublishFrame(uint64_t frameIdx, double gpuMs, double cpuMs)
{
	Publish(EventType::FrameEnd, FramePayload{frameIdx, gpuMs, cpuMs});
}
...
void Unsubscribe(uint32_t handlerId)
{
	auto& es = EventState::Get();
	std::lock_guard lk(es.handlerMtx);
	es.handlers.erase(std::remove_if(es.handlers.begin(), es.handlers.end(),
	...
	es.watchdog.thread = std::thread(
		[&wd = es.watchdog]()
		{
			while (wd.running.load())
			{
				...
				{
					Publish(EventType::WatchdogTimeout, CustomPayload{"watchdog", "guest timeout"});
					PS5X_ERROR("[Events] Watchdog TIMEOUT after %llu ms",
							   static_cast<unsigned long long>(elapsed / 1000));
					break;
				}
				else if (elapsed >= wd.cfg.warningUs)
				{
					Publish(EventType::WatchdogWarning, CustomPayload{"watchdog", "guest unresponsive"});
					PS5X_WARN("[Events] Watchdog warning: guest unresponsive for %llu ms",
							  static_cast<unsigned long long>(elapsed / 1000));
				}
			}
		});
	PS5X_INFO("[Events] Watchdog armed: warning=%llums timeout=%llums",
...
// ── RAII scope timer ──────────────────────────────────────────────────────
ScopeTimer::ScopeTimer(std::string label) : label_(std::move(label)), t0_(Clock::now())
{
...
{
	uint64_t dur =
		static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::microseconds>(Clock::now() - t0_).count());
...
EventCategory Categorise(EventType t)
{
	...
	switch (t)
	{
	case ET::ProcessCreated:
	case ET::ProcessStarted:
	case ET::ProcessExited:
	case ET::ProcessFaulted:
		return EC::Lifecycle;
	case ET::ThreadSpawned:
	case ET::ThreadExited:
	case ET::ThreadFaulted:
		return EC::Thread;
	case ET::ModuleLoaded:
	case ET::ModuleUnloaded:
	case ET::SymbolResolved:
	case ET::SymbolMissing:
		return EC::Module;
	case ET::PageFault:
	case ET::MemoryProtViolation:
	case ET::MemoryLeak:
		return EC::Memory;
	case ET::FrameBegin:
	case ET::FrameEnd:
	case ET::SwapchainResize:
	case ET::ShaderCompiled:
		return EC::GPU;
	case ET::SyscallEnter:
	case ET::SyscallReturn:
	case ET::SyscallUnknown:
		return EC::Syscall;
	case ET::WatchdogWarning:
	case ET::WatchdogTimeout:
		return EC::Lifecycle;
	case ET::ProfileMark:
	case ET::ProfileBegin:
	case ET::ProfileEnd:
		return EC::Profile;
	case ET::Custom:
		return EC::Custom;
...
{
	return Subscribe(
		[cat, fn = std::move(fn)](const RuntimeEvent& ev)
		{
			if (Categorise(ev.type) == cat)
				fn(ev);
		});
}
...
std::string ExportTimelineJson(size_t maxEntries)
{
	...
	{
		auto& e = entries[i];
		out += "  {\"ts\":" + std::to_string(e.timestampUs) + ",\"type\":\"" + EventTypeName(e.type) + "\"" +
			   ",\"label\":\"" + e.label + "\"" + ",\"dur\":" + std::to_string(e.durationUs) + "}";
		if (i + 1 < entries.size())
			out += ',';
		out += '\n';
	}
...
std::string ExportTimelineCsv(size_t maxEntries)
{
	...
	for (auto& e : entries)
	{
		out += std::to_string(e.timestampUs) + ',' + EventTypeName(e.type) + ',' + '"' + e.label + '"' + ',' +
