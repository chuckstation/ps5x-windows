...
void TakeSnapshot(std::string_view label)
{
	...
	PS5X_DEBUG("[MemDiag] Snapshot '%.*s': regions=%u committed=%zu", static_cast<int>(label.size()), label.data(),
...
std::string DiffSnapshots(const Snapshot& a, const Snapshot& b)
{
	...
	int64_t dc = static_cast<int64_t>(b.stats.totalCommitted) - static_cast<int64_t>(a.stats.totalCommitted);
	int64_t dr = static_cast<int64_t>(b.stats.regionCount) - static_cast<int64_t>(a.stats.regionCount);

	oss << "  committed: " << a.stats.totalCommitted << " → " << b.stats.totalCommitted << "  (" << (dc >= 0 ? "+" : "")
		<< dc << " bytes)\n";
	oss << "  regions:   " << a.stats.regionCount << " → " << b.stats.regionCount << "  (" << (dr >= 0 ? "+" : "") << dr
		<< ")\n";
	...
	for (const auto& rb : b.regions)
	{
		...
		for (const auto& ra : a.regions)
			if (ra.base == rb.base && ra.size == rb.size)
			{
				found = true;
				break;
			}
		if (!found)
			oss << "  + 0x" << std::hex << rb.base << "+" << rb.size << " [" << rb.tag << "]\n";
	}
	...
	for (const auto& ra : a.regions)
	{
		...
		if (!found)
			oss << "  - 0x" << std::hex << ra.base << "+" << ra.size << " [" << ra.tag << "]\n";
	}
...
{
	auto& d = DiagState::Get();
	if (!d.historyEnabled.load(std::memory_order_relaxed))
		return;
	std::lock_guard lk(d.mtx);
...
FragReport ComputeFragmentation()
{
	...
	if (stats.totalReserved > 0)
		rep.fragmentationPct =
			100.0 * static_cast<double>(stats.totalCommitted) / static_cast<double>(stats.totalReserved);
...
MemOverlay GetOverlay()
{
	...
	ov.usagePct = (s.totalReserved > 0)
					  ? 100.0 * static_cast<double>(s.totalCommitted) / static_cast<double>(s.totalReserved)
					  : 0.0;
	return ov;
}
...
std::string FormatOverlay()
{
	...
	oss << std::fixed << std::setprecision(1) << "Mem: " << mb(ov.committed) << "MB / " << mb(ov.reserved) << "MB  ("
		<< ov.usagePct << "%)  regions=" << ov.regionCount;
	return oss.str();
}
...
void DumpMemoryMap()
{
	...
	{
		PS5X_INFO("[MemDiag]  0x%016zx + 0x%08zx  prot=%u  type=%u  [%s]", r.base, r.size,
				  static_cast<uint32_t>(r.prot), static_cast<uint32_t>(r.type), r.tag.c_str());
	}
	auto ov = GetOverlay();
	PS5X_INFO("[MemDiag]  Total committed: %zu bytes  reserved: %zu bytes", ov.committed, ov.reserved);
}
...
void DumpFragmentation()
{
	...
	PS5X_INFO("[MemDiag]  Committed:   %zu bytes (%.1f%%)", rep.totalCommittedBytes, rep.fragmentationPct);
	PS5X_INFO("[MemDiag]  Regions:     %u", rep.regionCount);
	PS5X_INFO("[MemDiag]  Gaps:        %u  (largest: %zu bytes)", rep.gapCount, rep.largestFreeGapBytes);
}
...
std::vector<uintptr_t> SearchPattern(const uint8_t* pattern, size_t patLen)
{
	...
	Memory::ForEachRegion(
		[&](const Memory::Region& r)
		{
			if (!r.committed || !Memory::IsReadable(r.base, r.size))
				return true;
...
DiagReport GetReport()
{
	...
	for (auto& [_, a] : leak_map)
		r.trackedBytes += a.bytes;
	return r;
}
