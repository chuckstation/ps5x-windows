...
bool Load(const std::filesystem::path& path)
{
	...
	PS5X_INFO("[Config] Loaded %zu entries from %s", kv_store.size(), path.string().c_str());
	return true;
}
...
bool Save(const std::filesystem::path& path)
{
	...
	PS5X_INFO("[Config] Saved %zu entries to %s", kv_store.size(), path.string().c_str());
	return true;
}
...
bool ValidateFirmwarePath(const std::filesystem::path& path)
{
	...
	PS5X_INFO("Firmware path accepted: %s (content not verified – user responsibility)", path.string().c_str());
	return true;
}
