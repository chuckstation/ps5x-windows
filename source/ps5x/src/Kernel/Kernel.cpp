...
void Init()
{
	PS5X_INFO("[Kernel] Memory subsystem initialised.");
}

void Shutdown()
{
	PS5X_INFO("[Kernel] Memory subsystem shutdown.");
}
...
void* VirtualAlloc(void* hint, size_t size, MapFlags flags)
{
	...
	if (!ptr)
		PS5X_ERROR("[Kernel] VirtualAlloc failed: size=%zu", size);
	return ptr;
#else
	int prot = PROT_NONE;
	auto f = static_cast<uint32_t>(flags);
	if (f & static_cast<uint32_t>(MapFlags::Read))
		prot |= PROT_READ;
	if (f & static_cast<uint32_t>(MapFlags::Write))
		prot |= PROT_WRITE;
	if (f & static_cast<uint32_t>(MapFlags::Execute))
		prot |= PROT_EXEC;
	void* ptr = mmap(hint, size, prot, MAP_ANON | MAP_PRIVATE, -1, 0);
	if (ptr == MAP_FAILED)
	{
		PS5X_ERROR("[Kernel] mmap failed: size=%zu", size);
		return nullptr;
	}
	return ptr;
#endif
