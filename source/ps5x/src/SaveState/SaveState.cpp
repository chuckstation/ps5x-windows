// PS5x – Save State Manager implementation
...
static bool DeserializeMemoryMap(BlobReader& reader)
{
	...
	for (uint32_t i = 0; i < count; ++i)
	{
		...
		if (!reader.ReadT(base))
			return false;
		if (!reader.ReadT(sz))
			return false;
		if (!reader.ReadT(prot))
			return false;
		if (!reader.ReadT(type))
			return false;
		if (!reader.ReadStr(tag))
			return false;

		// Read the page data
		std::vector<uint8_t> pageData(sz);
		if (sz > 0 && !reader.Read(pageData.data(), sz))
			return false;
		...
		if (!existing.has_value())
		{
			...
			if (newBase == 0)
			{
				PS5X_ERROR("[SaveState] Failed to re-commit region at 0x%016lX size=%zu",
...
static bool DeserializeCpuContext(BlobReader& reader)
{
	...
	for (size_t i = 0; i < ctx.gpr.size(); ++i)
	{
		if (!reader.ReadT(ctx.gpr[i]))
			return false;
	}

	if (!reader.ReadT(ctx.rip))
		return false;
	if (!reader.ReadT(ctx.rflags))
		return false;
	if (!reader.ReadT(ctx.cs))
		return false;
	if (!reader.ReadT(ctx.ss))
		return false;

	for (size_t i = 0; i < ctx.xmm.size(); ++i)
	{
		if (!reader.Read(ctx.xmm[i].data(), 16))
			return false;
	}
...
static bool DeserializeGpuState(BlobReader& reader)
{
	...
	GPU::GpuStats stats{};
	if (!reader.ReadT(stats.submits))
		return false;
	if (!reader.ReadT(stats.flips))
		return false;
	if (!reader.ReadT(stats.barriers))
		return false;
	if (!reader.ReadT(stats.fencesSignaled))
		return false;
	if (!reader.ReadT(stats.activeQueues))
		return false;

	PS5X_DEBUG("[SaveState] GPU state restored: submits=%llu flips=%llu queues=%u",
			   static_cast<unsigned long long>(stats.submits), static_cast<unsigned long long>(stats.flips),
			   stats.activeQueues);
	return true;
}
...
struct SaveStateManager
{
	...
	static SaveStateManager& Get()
	{
		static SaveStateManager s;
		return s;
	}
...
bool Init(const std::filesystem::path& saveDir)
{
	...
	if (ec)
	{
		PS5X_ERROR("[SaveState] Failed to create save directory '%s': %s", saveDir.string().c_str(),
				   ec.message().c_str());
	...
	PS5X_INFO("[SaveState] Initialised. Save directory: %s", saveDir.string().c_str());
	return true;
}
...
SaveResult Save(int slot, const std::string& description)
{
	...
	if (!mgr.initialised)
	{
		PS5X_ERROR("[SaveState] Not initialised.");
		return SaveResult::InvalidState;
	}

	if (slot < 0 || slot > 9)
	{
		PS5X_ERROR("[SaveState] Invalid slot %d (must be 0-9).", slot);
		return SaveResult::InvalidState;
	}
	...
	std::memcpy(header.magic, "PS5xSS\0\0", 8);
	header.version = 1;
	...
	size_t off = 0;
	std::memcpy(payload.data() + off, cpuBlob.data.data(), cpuBlob.data.size());
	off += cpuBlob.data.size();
	std::memcpy(payload.data() + off, memBlob.data.data(), memBlob.data.size());
	off += memBlob.data.size();
	std::memcpy(payload.data() + off, gpuBlob.data.data(), gpuBlob.data.size());
	off += gpuBlob.data.size();
	...
	if (!f.is_open())
	{
		PS5X_ERROR("[SaveState] Failed to open '%s' for writing.", path.string().c_str());
	...
	if (!f.good())
	{
		PS5X_ERROR("[SaveState] Write error on slot %d.", slot);
		return SaveResult::DiskFull;
	}

	f.close();
	PS5X_INFO("[SaveState] Saved slot %d – %zu bytes (cpu=%u mem=%u gpu=%u) crc=0x%08X", slot,
			  sizeof(header) + payloadSize, header.cpuStateSize, header.memoryMapSize, header.gpuStateSize,
			  header.checksum);
...
LoadResult Load(int slot)
{
	...
	{
		PS5X_ERROR("[SaveState] Slot %d file not found: %s", slot, path.string().c_str());
	...
	if (!f.is_open())
	{
		PS5X_ERROR("[SaveState] Failed to open '%s' for reading.", path.string().c_str());
		return LoadResult::CorruptFile;
	}
	...
	if (!f.good())
	{
		PS5X_ERROR("[SaveState] Read error on slot %d.", slot);
		return LoadResult::CorruptFile;
	}
	f.close();
	...
	{
		PS5X_ERROR("[SaveState] File too small for header (%zu bytes).", fileSize);
		return LoadResult::CorruptFile;
	}
	...
	// Validate magic
	if (std::memcmp(header.magic, "PS5xSS\0\0", 8) != 0)
	{
		PS5X_ERROR("[SaveState] Invalid magic in slot %d.", slot);
		return LoadResult::CorruptFile;
	}
	...
	if (header.version != 1)
	{
		PS5X_ERROR("[SaveState] Version mismatch: file=%u expected=1.", header.version);
		return LoadResult::VersionMismatch;
	}

	// ── Validate checksum ───────────────────────────────────────────────
	size_t payloadOffset = sizeof(SaveStateHeader);
	size_t payloadSize = static_cast<size_t>(header.cpuStateSize) + static_cast<size_t>(header.memoryMapSize) +
						 static_cast<size_t>(header.gpuStateSize);

	if (payloadOffset + payloadSize > fileSize)
	{
		PS5X_ERROR("[SaveState] File truncated: expected %zu bytes, got %zu.", payloadOffset + payloadSize, fileSize);
		return LoadResult::CorruptFile;
	}
	...
	{
		PS5X_ERROR("[SaveState] CRC mismatch: computed=0x%08X stored=0x%08X.", computedCrc, header.checksum);
		return LoadResult::ChecksumMismatch;
	}

	// ── Restore CPU ─────────────────────────────────────────────────────
	{
		BlobReader reader{fileData.data() + payloadOffset, header.cpuStateSize, 0};
		if (!DeserializeCpuContext(reader))
		{
			PS5X_ERROR("[SaveState] Failed to deserialize CPU state.");
			return LoadResult::CorruptFile;
		}
	}
	...
	// ── Restore Memory ──────────────────────────────────────────────────
	{
		...
		BlobReader reader{fileData.data() + memOffset, header.memoryMapSize, 0};
		if (!DeserializeMemoryMap(reader))
		{
			PS5X_ERROR("[SaveState] Failed to deserialize memory map.");
	...
	// ── Restore GPU ─────────────────────────────────────────────────────
	{
		...
		BlobReader reader{fileData.data() + gpuOffset, header.gpuStateSize, 0};
		if (!DeserializeGpuState(reader))
		{
			PS5X_ERROR("[SaveState] Failed to deserialize GPU state.");
			return LoadResult::CorruptFile;
		}
	}

	PS5X_INFO("[SaveState] Loaded slot %d – \"%s\" (ts=%llu)", slot, header.description,
...
bool Delete(int slot)
{
	...
	if (ok)
	{
		PS5X_INFO("[SaveState] Deleted slot %d.", slot);
	}
	else
	{
		PS5X_ERROR("[SaveState] Failed to delete slot %d: %s", slot, ec.message().c_str());
	}
	return ok;
}
