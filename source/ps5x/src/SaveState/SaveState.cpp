...
bool Init(const std::filesystem::path& saveDir)
{
    ...
    if (ec) {
        PS5X_ERROR("[SaveState] Failed to create save directory '%s': %s",
...
SaveResult Save(int slot, const std::string& description)
{
    ...
    f.close();
    PS5X_INFO("[SaveState] Saved slot %d – %zu bytes (cpu=%u mem=%u gpu=%u) crc=0x%08X",
...
LoadResult Load(int slot)
{
    ...
    if (payloadOffset + payloadSize > fileSize) {
        PS5X_ERROR("[SaveState] File truncated: expected %zu bytes, got %zu.",
    ...
    if (computedCrc != header.checksum) {
        PS5X_ERROR("[SaveState] CRC mismatch: computed=0x%08X stored=0x%08X.",
    ...
    PS5X_INFO("[SaveState] Loaded slot %d – \"%s\" (ts=%llu)",
