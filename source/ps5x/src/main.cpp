...
int main(int argc, char* argv[])
{
	...
	if (!PS5x::Config::ValidateFirmwarePath(cfg.emulator.firmwarePath))
	{
		PS5X_WARN("Firmware not configured.  "
				  "Set emulator.firmwarePath in %s.",
	...
	if (!cfg.emulator.gameContentPath.empty())
		PS5x::Filesystem::Mount(PS5x::Filesystem::MountPoint::App0, cfg.emulator.gameContentPath);
	if (!cfg.emulator.saveDataPath.empty())
		PS5x::Filesystem::Mount(PS5x::Filesystem::MountPoint::SaveData, cfg.emulator.saveDataPath);
	if (!cfg.emulator.firmwarePath.empty())
		PS5x::Filesystem::Mount(PS5x::Filesystem::MountPoint::System, cfg.emulator.firmwarePath);
	...
	PS5x::Logger::AddSink(
		[](PS5x::Logger::Level lvl, std::string_view tag, std::string_view msg)
		{
	...
	// Handle UI events
	PS5x::UI::RegisterEventCallback(
		[&](PS5x::UI::UIEvent ev, const std::string& payload)
		{
			switch (ev)
			{
			case PS5x::UI::UIEvent::LoadElf:
			{
				PS5x::Loader::ExecutableInfo _info;
				PS5x::Loader::LoadExecutable(payload, _info);
			}
			break;
