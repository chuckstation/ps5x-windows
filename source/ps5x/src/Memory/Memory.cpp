// PS5x – Memory Manager implementation
...
void AddRegion(MemState& st, const Region& r)
{
	...
	if (r.committed)
		st.stats.totalCommitted += r.size;
}

void RemoveRegion(MemState& st, uintptr_t base)
{
	auto it = std::find_if(st.regions.begin(), st.regions.end(), [base](const Region& r) { return r.base == base; });
	if (it == st.regions.end())
		return;
	st.stats.totalReserved -= it->size;
	if (it->committed)
		st.stats.totalCommitted -= it->size;
	st.stats.regionCount--;
	st.regions.erase(it);
}
...
bool Init()
{
	...
	st.heapAllocs.clear();
	st.stats = {};
	PS5X_INFO("[Memory] Manager initialised (page=0x%zx).", PAGE_SIZE);
	return true;
}
...
void Shutdown()
{
	...
	{
		PS5X_WARN("[Memory] Leak: heap alloc at 0x%zx size=%zu", ptr, ha.size);
#ifdef _WIN32
	...
	st.heapAllocs.clear();
	st.stats = {};
	PS5X_INFO("[Memory] Shutdown complete.");
}
...
uintptr_t Reserve(uintptr_t hint, size_t size, std::string_view tag)
{
...
#ifdef _WIN32
	auto* ptr = VirtualAlloc(hint ? reinterpret_cast<LPVOID>(hint) : nullptr, size, MEM_RESERVE, PAGE_NOACCESS);
	if (!ptr)
	{
		// retry without hint
	...
	if (!ptr)
	{
		PS5X_ERROR("[Memory] Reserve failed: size=%zu hint=0x%zx", size, hint);
		return 0;
	}
	...
	void* ptr = mmap(hint ? reinterpret_cast<void*>(hint) : nullptr, size, PROT_NONE, flags, -1, 0);
	...
	PS5X_TRACE("[Memory] Reserved 0x%zx bytes @ 0x%zx [%s]", size, base, std::string(tag).c_str());
	return base;
}
...
#ifdef _WIN32
	auto* ptr = VirtualAlloc(reinterpret_cast<LPVOID>(base), size, MEM_COMMIT, ProtToWin(prot));
	if (!ptr)
	{
		PS5X_ERROR("[Memory] Commit failed: base=0x%zx size=%zu", base, size);
		return false;
	}
#else
	if (mprotect(reinterpret_cast<void*>(base), size, ProtToMmap(prot)) != 0)
	{
		PS5X_ERROR("[Memory] Commit/mprotect failed: base=0x%zx size=%zu", base, size);
		return false;
	}
#endif
...
uintptr_t Map(uintptr_t hint, size_t size, Prot prot, AllocType type, std::string_view tag)
{
	...
	auto* ptr = VirtualAlloc(hint ? reinterpret_cast<LPVOID>(hint) : nullptr, size, MEM_RESERVE | MEM_COMMIT, winProt);
	if (!ptr && hint)
		ptr = VirtualAlloc(nullptr, size, MEM_RESERVE | MEM_COMMIT, winProt);
	if (!ptr)
	{
		PS5X_ERROR("[Memory] Map failed: size=%zu", size);
		return 0;
	}
	auto base = reinterpret_cast<uintptr_t>(ptr);
#else
	int flags = MAP_PRIVATE | MAP_ANON;
	void* ptr = mmap(hint ? reinterpret_cast<void*>(hint) : nullptr, size, ProtToMmap(prot), flags, -1, 0);
	if (ptr == MAP_FAILED)
	{
		PS5X_ERROR("[Memory] Map failed: size=%zu", size);
		return 0;
	...
	PS5X_TRACE("[Memory] Map 0x%zx bytes @ 0x%zx prot=%u [%s]", size, base, static_cast<uint32_t>(prot),
...
bool Protect(uintptr_t base, size_t size, Prot prot)
{
	...
	{
		PS5X_ERROR("[Memory] Protect failed: base=0x%zx", base);
		return false;
	}
#else
	if (mprotect(reinterpret_cast<void*>(base), size, ProtToMmap(prot)) != 0)
...
bool Unmap(uintptr_t base, size_t size)
{
	...
	{
		PS5X_ERROR("[Memory] Unmap failed: base=0x%zx", base);
		return false;
	}
#else
...
void* Alloc(size_t size, size_t align, AllocType type)
{
	...
	if (!ptr)
	{
		PS5X_ERROR("[Memory] Alloc failed: size=%zu align=%zu", size, align);
		return nullptr;
	}
...
void Free(void* ptr)
{
	...
	{
		PS5X_WARN("[Memory] Free unknown ptr 0x%zx", base);
		return;
	}
...
{
	if (HasProt(base, size, Prot::Read))
		return true;
#if !defined(_WIN32)
	int fd[2];
	if (pipe(fd) < 0)
...
{
	if (HasProt(base, size, Prot::Write))
		return true;
#if !defined(_WIN32)
	if (!IsReadable(base, size))
		return false;
...
{
	return HasProt(base, size, Prot::Exec);
}
...
void ReportLeaks()
{
	auto& st = MemState::Get();
	std::lock_guard lock(st.mutex);
	if (st.regions.empty() && st.heapAllocs.empty())
		return;

	PS5X_WARN("[Memory] === Leak report ===");
	for (const auto& r : st.regions)
		PS5X_WARN("[Memory]  Region 0x%zx+0x%zx [%s]", r.base, r.size, r.tag.c_str());
	for (const auto& [ptr, ha] : st.heapAllocs)
		PS5X_WARN("[Memory]  Heap   0x%zx size=%zu", ptr, ha.size);
}
