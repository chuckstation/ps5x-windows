...
struct Port
{
	...
	explicit Port(const PortConfig& c, AudioFillFn fn, size_t ringBytes) : cfg(c), fill(std::move(fn)), ring(ringBytes)
	{
	}
};
...
void SDLCALL AudioCallback(void* /*userdata*/, uint8_t* stream, int len)
{
	...
	for (auto& [h, port] : st.ports)
	{
		...
		if (port->fill)
		{
			size_t frameSz = port->cfg.channels * (port->cfg.format == SampleFormat::Int16 ? 2 : 4);
		...
		else if (port->sineTest)
		{
			...
			{
				float s = std::sin(static_cast<float>(port->sinePhase * 2.0 * std::numbers::pi));
				port->sinePhase += port->sineFreq / static_cast<double>(port->cfg.sampleRate);
				if (port->sinePhase >= 1.0)
					port->sinePhase -= 1.0;
		...
		{
			int32_t v = static_cast<int32_t>(out[i]) + static_cast<int32_t>(static_cast<float>(src[i]) * vol);
...
bool Init(const AudioConfig& cfg)
{
	...
	st.latencyMs = static_cast<double>(have.samples) / static_cast<double>(have.freq) * 1000.0;
	PS5X_INFO("[Audio] SDL device %u: %d Hz %u ch buf=%u lat=%.1f ms", st.device, have.freq, have.channels,
			  have.samples, st.latencyMs);
	...
	st.initialised = true;
	PS5X_INFO("[Audio] Init: %u Hz %u ch buf=%u", cfg.sampleRate, cfg.channels, cfg.bufferSamples);
	return true;
}
...
{
	return AudioState::Get().masterVolume.load();
}
...
struct AudioExtState
{
	...
	static AudioExtState& Get()
	{
		static AudioExtState s;
		return s;
	}
};
...
bool SetChannelVolumes(PortHandle h, const float volumes[8])
{
	...
	auto& st = AudioState::Get();
	std::lock_guard lk(st.mutex);
	if (st.ports.find(h) == st.ports.end())
		return false;
...
bool SetPortSpatialPosition(PortHandle h, const SpatialPos& pos)
{
	...
	if (st.ports.find(h) == st.ports.end())
		return false;
	s.spatialPos[h] = pos;
	PS5X_DEBUG("[Audio] Port %d spatial pos (%.2f, %.2f, %.2f)", h, pos.x, pos.y, pos.z);
	return true;
}
