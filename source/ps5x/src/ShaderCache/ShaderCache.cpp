...
void RunWorker()
{
	...
	while (cs.workerRunning.load())
	{
		CompileJob job;
		{
			...
			cs.jobCv.wait(ul, [&] { return !cs.jobQueue.empty() || !cs.workerRunning.load(); });
			if (!cs.workerRunning.load() && cs.jobQueue.empty())
				break;
			if (cs.jobQueue.empty())
				continue;
		...
		if (!binary.empty())
		{
			...
			if (cs.entries.size() >= cs.maxEntries)
			{
				...
				for (auto it = cs.entries.begin(); it != cs.entries.end(); ++it)
					if (it->second.lastUsedUs < oldest->second.lastUsedUs)
						oldest = it;
...
bool Init(const std::filesystem::path& cacheDir, size_t maxEntries)
{
	...
	cs.compilations.store(0);
	cs.totalCompileMs.store(0.0);
	cs.pendingJobs.store(0);
	...
	PS5X_INFO("[ShaderCache] Init: maxEntries=%zu dir=%s", maxEntries,
			  cacheDir.empty() ? "(none)" : cacheDir.string().c_str());
	return true;
}
...
void Shutdown()
{
	...
	PS5X_INFO("[ShaderCache] Shutdown. Compiled=%llu hits=%llu misses=%llu",
			  static_cast<unsigned long long>(cs.compilations.load()), static_cast<unsigned long long>(cs.hits.load()),
...
std::optional<CacheEntry> Compile(const ShaderKey& key, const std::vector<uint8_t>& spirv, const std::string& debugName)
{
	auto& cs = CacheState::Get();
	...
	else
	{
		...
		binary = spirv;
		PS5X_DEBUG("[ShaderCache] No compiler set; storing raw SPIR-V for '%s'", debugName.c_str());
	}
	...
	PS5X_INFO("[ShaderCache] Compiled '%s' [%s] %.2f ms binary=%zu bytes", debugName.c_str(), StageName(key.stage), ms,
			  binary.size());
...
{
	return CacheState::Get().pendingJobs.load();
}
...
bool LoadFromDisk(const std::filesystem::path& path)
{
	...
	for (uint32_t i = 0; i < count; ++i)
	{
		...
		double ms = 0.0;
		f.read(reinterpret_cast<char*>(&ms), sizeof(double));
		uint64_t ts = 0;
		f.read(reinterpret_cast<char*>(&ts), sizeof(uint64_t));

		if (!f)
			break;
		CacheEntry e;
		e.key = k;
		e.binary = std::move(bin);
		e.debugName = std::move(name);
		e.compilationMs = ms;
		e.compiledAtUs = ts;
		e.lastUsedUs = ts;
		e.valid = true;
...
uint32_t Evict(uint64_t maxAgeUs)
{
	...
	if (n)
		PS5X_INFO("[ShaderCache] Evicted %u stale entries.", n);
	return n;
}
...
{
	auto s = GetStats();
	PS5X_INFO("[ShaderCache] entries=%u hits=%llu misses=%llu evictions=%llu", s.entries,
	...
	PS5X_INFO("[ShaderCache] compilations=%llu totalMs=%.1f avgMs=%.2f pending=%u",
			  static_cast<unsigned long long>(s.compilations), s.totalCompileMs, s.avgCompileMs, s.pendingJobs);
}
