...
void Shutdown()
{
	...
	for (auto& [h, e] : sv.shms)
		if (e.memBase)
			Memory::Unmap(e.memBase, e.desc.size);
...
{
	uintptr_t base = Memory::Map(0, size, Memory::Prot::RW, Memory::AllocType::System, name);
	if (!base)
		return INVALID_SHM;
	...
	PS5X_INFO("[KSvc] CreateShm '%.*s' size=%zu h=%d", static_cast<int>(name.size()), name.data(), size, h);
	return h;
}
...
MqHandle CreateMq(std::string_view name, const MqAttr& attr)
{
	...
	sv.mqs[h] = mq;
	PS5X_INFO("[KSvc] CreateMq '%.*s' max=%u msgSz=%u h=%d", static_cast<int>(name.size()), name.data(),
...
bool SendMsg(MqHandle h, const void* data, size_t size, uint32_t priority, uint64_t timeoutUs)
{
	auto& sv = SvcState::Get();
	MqEntry* mq = nullptr;
	{
	...
	if (timeoutUs == UINT64_MAX)
		mq->cvSend.wait(ul, pred);
	else if (!mq->cvSend.wait_for(ul, std::chrono::microseconds(timeoutUs), pred))
		return false;

	if (size > mq->attr.maxMsgSize)
		size = mq->attr.maxMsgSize;
	MsgEntry msg;
...
bool RecvMsg(MqHandle h, void* buf, size_t bufSize, size_t* bytesOut, uint32_t* priorityOut, uint64_t timeoutUs)
{
	auto& sv = SvcState::Get();
	MqEntry* mq = nullptr;
	{
...
bool WaitCondVar(CvHandle cvh, KernelRuntime::KHandle /*mutex*/, uint64_t timeoutUs)
{
	...
	return cv->cv.wait_for(ul, std::chrono::microseconds(timeoutUs)) == std::cv_status::no_timeout;
}
...
{
	return KernelRuntime::SetThreadPriority(h, std::clamp(psPriority, 256, 767));
}
