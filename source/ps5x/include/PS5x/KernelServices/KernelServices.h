...
bool SendMsg(MqHandle h, const void* data, size_t size, uint32_t priority = 0, uint64_t timeoutUs = UINT64_MAX);
bool RecvMsg(MqHandle h, void* buf, size_t bufSize, size_t* bytesOut = nullptr, uint32_t* priorityOut = nullptr,
...
bool WaitCondVar(CvHandle cv, KernelRuntime::KHandle mutex, uint64_t timeoutUs = UINT64_MAX);
