...
namespace ChuckStation5::Renderer { class IRendererBackend; }
...
// ── Lifecycle ─────────────────────────────────────────────────────────────
bool Init(ChuckStation5::Renderer::IRendererBackend* backend);
void Shutdown();
...
enum class QueueType : uint8_t { Graphics = 0, Compute = 1, Transfer = 2 };

CmdQueueHandle CreateQueue(QueueType type = QueueType::Graphics);
bool           DestroyQueue(CmdQueueHandle h);
bool           SubmitToQueue(CmdQueueHandle h, const CommandBuffer& dcb,
                             const CommandBuffer& ccb);
