// ChuckStation5 – Logger implementation
...
void Dispatch(Level level, std::string_view tag, std::string_view message,
...
void Init(std::string_view logPath, bool console, Level minLevel)
{
    ...
    CHUCKSTATION5_INFO("Logger initialized (level=%u, file=%s)",
              static_cast<unsigned>(minLevel), logPath.empty() ? "none" : std::string(logPath).c_str());
}
...
bool IsEnabled(Level level)
{
    return static_cast<uint8_t>(level) >=
           static_cast<uint8_t>(State::Get().minLevel.load(std::memory_order_relaxed));
}
