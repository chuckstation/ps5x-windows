// ChuckStation5 – Metrics & Telemetry implementation
...
namespace ChuckStation5::Metrics {
...
bool Init()
{
    ...
    if (reg.initialized)
    {
        CHUCKSTATION5_WARN("Metrics registry already initialized – skipping");
        return true;
    }

    reg.initialized = true;
    CHUCKSTATION5_INFO("Metrics registry initialized");
    return true;
}
...
void Shutdown()
{
    ...
    CHUCKSTATION5_INFO("Metrics registry shut down");
}
...
void LogAll()
{
    ...
    CHUCKSTATION5_INFO("=== Metrics Snapshot ===");

    // Counters
    if (!reg.counters.empty())
    {
        CHUCKSTATION5_INFO("  Counters:");
        for (const auto& [name, ctr] : reg.counters)
            CHUCKSTATION5_INFO("    %-40s : %lld", name.c_str(),
    ...
    if (!reg.gauges.empty())
    {
        CHUCKSTATION5_INFO("  Gauges:");
        for (const auto& [name, g] : reg.gauges)
            CHUCKSTATION5_INFO("    %-40s : %.6f", name.c_str(), g.Get());
    }

    // Histograms
    if (!reg.histograms.empty())
    {
        CHUCKSTATION5_INFO("  Histograms:");
        for (const auto& [name, h] : reg.histograms)
        {
            auto snap = h.GetSnapshot();
            CHUCKSTATION5_INFO("    %-40s : count=%llu min=%.2f max=%.2f mean=%.2f "
    ...
    CHUCKSTATION5_INFO("=== End Metrics Snapshot ===");
}
...
void ResetAll()
{
    ...
    CHUCKSTATION5_INFO("All metrics reset");
}
