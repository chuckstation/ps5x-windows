// ChuckStation5 – Loader implementation (Phase 2 – full ELF64 parser)
...
namespace ChuckStation5::Loader {
...
void Init()     { CHUCKSTATION5_INFO("[Loader] Initialised."); }
void Shutdown() { Reset(); CHUCKSTATION5_INFO("[Loader] Shutdown."); }
...
void Reset() {
    ...
    CHUCKSTATION5_INFO("[Loader] Reset.");
}

LoadResult InspectElf(const std::filesystem::path& path, ExecutableInfo& out) {
    std::ifstream f(path, std::ios::binary);
    if (!f.is_open()) {
        CHUCKSTATION5_ERROR("[Loader] Not found: %s", path.string().c_str());
    ...
    CHUCKSTATION5_INFO("[Loader] Inspect OK: %s  entry=0x%llx segs=%zu PIC=%d symbols=%zu relas=%zu",
...
LoadResult MapSegments(ExecutableInfo& info, const std::filesystem::path& path) {
    ...
    for (auto& seg : info.segments) {
        ...
        CHUCKSTATION5_INFO("[Loader]  LOAD host=0x%zx va=0x%llx sz=0x%zx flags=%c%c%c",
...
LoadResult LoadExecutable(const std::filesystem::path& path, ExecutableInfo& out) {
    CHUCKSTATION5_INFO("[Loader] LoadExecutable: %s", path.string().c_str());
    ...
    std::lock_guard lk(st.mtx);
    st.current = out;
    CHUCKSTATION5_INFO("[Loader] Loaded. entry=0x%llx base=0x%llx",
              static_cast<unsigned long long>(out.entryPoint),
...
LoadResult Execute() {
    ...
    CHUCKSTATION5_INFO("[Loader] Launching guest entry 0x%llx on Win32 thread.",
    ...
    // However, under the ChuckStation5 interpreter model, execution is driven by
    // Cpu::SetRip(entry) + Cpu::Run() from the main loop.
    Cpu::SetRip(entry);
    CHUCKSTATION5_INFO("[Loader] RIP set to guest entry 0x%llx. Call Cpu::Run() to execute.",
              static_cast<unsigned long long>(entry));
#else
    Cpu::SetRip(entry);
#endif
...
LoadResult ValidateFirmware(const std::filesystem::path& p) {
    if (p.empty() || !std::filesystem::exists(p)) {
        CHUCKSTATION5_ERROR("[Loader] Firmware absent: '%s'. ChuckStation5 does NOT supply firmware.",
                   p.string().c_str());
...
#pragma pack(pop)
    ...
    CHUCKSTATION5_INFO("[Loader] SFO title=%s ver=%s",
