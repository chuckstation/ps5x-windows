// ChuckStation5 – Execution Framework implementation
...
void SetState(ExecState next)
{
    ...
    CHUCKSTATION5_INFO("[Exec] State %s → %s",
...
void SetError(std::string msg)
{
    ...
    CHUCKSTATION5_ERROR("[Exec] %s", ctx.lastError.c_str());
}
...
bool Init()
{
    ...
    CHUCKSTATION5_INFO("[Exec] Execution framework initialised.");
    return true;
}
...
void Shutdown()
{
    ...
    CHUCKSTATION5_INFO("[Exec] Shutdown.");
}
...
bool LoadProgram(const std::filesystem::path& elfPath, const LoadOptions& opts)
{
    ...
    if (!opts.firmwarePath.empty()) {
        ...
        if (r != Loader::LoadResult::Ok) {
            CHUCKSTATION5_WARN("[Exec] Firmware validation: %s (continuing)",
                      Loader::LoadResultStr(r));
        }
    } else {
        CHUCKSTATION5_INFO("[Exec] No firmware path set. "
                  "ChuckStation5 does not supply firmware – provide your own.");
    }
    ...
    CHUCKSTATION5_INFO("[Exec] Program loaded in %.2f ms: %s  PID=%u",
...
bool Start()
{
    ...
    CHUCKSTATION5_INFO("[Exec] Execution started. PID=%u", ctx.pid);
    return true;
}
...
void ForceTerminate()
{
    ...
    SetState(ExecState::Terminated);
    CHUCKSTATION5_INFO("[Exec] Force terminated.");
}
...
StepResult StepInstruction()
{
    ...
    if (ctx.state.load() != ExecState::Paused) {
        CHUCKSTATION5_WARN("[Exec] StepInstruction called outside Paused state.");
        return StepResult::Fault;
    }
    // Delegate to debugger for single-step
    Debugger::StepInto();
    CHUCKSTATION5_TRACE("[Exec] StepInstruction → Ok");
    return StepResult::Ok;
}

void DispatchException(uint8_t vector, uint64_t errorCode)
{
    CHUCKSTATION5_WARN("[Exec] Guest exception vector=%u ec=0x%llx", vector,
...
{
    CHUCKSTATION5_DEBUG("[Exec] InjectTrap INT%u", trapNumber);
...
void ReportFault(uint64_t faultAddr, const std::string& description)
{
    ...
    CHUCKSTATION5_ERROR("[Exec] Fault @ 0x%llx: %s",
...
void ReportGuestPanic(const std::string& message)
{
    ...
    CHUCKSTATION5_ERROR("[Exec] Guest panic: %s", message.c_str());
}
