// ChuckStation5 – Kernel Services implementation
...
bool Init()
{
    ...
    CHUCKSTATION5_INFO("[KSvc] Kernel services initialised.");
    return true;
}
...
void Shutdown()
{
    ...
    for (auto& [h,c] : sv.cvs) delete c;
    sv.shms.clear(); sv.mqs.clear(); sv.rws.clear(); sv.cvs.clear();
    CHUCKSTATION5_INFO("[KSvc] Shutdown.");
}
...
ShmHandle CreateShm(std::string_view name, size_t size)
{
    ...
    CHUCKSTATION5_INFO("[KSvc] CreateShm '%.*s' size=%zu h=%d",
...
MqHandle CreateMq(std::string_view name, const MqAttr& attr)
{
    ...
    sv.mqs[h]  = mq;
    CHUCKSTATION5_INFO("[KSvc] CreateMq '%.*s' max=%u msgSz=%u h=%d",
