// ChuckStation5 – Memory Diagnostics implementation
...
bool Init()
{
    ...
    CHUCKSTATION5_INFO("[MemDiag] Initialised.");
    return true;
}
...
void Shutdown()
{
    ...
    d.snapshots.clear();
    d.history.clear();
    CHUCKSTATION5_INFO("[MemDiag] Shutdown.");
}
...
void TakeSnapshot(std::string_view label)
{
    ...
    CHUCKSTATION5_DEBUG("[MemDiag] Snapshot '%.*s': regions=%u committed=%zu",
...
void EnableHistory(bool enable)
{
    ...
    CHUCKSTATION5_INFO("[MemDiag] Allocation history %s.", enable ? "enabled" : "disabled");
}
...
{
    auto regions = Memory::GetRegions();
    CHUCKSTATION5_INFO("[MemDiag] === Memory Map (%zu regions) ===", regions.size());
    for (const auto& r : regions) {
        CHUCKSTATION5_INFO("[MemDiag]  0x%016zx + 0x%08zx  prot=%u  type=%u  [%s]",
                  r.base, r.size,
    ...
    auto ov = GetOverlay();
    CHUCKSTATION5_INFO("[MemDiag]  Total committed: %zu bytes  reserved: %zu bytes",
...
void DumpFragmentation()
{
    ...
    CHUCKSTATION5_INFO("[MemDiag] === Fragmentation Report ===");
    CHUCKSTATION5_INFO("[MemDiag]  Reserved:    %zu bytes", rep.totalReservedBytes);
    CHUCKSTATION5_INFO("[MemDiag]  Committed:   %zu bytes (%.1f%%)", rep.totalCommittedBytes,
              rep.fragmentationPct);
    CHUCKSTATION5_INFO("[MemDiag]  Regions:     %u", rep.regionCount);
    CHUCKSTATION5_INFO("[MemDiag]  Gaps:        %u  (largest: %zu bytes)",
...
std::vector<uintptr_t> SearchPattern(const uint8_t* pattern, size_t patLen)
{
    ...
    CHUCKSTATION5_INFO("[MemDiag] Pattern search: %zu match(es)", results.size());
    return results;
}
