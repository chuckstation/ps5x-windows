// ChuckStation5 – Audio implementation (Phase 2 – ring-buffer + sine validation)
...
bool Init(const AudioConfig& cfg)
{
    ...
    if (SDL_WasInit(SDL_INIT_AUDIO) == 0) {
        if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
            CHUCKSTATION5_ERROR("[Audio] SDL_InitSubSystem(AUDIO): %s", SDL_GetError());
            return false;
        }
    }
    ...
    if (st.device == 0) {
        CHUCKSTATION5_ERROR("[Audio] SDL_OpenAudioDevice: %s", SDL_GetError());
        return false;
    }

    st.latencyMs = static_cast<double>(have.samples) /
                   static_cast<double>(have.freq) * 1000.0;
    CHUCKSTATION5_INFO("[Audio] SDL device %u: %d Hz %u ch buf=%u lat=%.1f ms",
              st.device, have.freq, have.channels, have.samples, st.latencyMs);

    SDL_PauseAudioDevice(st.device, 0); // start
#else
    CHUCKSTATION5_WARN("[Audio] SDL2 not available – audio is silent.");
#endif

    st.initialised = true;
    CHUCKSTATION5_INFO("[Audio] Init: %u Hz %u ch buf=%u",
...
void Shutdown()
{
    ...
    CHUCKSTATION5_INFO("[Audio] Shutdown.");
}
...
PortHandle OpenPort(const PortConfig& cfg, AudioFillFn fillCallback)
{
    ...
    std::lock_guard lk(st.mutex);
    if (!st.initialised) {
        CHUCKSTATION5_ERROR("[Audio] OpenPort before Init.");
        return INVALID_PORT;
    }
    ...
    CHUCKSTATION5_INFO("[Audio] Port %d: %u Hz %u ch buf=%u", h, cfg.sampleRate, cfg.channels, cfg.bufferSamples);
    return h;
}
...
bool Start(PortHandle h)
{
    ...
    CHUCKSTATION5_INFO("[Audio] Port %d started.", h);
    return true;
}
...
bool Stop(PortHandle h)
{
    ...
    CHUCKSTATION5_INFO("[Audio] Port %d stopped.", h);
    return true;
}
...
bool SetPortSpatialPosition(PortHandle h, const SpatialPos& pos)
{
    ...
    if (st.ports.find(h) == st.ports.end()) return false;
    s.spatialPos[h] = pos;
    CHUCKSTATION5_DEBUG("[Audio] Port %d spatial pos (%.2f, %.2f, %.2f)",
               h, pos.x, pos.y, pos.z);
    return true;
}
...
bool SelectDevice(const std::string& name)
{
    ...
    CHUCKSTATION5_INFO("[Audio] SelectDevice '%s'", name.c_str());
