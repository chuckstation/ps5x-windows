// ChuckStation5 – GPU Command Processor
...
namespace ChuckStation5::CommandProcessor {
...
struct CpState
{
    ...
    static CpState& Get() { static CpState s; return s; }
};
...
{
    switch (op) {
        case CommandOpcode::Nop:              return "Nop";
        case CommandOpcode::SetRenderTarget:  return "SetRenderTarget";
        case CommandOpcode::SetDepthTarget:   return "SetDepthTarget";
        case CommandOpcode::ClearColor:       return "ClearColor";
        case CommandOpcode::ClearDepth:       return "ClearDepth";
        case CommandOpcode::DrawIndexed:      return "DrawIndexed";
        case CommandOpcode::DrawDirect:       return "DrawDirect";
        case CommandOpcode::Dispatch:         return "Dispatch";
        case CommandOpcode::SetViewport:      return "SetViewport";
        case CommandOpcode::SetScissor:       return "SetScissor";
        case CommandOpcode::BarrierTransition:return "Barrier";
        case CommandOpcode::WaitFence:        return "WaitFence";
        case CommandOpcode::SignalFence:      return "SignalFence";
        case CommandOpcode::BeginRenderPass:  return "BeginRenderPass";
        case CommandOpcode::EndRenderPass:    return "EndRenderPass";
        case CommandOpcode::PushDebugLabel:   return "PushDebugLabel";
        case CommandOpcode::PopDebugLabel:    return "PopDebugLabel";
        case CommandOpcode::End:              return "End";
    }
    return "Unknown";
}
...
void CommandList::DrawDirect(uint32_t vertexCount, uint32_t instanceCount,
...
void CommandList::Dispatch(uint32_t gx, uint32_t gy, uint32_t gz)
{
    ...
    WriteU32(gx); WriteU32(gy); WriteU32(gz);
}

void CommandList::SetViewport(const Viewport& vp)
{
    WriteOpcode(CommandOpcode::SetViewport);
    WriteF32(vp.x); WriteF32(vp.y);
    WriteF32(vp.width); WriteF32(vp.height);
    WriteF32(vp.minDepth); WriteF32(vp.maxDepth);
}

void CommandList::SetScissor(const Scissor& sc)
{
    WriteOpcode(CommandOpcode::SetScissor);
    WriteI32(sc.x); WriteI32(sc.y);
    WriteU32(sc.width); WriteU32(sc.height);
}
...
void CommandList::BeginRenderPass(const std::string& label)
{
    ...
    WriteU32(len);
    if (len) WriteBytes(label.data(), len);
}
...
bool Init(Renderer::IRendererBackend* backend)
{
    ...
    st.initialised = true;
    CHUCKSTATION5_INFO("[CmdProc] Command processor initialised. Backend: %s",
...
void Shutdown()
{
    ...
    CHUCKSTATION5_INFO("[CmdProc] Shutdown. Commands processed: %llu",
...
int32_t Process(const uint8_t* data, size_t size)
{
    ...
    while (ptr + 4 <= end) {
        ...
        CHUCKSTATION5_TRACE("[CmdProc] %s", OpcodeName(op));
        ...
        case CommandOpcode::ClearColor: {
            ...
            if (st.backend) GPU::ClearRenderTarget(slot, r, g, b, a);
            break;
        }
        ...
        case CommandOpcode::ClearDepth: {
            ...
            if (st.backend) GPU::ClearDepthTarget(depth, static_cast<uint8_t>(stencil));
            break;
        }
        ...
        case CommandOpcode::DrawDirect: {
            ...
            CHUCKSTATION5_TRACE("[CmdProc] DrawDirect");
            break;
        }
        ...
        case CommandOpcode::BarrierTransition: {
            ...
            GPU::Barrier b{gpuAddr,
                           static_cast<GPU::ResourceState>(before),
                           static_cast<GPU::ResourceState>(after)};
            GPU::InsertBarrier(b);
            {
                std::lock_guard lk2(st.mtx);
                ++st.stats.barriers;
        ...
        case CommandOpcode::BeginRenderPass: {
            ...
            CHUCKSTATION5_DEBUG("[CmdProc] BeginRenderPass '%s'", label.c_str());
            break;
        }
        ...
        default: {
            ...
            CHUCKSTATION5_WARN("[CmdProc] Unknown opcode 0x%08x — skipping", raw_op);
    ...
    RuntimeEvents::PublishGpuEvent(RuntimeEvents::EventType::FrameEnd,
                                    st.stats.commandListsProcessed, ms, 0.0);
