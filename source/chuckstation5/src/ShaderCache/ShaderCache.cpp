// ChuckStation5 – Shader Cache implementation
...
namespace ChuckStation5::ShaderCache {
...
void RunWorker()
{
    ...
    while (cs.workerRunning.load()) {
        ...
        if (!binary.empty()) {
            ...
            CHUCKSTATION5_DEBUG("[ShaderCache] Compiled '%s' in %.2f ms", job.debugName.c_str(), ms);
        } else {
            CHUCKSTATION5_WARN("[ShaderCache] Compilation failed for '%s'", job.debugName.c_str());
        }
...
bool Init(const std::filesystem::path& cacheDir, size_t maxEntries)
{
    ...
    CHUCKSTATION5_INFO("[ShaderCache] Init: maxEntries=%zu dir=%s",
...
void Shutdown()
{
    ...
    std::lock_guard lk(cs.mtx);
    cs.entries.clear();
    CHUCKSTATION5_INFO("[ShaderCache] Shutdown. Compiled=%llu hits=%llu misses=%llu",
...
void SetCompiler(CompileFn fn)
{
    ...
    CHUCKSTATION5_INFO("[ShaderCache] Compiler registered.");
}
                                   ...
                                   const std::string& debugName)
{
    ...
    if (cs.compileFn) {
        ...
        binary = spirv;
        CHUCKSTATION5_DEBUG("[ShaderCache] No compiler set; storing raw SPIR-V for '%s'",
                   debugName.c_str());
    }
    ...
    CHUCKSTATION5_INFO("[ShaderCache] Compiled '%s' [%s] %.2f ms binary=%zu bytes",
...
uint32_t InvalidateAll()
{
    ...
    cs.entries.clear();
    CHUCKSTATION5_INFO("[ShaderCache] InvalidateAll: removed %u entries", n);
    return n;
}
...
bool SaveToDisk(const std::filesystem::path& path)
{
    ...
    if (!f.is_open()) {
        CHUCKSTATION5_ERROR("[ShaderCache] SaveToDisk failed: %s", path.string().c_str());
        return false;
    }
    ...
    CHUCKSTATION5_INFO("[ShaderCache] Saved %u entries to %s", count, path.string().c_str());
    return true;
}
...
bool LoadFromDisk(const std::filesystem::path& path)
{
    ...
    CHUCKSTATION5_INFO("[ShaderCache] Loaded %u/%u entries from disk.", loaded, count);
    return loaded > 0;
}
...
uint32_t Evict(uint64_t maxAgeUs)
{
    ...
    if (n) CHUCKSTATION5_INFO("[ShaderCache] Evicted %u stale entries.", n);
    return n;
}
...
{
    auto s = GetStats();
    CHUCKSTATION5_INFO("[ShaderCache] entries=%u hits=%llu misses=%llu evictions=%llu",
              s.entries,
              static_cast<unsigned long long>(s.hits),
              static_cast<unsigned long long>(s.misses),
              static_cast<unsigned long long>(s.evictions));
    CHUCKSTATION5_INFO("[ShaderCache] compilations=%llu totalMs=%.1f avgMs=%.2f pending=%u",
