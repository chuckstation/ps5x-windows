// ChuckStation5 – Runtime Manager implementation
...
{
    switch (s) {
        case SubsystemState::Unregistered: return "Unregistered";
        case SubsystemState::Registered:   return "Registered";
        case SubsystemState::Initialising: return "Initialising";
        case SubsystemState::Running:      return "Running";
        case SubsystemState::ShuttingDown: return "ShuttingDown";
...
struct RuntimeState {
    ...
    static RuntimeState& Get() { static RuntimeState s; return s; }
};
...
std::vector<SubsystemId> TopoSort(const RuntimeState& st)
{
    ...
    for (auto& [k, e] : st.entries) {
        if (!inDegree.count(k)) inDegree[k] = 0;
...
bool InitAll()
{
    ...
    for (auto id : order) {
        ...
        bool isOptional = false;
        {
            ...
            if (it == st.entries.end()) continue;
            isOptional = it->second.desc.optional;
            for (auto dep : it->second.desc.deps) {
                uint32_t dk = static_cast<uint32_t>(dep);
                auto dit = st.entries.find(dk);
                if (dit == st.entries.end() ||
                    dit->second.timing.state != SubsystemState::Running) {
                    depsOk = false;
                    break;
                }
            }
        ...
        if (!depsOk && !isOptional) { ok = false; continue; }
        if (!depsOk) continue;
...
void ShutdownAll()
{
    ...
    for (auto id : order) {
        ...
        if (it == st.entries.end()) continue;
        auto& e = it->second;
        if (e.timing.state != SubsystemState::Running) continue;

        e.timing.state = SubsystemState::ShuttingDown;
        auto t0 = Clock::now();
        if (e.desc.shutdown) e.desc.shutdown();
        e.timing.shutdownMs = ElapsedMs(t0);
        e.timing.state = SubsystemState::Stopped;
        CHUCKSTATION5_INFO("[Runtime]  ↓ %-14s  %.2f ms", SubsystemName(id), e.timing.shutdownMs);
    }
...
bool InitOne(SubsystemId id)
{
    ...
    uint32_t k = static_cast<uint32_t>(id);
    auto it = st.entries.find(k);
    if (it == st.entries.end()) return false;
    auto& e = it->second;
    if (e.timing.state == SubsystemState::Running) return true;
    ...
    if (ok) st.initOrder.push_back(id);
    return ok;
}
...
void ShutdownOne(SubsystemId id)
{
    ...
    uint32_t k = static_cast<uint32_t>(id);
    auto it = st.entries.find(k);
    if (it == st.entries.end()) return;
    auto& e = it->second;
    if (e.timing.state != SubsystemState::Running) return;
    e.timing.state = SubsystemState::ShuttingDown;
    auto t0 = Clock::now();
    if (e.desc.shutdown) e.desc.shutdown();
    e.timing.shutdownMs = ElapsedMs(t0);
...
SubsystemState GetState(SubsystemId id)
{
    ...
    uint32_t k = static_cast<uint32_t>(id);
    auto it = st.entries.find(k);
    if (it == st.entries.end()) return SubsystemState::Unregistered;
...
std::optional<SubsystemTiming> GetTiming(SubsystemId id)
{
    ...
    uint32_t k = static_cast<uint32_t>(id);
    auto it = st.entries.find(k);
    if (it == st.entries.end()) return std::nullopt;
...
void ReportTimings()
{
    ...
    CHUCKSTATION5_INFO("[Runtime] ── Subsystem Timing Report ──────────────────");
    double total = 0.0;
    for (const auto& t : timings) {
        if (t.state == SubsystemState::Unregistered) continue;
        CHUCKSTATION5_INFO("[Runtime]  %-14s  %s  init=%.2f ms  shutdown=%.2f ms%s",
                  SubsystemName(t.id),
                  SubsystemStateName(t.state),
                  t.initMs, t.shutdownMs,
                  t.failReason.empty() ? "" : (" [" + t.failReason + "]").c_str());
        total += t.initMs;
    }
    CHUCKSTATION5_INFO("[Runtime]  Total init time: %.2f ms", total);
    CHUCKSTATION5_INFO("[Runtime] ────────────────────────────────────────────");
}
...
void RegisterDefaults()
{
    ...
    Register({
        Id::Logger, "Logger",
        []{ ChuckStation5::Logger::Init("chuckstation5.log", true, ChuckStation5::Logger::Level::Info); return true; },
    ...
    Register({
        Id::Config, "Config",
        []{ ChuckStation5::Config::Reset(); return true; },
        []{ },
        {Id::Logger}, false
    });
    ...
    Register({
        Id::Memory, "Memory",
        []{ return ChuckStation5::Memory::Init(); },
    ...
    Register({
        Id::Kernel, "Kernel",
        []{ ChuckStation5::Kernel::Init(); return true; },
    ...
    Register({
        ...
        []{ ChuckStation5::Filesystem::Init(); return true; },
        []{ ChuckStation5::Filesystem::Shutdown(); },
        {Id::Logger, Id::Config}, false
    });

    // Loader
    Register({
        Id::Loader, "Loader",
        []{ ChuckStation5::Loader::Init(); return true; },
        []{ ChuckStation5::Loader::Shutdown(); },
        {Id::Memory, Id::Filesystem}, false
    });

    // KytyAdapter
    Register({
        Id::KytyAdapter, "KytyAdapter",
        []{ return ChuckStation5::KytyAdapter::Init(); },
    ...
    Register({
        ...
        []{
            ...
            ChuckStation5::Audio::AudioConfig ac;
            ac.sampleRate    = cfg.audio.sampleRate;
            ac.bufferSamples = cfg.audio.bufferSize;
            ac.masterVolume  = cfg.audio.masterVolume;
            return ChuckStation5::Audio::Init(ac); // may return false in CI – optional
        },
        []{ ChuckStation5::Audio::Shutdown(); },
        {Id::Config}, true   // optional
    });

    // Input
    Register({
        Id::Input, "Input",
        []{ ChuckStation5::Input::Init(); return true; },
    ...
    // Debugger
    Register({
        Id::Debugger, "Debugger",
        []{
            if (ChuckStation5::Config::Get().debug.enableDebugger)
