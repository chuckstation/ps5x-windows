// ChuckStation5 – Filesystem implementation (Phase 2 – expanded VFS)
...
namespace ChuckStation5::Filesystem {
...
void Init()
{
    ...
    st.files.clear();
    st.nextFd = 1;
    CHUCKSTATION5_INFO("[FS] Virtual filesystem initialised.");
}
...
void Shutdown()
{
    ...
    st.files.clear();
    for (auto& m : st.mounts) m = MountEntry{};
    CHUCKSTATION5_INFO("[FS] Shutdown.");
}
...
bool Mount(MountPoint mp, const std::filesystem::path& hostPath, bool readOnly)
{
    ...
    if (idx >= st.mounts.size()) {
        CHUCKSTATION5_ERROR("[FS] Mount: invalid MountPoint %u", static_cast<unsigned>(mp));
        return false;
    }
    if (!std::filesystem::exists(hostPath)) {
        ...
        if (ec) {
            CHUCKSTATION5_ERROR("[FS] Mount: host path absent and can't create: %s",
    ...
    CHUCKSTATION5_INFO("[FS] Mount %s → %s%s",
...
bool Unmount(MountPoint mp)
{
    ...
    CHUCKSTATION5_INFO("[FS] Unmounted %s", kMountPrefix[idx]);
    return true;
}
...
std::optional<std::filesystem::path> Resolve(std::string_view guestPath)
{
    ...
    if (!r) CHUCKSTATION5_WARN("[FS] Unresolvable: %.*s",
...
FileHandle Open(std::string_view guestPath, OpenFlags flags)
{
    ...
    if (rdOnly && (flags & OpenFlags::Write)) {
        CHUCKSTATION5_ERROR("[FS] Open: write denied on read-only mount: %.*s",
                   static_cast<int>(guestPath.size()), guestPath.data());
        return INVALID_FD;
    }
    ...
    if (!fp) {
        CHUCKSTATION5_ERROR("[FS] Open failed: %s (%s)", hostOpt->string().c_str(),
                   std::strerror(errno));
        return INVALID_FD;
    }
...
void DumpMounts()
{
    auto& st = VfsState::Get();
    std::lock_guard lk(st.mtx);
    CHUCKSTATION5_INFO("[FS] === Mount table ===");
    for (size_t i = 0; i < st.mounts.size(); ++i) {
        const auto& m = st.mounts[i];
        if (m.valid)
            CHUCKSTATION5_INFO("[FS]  %-12s → %s%s",
                      kMountPrefix[i], m.hostPath.string().c_str(),
                      m.readOnly ? " [ro]" : "");
        else
            CHUCKSTATION5_INFO("[FS]  %-12s (unmounted)", kMountPrefix[i]);
    }
}
...
void EnableTracing(bool enable)
{
    ...
    CHUCKSTATION5_INFO("[VFS] Tracing %s", enable ? "enabled" : "disabled");
}
...
std::string CreateTempFile(std::string_view name)
{
    ...
    auto& st = VfsState::Get();
    {
        ...
        if (!m.valid) {
            ...
            auto tmpDir = std::filesystem::temp_directory_path() / "chuckstation5_vfs_temp";
    ...
    CHUCKSTATION5_DEBUG("[VFS] CreateTempFile → %s", guestPath.c_str());
    return guestPath;
}
