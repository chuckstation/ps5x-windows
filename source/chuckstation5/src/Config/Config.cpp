// ChuckStation5 – Config implementation
...
namespace ChuckStation5::Config {

namespace {

struct ConfigState
{
    ChuckStation5Config    cfg;
    std::mutex    mtx;
...
const ChuckStation5Config& Get()
{
    return ConfigState::Get().cfg;
}

ChuckStation5Config& GetMutable()
{
    return ConfigState::Get().cfg;
}
...
bool Load(const std::filesystem::path& path)
{
    ...
    {
        CHUCKSTATION5_WARN("Config file not found: %s – using defaults.", path.string().c_str());
        return false;
    }
    ...
    if (!f.is_open())
    {
        CHUCKSTATION5_ERROR("Cannot open config: %s", path.string().c_str());
        return false;
    }
    ...
    CHUCKSTATION5_INFO("[Config] Loaded %zu entries from %s",
...
bool Save(const std::filesystem::path& path)
{
    ...
    if (!f.is_open())
    {
        CHUCKSTATION5_ERROR("Cannot write config: %s", path.string().c_str());
        return false;
    }
    ...
    std::lock_guard lk(kv_mtx);
    f << "# ChuckStation5 v1.0.0 configuration (auto-generated)\n";
    f << "# Repository: github.com/libaerto/chuckstation5-windows\n";
    ...
    CHUCKSTATION5_INFO("[Config] Saved %zu entries to %s",
              kv_store.size(), path.string().c_str());
    return true;
}
...
{
    // ChuckStation5 NEVER bundles, downloads, or provides firmware.
    // The user must supply their own legally-obtained firmware.
    if (path.empty())
    {
        CHUCKSTATION5_ERROR("Firmware path is not set.  "
                   "You must provide your own PS5 firmware.  "
                   "ChuckStation5 does not supply, bundle, or download firmware.");
        return false;
    }

    if (!std::filesystem::exists(path))
    {
        CHUCKSTATION5_ERROR("Firmware path does not exist: %s", path.string().c_str());
        return false;
    }

    CHUCKSTATION5_INFO("Firmware path accepted: %s (content not verified – user responsibility)",
              path.string().c_str());
    return true;
}
