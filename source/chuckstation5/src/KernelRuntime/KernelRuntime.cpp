// ChuckStation5 – Kernel Runtime implementation
...
namespace ChuckStation5::KernelRuntime
{
...
const char* KObjectTypeName(KObjectType t)
{
	...
	case KObjectType::Event:
		return "Event";
	case KObjectType::Timer:
		return "Timer";
	case KObjectType::File:
		return "File";
	case KObjectType::Socket:
		return "Socket";
	case KObjectType::Module:
		return "Module";
	}
	return "?";
}
...
bool Init()
{
	...
	CHUCKSTATION5_INFO("[KernelRuntime] Initialised.");
	return true;
}
...
void Reset()
{
	...
	for (auto& [h, e] : st.handles)
	{
		if (e.type == KObjectType::Timer)
		{
			...
			if (kt->thread.joinable())
				kt->thread.detach();
		}
	}
	st.handles.clear();
	st.tlsKeys.clear();
	CHUCKSTATION5_INFO("[KernelRuntime] Reset.");
}

void Shutdown()
{
	Reset();
	CHUCKSTATION5_INFO("[KernelRuntime] Shutdown.");
}
...
KHandle CreateThread(ThreadEntryFn fn, void* arg, const ThreadAttr& attr)
{
	...
	CHUCKSTATION5_INFO("[KernelRuntime] CreateThread '%s' h=%d stack=0x%llx+%zu", attr.name.c_str(), h,
...
bool StartThread(KHandle h)
{
	...
	if (!kt)
		return false;
	if (kt->state.load() != ThreadState::Created)
		return false;
	...
	kt->osThread = std::thread(
		[kt, h]()
		{
			...
			CHUCKSTATION5_DEBUG("[KernelRuntime] Thread '%s' exited code=%d", kt->info.name.c_str(), code);
		});

	CHUCKSTATION5_INFO("[KernelRuntime] Started thread '%s' h=%d", kt->info.name.c_str(), h);
	return true;
}
...
bool JoinThread(KHandle h, int* exitCode, uint64_t timeoutUs)
{
	...
	if (kt->osThread.joinable())
	{
		...
		else
		{
			...
			while (kt->state.load() != ThreadState::Dead && std::chrono::steady_clock::now() < deadline)
			{
...
bool SetThreadAffinity(KHandle h, uint64_t mask)
{
	...
	if (kt->osThread.native_handle())
	{
		...
		{
			CHUCKSTATION5_WARN("[KernelRuntime] SetThreadAffinityMask failed (err=%lu).", ::GetLastError());
			return false;
		}
		CHUCKSTATION5_DEBUG("[KernelRuntime] Thread h=%d affinity set to 0x%llx.", h, static_cast<unsigned long long>(mask));
	}
#endif
	return true;
}
...
{
	return KernelState::currentHandle;
}
...
KHandle CreateMutex(const MutexAttr& attr, std::string_view name)
{
	...
	CHUCKSTATION5_TRACE("[KernelRuntime] CreateMutex '%.*s' h=%d", static_cast<int>(name.size()), name.data(), h);
	return h;
}

KHandle Lock(KHandle h)
{
	LockMutex(h);
	return h;
}
...
KHandle CreateSemaphore(const SemaphoreAttr& attr, std::string_view name)
{
	...
	CHUCKSTATION5_TRACE("[KernelRuntime] CreateSemaphore '%.*s' init=%d h=%d", static_cast<int>(name.size()), name.data(),
			   attr.initialValue, h);
	return h;
}
...
KHandle CreateEvent(const EventAttr& attr, std::string_view name)
{
	...
	CHUCKSTATION5_TRACE("[KernelRuntime] CreateEvent '%.*s' h=%d", static_cast<int>(name.size()), name.data(), h);
	return h;
}
...
bool WaitEvent(KHandle h, uint64_t timeoutUs)
{
	...
	if (timeoutUs == UINT64_MAX)
	{
		...
		ok = ke->cv.wait_for(lk, std::chrono::microseconds(timeoutUs), pred);
	if (ok && ke->attr.autoReset)
		ke->set = false;
	return ok;
}
...
struct TlsStore
{
	...
	struct KeyHash
	{
		...
		{
			return std::hash<ThreadId>{}(k.first) ^ (std::hash<uint32_t>{}(k.second) << 16);
		}
	};
...
KernelStats GetStats()
{
	...
	for (auto& [h, e] : st.handles)
	{
		if (e.type != KObjectType::Thread)
			continue;
		auto kt = std::static_pointer_cast<KThread>(e.obj);
		if (kt->state.load() == ThreadState::Running)
			++s.runningThreads;
	}
	return s;
}
...
bool RegisterName(std::string_view name, KHandle handle, NamespaceId ns)
{
	...
	CHUCKSTATION5_DEBUG("[KR] RegisterName '%s' ns=%u h=%d", std::string(name).c_str(), ns.value, handle);
	return true;
}
...
KHandle DuplicateHandle(KHandle src)
{
	...
	{
		CHUCKSTATION5_WARN("[KR] DuplicateHandle: unknown handle %d", src);
		return INVALID_HANDLE;
	}
	KHandle newH = st.nextHandle.fetch_add(1, std::memory_order_relaxed);
	st.handles[newH] = it->second; // shared ownership
	CHUCKSTATION5_DEBUG("[KR] DuplicateHandle %d → %d", src, newH);
	return newH;
}
...
void SetResourceLimits(const ResourceLimits& limits)
{
	...
	st.limits = limits;
	CHUCKSTATION5_INFO("[KR] ResourceLimits: maxMem=%zu maxThreads=%u maxHandles=%u", limits.maxMemoryBytes, limits.maxThreads,
			  limits.maxHandles);
}
...
IpcPortHandle CreateIpcPort(std::string_view name)
{
	...
	CHUCKSTATION5_INFO("[IPC] CreatePort '%s' h=%d", entry->name.c_str(), h);
	return h;
}
...
IpcPortHandle ConnectIpcPort(std::string_view name)
{
	...
	{
		CHUCKSTATION5_WARN("[IPC] ConnectPort '%s' not found", std::string(name).c_str());
		return INVALID_IPC_PORT;
	}
	IpcPortHandle h = st.nextIpc++;
	st.ipcByHandle[h] = it->second; // shared queue
	CHUCKSTATION5_DEBUG("[IPC] ConnectPort '%s' h=%d", std::string(name).c_str(), h);
	return h;
}
