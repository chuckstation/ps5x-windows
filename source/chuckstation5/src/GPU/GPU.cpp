// ChuckStation5 – GPU implementation (Windows, Phase 8)
...
struct GpuHeap
{
	...
	{
#if defined(_WIN32)
		base = ::VirtualAlloc(nullptr, kSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		if (!base)
		{
			CHUCKSTATION5_ERROR("[GPU] GpuHeap: VirtualAlloc failed (err=%lu).", ::GetLastError());
			return false;
		}
		CHUCKSTATION5_INFO("[GPU] GPU heap: %zu MB reserved via VirtualAlloc.", kSize / (1024 * 1024));
		return true;
#else
	...
	void* Alloc(size_t bytes, size_t align = 256)
	{
		...
		{
			CHUCKSTATION5_ERROR("[GPU] GpuHeap: out of GPU memory.");
			return nullptr;
		}
...
struct GpuExtState
{
	...
	static GpuExtState& Get()
	{
		static GpuExtState s;
		return s;
	}
};
...
bool Init(Renderer::IRendererBackend* backend)
{
	...
	if (backend)
	{
		st.backend = backend;
		st.nullMode = false;
		CHUCKSTATION5_INFO("[GPU] Initialised with backend: %s", backend->Name().data());
	}
	else
	{
#if !defined(_WIN32)
		...
		st.backend = nullptr;
		st.nullMode = true;
		CHUCKSTATION5_INFO("[GPU] Initialised in null/headless mode (testing).");
	}
	st.currentRT = {};
	st.currentDT = {};
	...
	if (!GpuHeap::Get().base)
	{
		if (!GpuHeap::Get().Init())
		{
			CHUCKSTATION5_WARN("[GPU] GPU heap init failed — allocs will return nullptr.");
		}
	}
	return true;
}
...
void Shutdown()
{
	...
	CHUCKSTATION5_INFO("[GPU] Shutdown.");
}
...
bool SetRenderTarget(uint32_t /*slot*/, const RenderTarget& rt)
{
	auto& st = GpuState::Get();
	st.currentRT = rt;
	CHUCKSTATION5_TRACE("[GPU] SetRenderTarget %ux%u fmt=%u", rt.width, rt.height, static_cast<uint32_t>(rt.format));
	return true;
}
...
bool SetDepthTarget(const DepthTarget& dt)
{
	...
	CHUCKSTATION5_TRACE("[GPU] SetDepthTarget %ux%u fmt=%u", dt.width, dt.height, static_cast<uint32_t>(dt.format));
	return true;
}

RenderTarget GetCurrentRenderTarget()
{
	return GpuState::Get().currentRT;
}
DepthTarget GetCurrentDepthTarget()
{
	return GpuState::Get().currentDT;
}
...
bool ClearRenderTarget(uint32_t /*slot*/, float r, float g, float b, float a)
{
	...
	if (!st.nullMode && st.currentRT.dataPtr)
	{
		...
		size_t pixels = static_cast<size_t>(st.currentRT.width) * static_cast<size_t>(st.currentRT.height);
	...
	CHUCKSTATION5_TRACE("[GPU] ClearRenderTarget rgba(%.2f,%.2f,%.2f,%.2f)", r, g, b, a);
	return true;
}

bool ClearDepthTarget(float depth, uint8_t stencil)
{
	CHUCKSTATION5_TRACE("[GPU] ClearDepthTarget d=%.3f s=%u", depth, stencil);
	return true;
}
...
bool Submit(const CommandBuffer& dcb, const CommandBuffer& ccb)
{
	...
	CHUCKSTATION5_TRACE("[GPU] Submit DCB=%zu CCB=%zu bytes", dcb.size, ccb.size);
	return true;
}
...
{
	auto& st = GpuState::Get();
	CHUCKSTATION5_TRACE("[GPU] Flip buf=%d arg=%lld", args.bufferIndex, static_cast<long long>(args.flipArg));
	if (st.backend)
		st.backend->Present();
...
CmdQueueHandle CreateQueue(QueueType type)
{
	...
	s.queues[h] = QueueEntry{type, 0};
	{
		...
		++s.stats.activeQueues;
	}
	CHUCKSTATION5_INFO("[GPU] CreateQueue h=%d type=%u", h, static_cast<uint8_t>(type));
	return h;
}
...
bool SignalFence(FenceHandle h)
{
	...
	}
	{
		...
		entry->signalled.store(true);
	}
	entry->cv.notify_all();
	{
		std::lock_guard sl(s.statsMtx);
		++s.stats.fencesSignaled;
	}
	return true;
}
...
bool WaitFence(FenceHandle h, uint64_t timeoutUs)
{
	...
	return entry->cv.wait_for(lk, std::chrono::microseconds(timeoutUs), [&] { return entry->signalled.load(); });
}
...
{
	CHUCKSTATION5_TRACE("[GPU] Barrier gpuAddr=0x%llx %u→%u", static_cast<unsigned long long>(barrier.gpuAddr),
			   static_cast<uint8_t>(barrier.before), static_cast<uint8_t>(barrier.after));
	auto& s = GpuExtState::Get();
	std::lock_guard sl(s.statsMtx);
	++s.stats.barriers;
	return true;
