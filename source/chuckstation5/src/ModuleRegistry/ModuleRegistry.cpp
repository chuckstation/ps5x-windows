// ChuckStation5 – Module Registry implementation
...
namespace ChuckStation5::ModuleRegistry {
...
bool Init()
{
    ...
    CHUCKSTATION5_INFO("[ModReg] Initialised.");
    return true;
}
...
{
    Reset();
    CHUCKSTATION5_INFO("[ModReg] Shutdown.");
}
...
void Reset()
{
    ...
    rs.loadOrder.clear();
    rs.mainModule = INVALID_MODULE;
    CHUCKSTATION5_INFO("[ModReg] Reset.");
}
                  ...
                  bool isMain)
{
    ...
    if (byNameIt != rs.byName.end()) {
        ...
        existing.refCount++;
        CHUCKSTATION5_DEBUG("[ModReg] '%s' already registered (refCount=%u)",
    ...
    CHUCKSTATION5_INFO("[ModReg] Registered '%s' id=%u exports=%zu",
...
ModuleId Load(const std::filesystem::path& path)
{
    ...
    auto& rs = RegistryState::Get();
    {
        ...
        if (it != rs.byName.end()) {
            ...
            CHUCKSTATION5_DEBUG("[ModReg] '%s' already loaded, refCount++", name.c_str());
            return it->second;
        }
    }
    ...
    if (r != Loader::LoadResult::Ok) {
        CHUCKSTATION5_ERROR("[ModReg] Load '%s' failed: %s",
...
bool Unload(ModuleId id)
{
    ...
    if (m.refCount > 1) {
        m.refCount--;
        CHUCKSTATION5_DEBUG("[ModReg] Unload '%s' refCount→%u", m.name.c_str(), m.refCount);
        return true;
    }

    CHUCKSTATION5_INFO("[ModReg] Unloading '%s' id=%u", m.name.c_str(), id);
    ...
    for (auto& [oid, om] : rs.modules) {
        ...
        for (auto dep : om.deps)
            if (dep == id) {
                CHUCKSTATION5_WARN("[ModReg] Cannot unload '%s' – '%s' depends on it",
...
bool ApplyRelocations(ModuleId id)
{
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (symId < m.elfInfo.symbols.size()) {
                ...
                if (sym.shndx != 0) {
                    ...
                    if (!S) {
                        CHUCKSTATION5_WARN("[ModReg] Unresolved symbol '%s' in '%s'",
            ...
            switch (type) {
                ...
                default:
                    CHUCKSTATION5_WARN("[ModReg] Unhandled reloc type %u in '%s'",
                              type, m.name.c_str());
                    ++skipped;
                    continue;
            }
    ...
    for (const auto& needed : m.elfInfo.needed) {
        if (rs.byName.find(needed) == rs.byName.end())
            CHUCKSTATION5_WARN("[ModReg] '%s' needs '%s' (not loaded)",
                      m.name.c_str(), needed.c_str());
    }

    CHUCKSTATION5_INFO("[ModReg] Relocated '%s': %u applied, %u skipped.",
...
void DumpModules()
{
    auto& rs = RegistryState::Get();
    std::lock_guard lk(rs.mtx);
    CHUCKSTATION5_INFO("[ModReg] === Module Table (%u modules) ===",
              static_cast<unsigned>(rs.modules.size()));
    for (auto id : rs.loadOrder) {
        auto it = rs.modules.find(id);
        if (it == rs.modules.end()) continue;
        const auto& m = it->second;
        CHUCKSTATION5_INFO("[ModReg]  [%3u] %-30s  refs=%u  exports=%zu  %s",
...
void DumpSymbols(ModuleId id)
{
    ...
    const auto& m = it->second;
    CHUCKSTATION5_INFO("[ModReg] Symbols in '%s':", m.name.c_str());
    for (const auto& sym : m.exports)
        CHUCKSTATION5_INFO("[ModReg]   0x%016llx  %s  sz=%zu",
...
void DumpDependencyGraph()
{
    auto& rs = RegistryState::Get();
    std::lock_guard lk(rs.mtx);
    CHUCKSTATION5_INFO("[ModReg] === Dependency Graph ===");
    for (auto id : rs.loadOrder) {
        auto it = rs.modules.find(id);
        if (it == rs.modules.end()) continue;
        const auto& m = it->second;
        if (m.deps.empty()) {
            CHUCKSTATION5_INFO("[ModReg]  %s  (no deps)", m.name.c_str());
        } else {
            for (auto dep : m.deps) {
                auto dit = rs.modules.find(dep);
                std::string dname = (dit != rs.modules.end()) ? dit->second.name : "?";
                CHUCKSTATION5_INFO("[ModReg]  %s  →  %s", m.name.c_str(), dname.c_str());
            }
        }
    }
}
