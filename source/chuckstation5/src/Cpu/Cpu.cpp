// ChuckStation5 – Guest CPU Backend (x86-64 interpreter)
...
static StepResult ExecuteOne(CpuState& st)
{
    ...
    if (ctx.rip == 0) {
        CHUCKSTATION5_ERROR("[Cpu] Null RIP – aborting");
    ...
    if (!st.bpByAddr.empty()) {
        ...
        if (it != st.bpByAddr.end()) {
            ...
            CHUCKSTATION5_DEBUG("[Cpu] Breakpoint hit @ 0x%llx",
    ...
    case 0x83: {
        ...
        switch (ext) {
            ...
            default:
                CHUCKSTATION5_WARN("[Cpu] 0x83 /%u not implemented @ 0x%llx", ext,
    ...
    case 0xCD: {
        ...
        CHUCKSTATION5_WARN("[Cpu] INT 0x%02x @ 0x%llx", vec,
                   static_cast<unsigned long long>(saved_rip));
        return StepResult::Unimplemented;
    }
    ...
    case 0x0F: {
        ...
        CHUCKSTATION5_WARN("[Cpu] 0x0F 0x%02x not implemented @ 0x%llx", op2,
                   static_cast<unsigned long long>(saved_rip));
        ++st.stats.unimplemented;
        return StepResult::Unimplemented;
    }
    ...
    default:
        CHUCKSTATION5_WARN("[Cpu] Unimplemented opcode 0x%02x @ 0x%llx", opcode,
                   static_cast<unsigned long long>(saved_rip));
        ++st.stats.unimplemented;
        return StepResult::Unimplemented;
    }
...
bool Init()
{
    ...
    st.initialised = true;
    CHUCKSTATION5_INFO("[Cpu] x86-64 interpreter initialised (Phase 8).");
    return true;
}
...
void Shutdown()
{
    ...
    st.insnCb  = nullptr;
    st.faultCb = nullptr;
    CHUCKSTATION5_INFO("[Cpu] Interpreter shut down. Instructions executed: %llu",
...
StepResult Run()
{
    ...
    st.running.store(false);
    CHUCKSTATION5_DEBUG("[Cpu] Run() ended with %s", StepResultName(result));
    return result;
}
...
uint32_t AddBreakpoint(uint64_t addr, const std::string& label)
{
    ...
    st.bpByAddr[addr] = id;
    CHUCKSTATION5_DEBUG("[Cpu] Breakpoint %u set @ 0x%llx (%s)",
