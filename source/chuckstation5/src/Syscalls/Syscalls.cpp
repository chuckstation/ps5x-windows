// ChuckStation5 – Syscall Dispatcher (Windows-native, Phase 8)
...
namespace ChuckStation5::Syscalls
{
...
struct SyscallState
{
	...
	static SyscallState& Get()
	{
		static SyscallState s;
		return s;
	}
};
...
static uintptr_t Brk(uintptr_t addr)
{
	...
	if (addr < 16 * 1024 * 1024)
		return addr; // Small guest address returned directly
	if (addr < brkBase)
		return brkCurrent;
	size_t newSize = addr - brkBase;
	if (newSize > kBrkReserve)
	{
		CHUCKSTATION5_WARN("[Syscall] brk: requested 0x%zx exceeds reserve.", newSize);
		return brkCurrent;
	}
#if defined(_WIN32)
	...
	if (toCommit > 0)
	{
		void* commit = ::VirtualAlloc(reinterpret_cast<void*>(brkBase + brkSize), toCommit, MEM_COMMIT, PAGE_READWRITE);
		if (!commit)
		{
			CHUCKSTATION5_ERROR("[Syscall] brk: VirtualAlloc commit failed (err=%lu).", ::GetLastError());
			return brkCurrent;
		}
		brkSize = newSize;
	}
...
static void* Mmap(void* hint, size_t length, int prot, int flags)
{
	...
	if (!p)
	{
		CHUCKSTATION5_WARN("[Syscall] mmap: VirtualAlloc failed len=%zu err=%lu.", length, ::GetLastError());
		return reinterpret_cast<void*>(~0ULL); // MAP_FAILED
	}
	return p;
#else
...
static int Munmap(void* addr, size_t length)
{
	...
	{
		CHUCKSTATION5_WARN("[Syscall] munmap: VirtualFree failed err=%lu.", ::GetLastError());
		return -1;
	}
	return 0;
#else
...
static int64_t Builtin_write(const SyscallArgs& a)
{
	...
	if (fd == 1 || fd == 2)
	{
		if (buf && count > 0)
		{
			CHUCKSTATION5_INFO("[Guest %s] %.*s", fd == 1 ? "stdout" : "stderr", static_cast<int>(count),
	...
	DWORD written = 0;
	if (buf && count > 0 &&
		::WriteFile(hFile, reinterpret_cast<const void*>(buf), static_cast<DWORD>(count), &written, nullptr))
		return static_cast<int64_t>(written);
#endif
	CHUCKSTATION5_WARN("[Syscall] write: unsupported fd=%llu", static_cast<unsigned long long>(fd));
	return -9; // EBADF
}
...
static int64_t Builtin_read(const SyscallArgs& a)
{
	...
	uint64_t count = a.arg2;
#if defined(_WIN32)
	HANDLE hFile = (fd == 0) ? ::GetStdHandle(STD_INPUT_HANDLE) : reinterpret_cast<HANDLE>(static_cast<uintptr_t>(fd));
	DWORD nRead = 0;
	if (buf && count > 0 && ::ReadFile(hFile, reinterpret_cast<void*>(buf), static_cast<DWORD>(count), &nRead, nullptr))
...
static int64_t Builtin_exit(const SyscallArgs& a)
{
	...
	CHUCKSTATION5_INFO("[Syscall] exit(%d)", code);
	Cpu::Stop();
	return 0;
}
...
static int64_t Builtin_clock_gettime(const SyscallArgs& a)
{
	...
	if (tsAddr)
	{
		...
		uint64_t totalNs = static_cast<uint64_t>(counter.QuadPart * 1'000'000'000LL / freq.QuadPart);
...
static int64_t Builtin_brk(const SyscallArgs& a)
{
	...
	CHUCKSTATION5_DEBUG("[Syscall] brk(0x%zx) → 0x%zx", static_cast<size_t>(addr), static_cast<size_t>(result));
...
static int64_t Builtin_mmap(const SyscallArgs& a)
{
	...
	if (p == reinterpret_cast<void*>(~0ULL))
		return -12; // ENOMEM
	CHUCKSTATION5_DEBUG("[Syscall] mmap(len=%zu, prot=%d) → 0x%p", length, prot, p);
...
static int64_t Builtin_munmap(const SyscallArgs& a)
{
	...
	CHUCKSTATION5_DEBUG("[Syscall] munmap(0x%p, %zu) → %d", addr, length, r);
	return r;
}
...
static int64_t Builtin_mprotect(const SyscallArgs& a)
{
	...
	{
		CHUCKSTATION5_WARN("[Syscall] mprotect: VirtualProtect failed err=%lu.", ::GetLastError());
		return -13; // EACCES
	}
#else
	(void)addr;
...
static int64_t Builtin_getrusage(const SyscallArgs& a)
{
	...
	if (structAddr)
		std::memset(reinterpret_cast<void*>(structAddr), 0, 144);
	return 0;
}
...
static int64_t Builtin_ioctl(const SyscallArgs& a)
{
	...
	if (request == FIONREAD)
	{
		...
		{
			int n = 0;
			std::memcpy(reinterpret_cast<void*>(argp), &n, 4);
		}
		return 0;
	}
	...
	if (fd >= 3)
	{
		...
		BOOL ok = ::DeviceIoControl(h, static_cast<DWORD>(request), inBuf, inSz, inBuf, inSz, &bytes, nullptr);
		if (ok)
		{
			CHUCKSTATION5_DEBUG("[Syscall] ioctl fd=%llu req=0x%llx → ok (%lu bytes)", static_cast<unsigned long long>(fd),
	...
	CHUCKSTATION5_WARN("[Syscall] ioctl fd=%llu req=0x%llx argp=0x%llx — ENOTTY", static_cast<unsigned long long>(fd),
			  static_cast<unsigned long long>(request), static_cast<unsigned long long>(argp));
	return -25; // ENOTTY
}

static int64_t Unknown_handler(const SyscallArgs& a)
{
	CHUCKSTATION5_WARN("[Syscall] UNKNOWN nr=%llu — ENOSYS", static_cast<unsigned long long>(a.number));
	return -38; // ENOSYS
}
...
bool Init()
{
	...
	st.initialised = true;
	CHUCKSTATION5_INFO("[Syscall] Dispatcher initialised (Windows-native).");
	return true;
}
...
void Shutdown()
{
	...
	CHUCKSTATION5_INFO("[Syscall] Dispatcher shut down.");
}
...
void RegisterBuiltins()
{
	...
	CHUCKSTATION5_INFO("[Syscall] Registered %zu built-in handlers.", std::size_t{16});
}
...
void RegisterSyscall(const SyscallDesc& desc)
{
	...
	CHUCKSTATION5_DEBUG("[Syscall] Registered %llu (%s)", static_cast<unsigned long long>(desc.number), desc.name.c_str());
}

void RegisterSyscall(uint64_t number, std::string name, HandlerFn handler, uint8_t argCount)
{
...
bool Dispatch(Cpu::CpuContext& ctx)
{
	...
	CHUCKSTATION5_TRACE("[Syscall] %llu (%s) → %lld", static_cast<unsigned long long>(args.number),
			   sname.empty() ? "unknown" : sname.c_str(), static_cast<long long>(result));

	RuntimeEvents::PublishCustom("syscall", (sname.empty() ? std::to_string(args.number) : sname) + " -> " +
...

	{
		...
		st.recentLog.push_back(
			{args.number, sname.empty() ? ("nr:" + std::to_string(args.number)) : sname, args, result, NowUs()});
	}
	return true;
}
...
std::vector<SyscallRecord> GetRecentLog(size_t maxEntries)
{
	...
	return std::vector<SyscallRecord>(st.recentLog.end() - static_cast<ptrdiff_t>(n), st.recentLog.end());
}
