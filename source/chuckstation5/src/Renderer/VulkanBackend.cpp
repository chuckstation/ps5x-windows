// ChuckStation5 – Renderer backends (Windows-only, Phase 8)
...
namespace ChuckStation5::Renderer {
...
public:
    bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
    {
...
#if defined(_DEBUG)
        {
            ...
            if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debug)))) {
                ...
                debug->Release();
                CHUCKSTATION5_DEBUG("[DX12] Debug layer enabled.");
            }
        }
#endif
        ...
        if (FAILED(hr)) {
            CHUCKSTATION5_ERROR("[DX12] CreateDXGIFactory1 failed (hr=0x%08X).", hr);
            return false;
        }
        ...
        for (UINT i = 0; factory_->EnumAdapters1(i, &adapter) != DXGI_ERROR_NOT_FOUND; ++i) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                CHUCKSTATION5_INFO("[DX12] Adapter: %s", adapterName);
                adapter->Release();
                break;
            }
            adapter->Release();
        ...
        if (!device_) {
            CHUCKSTATION5_ERROR("[DX12] No D3D12-capable adapter found.");
            return false;
        }
        ...
        if (FAILED(hr)) { CHUCKSTATION5_ERROR("[DX12] CreateCommandQueue failed."); return false; }
        ...
        if (hwnd_) {
            ...
            if (SUCCEEDED(factory_->QueryInterface(IID_PPV_ARGS(&f4)))) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    sc1->Release();
                    CHUCKSTATION5_INFO("[DX12] Swapchain created %ux%u.", width_, height_);
                }
            }
        } else {
            CHUCKSTATION5_INFO("[DX12] Headless mode — no swapchain.");
        }
        ...
        CHUCKSTATION5_INFO("[DX12] Backend ready.");
        ready_ = true;
        return true;

#else // non-Windows
        CHUCKSTATION5_INFO("[DX12] Windows not detected — backend inactive.");
        (void)sc;
...
#endif
        ready_ = false;
        CHUCKSTATION5_INFO("[DX12] Backend shut down.");
    }
    ...
    bool Resize(uint32_t w, uint32_t h) override
    {
        ...
        if (FAILED(hr)) { CHUCKSTATION5_ERROR("[DX12] ResizeBuffers failed."); return false; }
        D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle =
            rtvHeap_->GetCPUDescriptorHandleForHeapStart();
        for (UINT f = 0; f < kFrameCount; ++f) {
            swapChain_->GetBuffer(f, IID_PPV_ARGS(&renderTargets_[f]));
        ...
        CHUCKSTATION5_INFO("[DX12] Resized to %ux%u.", w, h);
#endif
        return true;
    }
...
public:
    bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
    {
        ...
        if (FAILED(hr)) {
            CHUCKSTATION5_ERROR("[DX11] D3D11CreateDevice failed (hr=0x%08X).", hr);
            return false;
        }
        ...
        CHUCKSTATION5_INFO("[DX11] Backend ready (feature level 0x%x).",
...
#endif
        ready_ = false;
        CHUCKSTATION5_INFO("[DX11] Backend shut down.");
    }
    ...
    bool Resize(uint32_t w, uint32_t h) override
    {
        ...
        CHUCKSTATION5_INFO("[DX11] Resized to %ux%u.", w, h);
#endif
        return true;
    }
...
public:
    bool Init(const Config::GraphicsConfig& cfg, const SwapChainDesc& sc) override
    {
        ...
        if (!hVulkan_) {
            CHUCKSTATION5_WARN("[Vulkan] vulkan-1.dll not found — backend unavailable.");
            return false;
        }
        ...
        if (!vkGetInstanceProcAddr) {
            CHUCKSTATION5_ERROR("[Vulkan] Failed to resolve vkGetInstanceProcAddr.");
        ...
        CHUCKSTATION5_INFO("[Vulkan] vulkan-1.dll loaded. Instance/device init pending Kyty milestone.");
        ready_ = true;
        return true;
#else
        ready_ = true;
...
#endif
        ready_ = false;
        CHUCKSTATION5_INFO("[Vulkan] Backend shut down.");
    }
    ...
    {
        width_ = w; height_ = h;
        CHUCKSTATION5_INFO("[Vulkan] Resize %ux%u (swapchain recreate pending Kyty milestone).", w, h);
        return true;
    }
...
public:
    bool Init(const Config::GraphicsConfig&, const SwapChainDesc& sc) override
    {
        ...
        CHUCKSTATION5_INFO("[NullRenderer] Headless backend ready (%ux%u).", width_, height_);
        return true;
    }
    void Shutdown() override { CHUCKSTATION5_INFO("[NullRenderer] Shut down."); }
...
std::unique_ptr<IRendererBackend> CreateBackend(Config::GraphicsBackend type)
{
    ...
    CHUCKSTATION5_ERROR("[Renderer] Unknown backend type %u", static_cast<unsigned>(type));
    return nullptr;
}
