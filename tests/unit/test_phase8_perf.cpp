...
TEST_CASE("Phase8::Perf::FrameTime::FPSCalculation", "[perf][phase8]")
{
    PerfTools::Init();
    // 16.667 ms ≈ 60 fps
    for (int i = 0; i < 10; ++i) PerfTools::RecordFrameTime(16.667);
...
TEST_CASE("Phase8::Perf::FrameTime::RollingWindow", "[perf][phase8]")
{
    ...
    for (int i = 0; i < 500; ++i) PerfTools::RecordFrameTime(16.667);
    auto stats = PerfTools::GetFrameStats();
...
TEST_CASE("Phase8::Perf::ScopeTimer::MeasuresPositiveDuration", "[perf][phase8]")
{
    PerfTools::Init();
    {
...
{
    PerfTools::Init();
    for (int i = 0; i < 5; ++i) { PerfTools::ScopeTimer t("A"); }
    for (int i = 0; i < 3; ++i) { PerfTools::ScopeTimer t("B"); }
    auto sections = PerfTools::GetSectionStats();
    bool foundA = false, foundB = false;
    for (auto& s : sections) {
        if (s.name == "A") { CHECK(s.callCount == 5); foundA = true; }
        if (s.name == "B") { CHECK(s.callCount == 3); foundB = true; }
    }
    CHECK(foundA);
    CHECK(foundB);
    PerfTools::Shutdown();
...
TEST_CASE("Phase8::Perf::CommandProcessor::EmptyListOverhead", "[perf][phase8]")
{
    GPU::Init(nullptr);
    CommandProcessor::Init(nullptr);
...
TEST_CASE("Phase8::Perf::Annotation::MultipleNamedBenchmarks", "[perf][phase8]")
{
    PerfTools::Init();
    std::vector<std::string> names = {"Alpha","Beta","Gamma","Delta"};
    for (auto& n : names) {
        PerfTools::BeginBenchmark(n);
        volatile int x = 0; for (int i=0;i<100;i++) x+=i; (void)x;
    ...
    for (auto& n : names) {
        bool found = false;
        for (auto& r : results) if (r.name == n) { found=true; break; }
        CHECK(found);
    }
    PerfTools::Shutdown();
}
