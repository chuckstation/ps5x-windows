...
{ ShaderKey k; k.spirvHash=spirvH; k.pipelineHash=pipeH; k.stage=stage; return k; }
...
// ── Compile ───────────────────────────────────────────────────────────────
TEST_CASE("ShaderCache – Compile without custom compiler stores SPIR-V", "[sc]")
{
    Setup();
    auto spirv = FakeSPIRV(0x10);
    auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), 0, ShaderStage::Vertex);
...
TEST_CASE("ShaderCache – Compile with custom compiler uses it", "[sc]")
{
    ...
    SetCompiler([&](const std::vector<uint8_t>& spirv, ShaderStage) -> std::vector<uint8_t> {
        compilerCalled = true;
    ...
    auto key   = MakeKey(HashSpirv(spirv.data(),spirv.size()), 1, ShaderStage::Fragment);
...
TEST_CASE("ShaderCache – Second Compile returns cache hit", "[sc]")
{
    ...
    auto key   = MakeKey(HashSpirv(spirv.data(),spirv.size()), 2, ShaderStage::Compute);
...
TEST_CASE("ShaderCache – Lookup succeeds after Compile", "[sc]")
{
    ...
    auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), 3, ShaderStage::Vertex);
...
TEST_CASE("ShaderCache – Invalidate removes specific entry", "[sc]")
{
    ...
    auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), 4, ShaderStage::Fragment);
...
TEST_CASE("ShaderCache – InvalidateAll clears cache", "[sc]")
{
    ...
    for (uint8_t i = 0; i < 5; ++i) {
        ...
        auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), i, ShaderStage::Vertex);
        Compile(key, spirv);
    }
...
TEST_CASE("ShaderCache – QueueCompile and FlushQueue", "[sc]")
{
    Setup();
    int compiled = 0;
    SetCompiler([&](const std::vector<uint8_t>& s, ShaderStage) -> std::vector<uint8_t> {
        compiled++;
        return s;
    });
    ...
    for (uint8_t i = 0; i < 4; ++i) {
        ...
        auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), i+10, ShaderStage::Vertex);
...
TEST_CASE("ShaderCache – Evict removes stale entries", "[sc]")
{
    ...
    auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), 99, ShaderStage::Vertex);
...
TEST_CASE("ShaderCache – SaveToDisk / LoadFromDisk roundtrip", "[sc]")
{
    ...
    // Write
    {
        ...
        auto key = MakeKey(HashSpirv(spirv.data(),spirv.size()), 200, ShaderStage::Compute);
