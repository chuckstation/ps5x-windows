...
TEST_CASE("Loader – LoadResultStr coverage", "[loader]") {
  REQUIRE(std::string(PS5x::Loader::LoadResultStr(
              PS5x::Loader::LoadResult::Ok)) == "Ok");
  REQUIRE(std::string(PS5x::Loader::LoadResultStr(
              PS5x::Loader::LoadResult::FileNotFound)) == "FileNotFound");
  REQUIRE(std::string(PS5x::Loader::LoadResultStr(
              PS5x::Loader::LoadResult::FirmwareRequired)) ==
          "FirmwareRequired");
}
...
TEST_CASE("Loader – firmware validation rejects empty path", "[loader]") {
  PS5x::Logger::Init("", false, PS5x::Logger::Level::Off);
  REQUIRE(PS5x::Loader::ValidateFirmware("") ==
          PS5x::Loader::LoadResult::FirmwareRequired);
  REQUIRE(PS5x::Loader::ValidateFirmware("/nonexistent/fw") ==
          PS5x::Loader::LoadResult::FirmwareRequired);
  PS5x::Logger::Shutdown();
}
