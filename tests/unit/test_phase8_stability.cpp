...
TEST_CASE("Phase8::Stability::Process::MultipleInitShutdown",
          "[stability][phase8]") {
  for (int i = 0; i < 5; ++i) {
    CHECK(Process::Init());
    Process::Shutdown();
...
TEST_CASE("Phase8::Stability::Process::ShutdownWithNoProcesses",
          "[stability][phase8]") {
  CHECK(Process::Init());
...
TEST_CASE("Phase8::Stability::Process::DoubleShutdownSafe",
          "[stability][phase8]") {
  Process::Init();
  Process::Shutdown();
...
TEST_CASE("Phase8::Stability::Process::InitAfterShutdown",
          "[stability][phase8]") {
  Process::Init();
  Process::Shutdown();
  CHECK(Process::Init());
...
TEST_CASE("Phase8::Stability::Memory::InitShutdownCycle",
          "[stability][phase8]") {
  for (int i = 0; i < 3; ++i) {
    CHECK(Memory::Init());
    Memory::Shutdown();
...
TEST_CASE("Phase8::Stability::Memory::AllocFreeBalance",
          "[stability][phase8]") {
  Memory::Init();
  ...
  CHECK(after.totalAllocated <=
        before.totalAllocated + 128); // padding tolerance
  Memory::Shutdown();
}

TEST_CASE("Phase8::Stability::Memory::MultipleAllocations",
          "[stability][phase8]") {
  Memory::Init();
  constexpr int N = 20;
...
TEST_CASE("Phase8::Stability::MemoryDiag::InitShutdown",
          "[stability][phase8]") {
  CHECK(MemoryDiag::Init());
  MemoryDiag::Shutdown();
}

TEST_CASE("Phase8::Stability::MemoryDiag::ReportAfterInit",
          "[stability][phase8]") {
  MemoryDiag::Init();
...
TEST_CASE("Phase8::Stability::MemoryDiag::RecordAndFree",
          "[stability][phase8]") {
  MemoryDiag::Init();
...
TEST_CASE("Phase8::Stability::MemoryDiag::UnfreedShowsAsLeak",
          "[stability][phase8]") {
  MemoryDiag::Init();
...
TEST_CASE("Phase8::Stability::Cpu::BreakpointsClearedOnReset",
          "[stability][phase8]") {
  Cpu::Init();
...
TEST_CASE("Phase8::Stability::Syscall::MultipleRegistrationsStable",
          "[stability][phase8]") {
  Syscalls::Init();
  for (uint64_t n = 50000; n < 50020; ++n) {
    Syscalls::RegisterSyscall(
        n, "bench_" + std::to_string(n),
...
TEST_CASE("Phase8::Stability::Syscall::StatsResetBetweenRuns",
          "[stability][phase8]") {
  Syscalls::Init();
...
TEST_CASE("Phase8::Stability::KernelRuntime::HandleExhaustion",
          "[stability][phase8]") {
  KernelRuntime::Init();
  ...
  for (auto h : handles)
    KernelRuntime::FreeHandle(h);
  KernelRuntime::Shutdown();
}

// ── Runtime subsystem stability ───────────────────────────────────────────

TEST_CASE("Phase8::Stability::Runtime::SubsystemNameTable",
          "[stability][phase8]") {
  // All valid SubsystemIds should return a non-empty string
  using SID = PS5x::Runtime::SubsystemId;
  std::vector<SID> ids = {
      PS5x::Runtime::SubsystemId::Logger,
      PS5x::Runtime::SubsystemId::Config,
      PS5x::Runtime::SubsystemId::Memory,
      PS5x::Runtime::SubsystemId::Kernel,
      PS5x::Runtime::SubsystemId::Filesystem,
      PS5x::Runtime::SubsystemId::Loader,
      PS5x::Runtime::SubsystemId::KytyAdapter,
      PS5x::Runtime::SubsystemId::Renderer,
      PS5x::Runtime::SubsystemId::GPU,
      PS5x::Runtime::SubsystemId::Audio,
      PS5x::Runtime::SubsystemId::Input,
      PS5x::Runtime::SubsystemId::Process,
      PS5x::Runtime::SubsystemId::Debugger,
      PS5x::Runtime::SubsystemId::UI,
  };
  for (auto id : ids) {
...
TEST_CASE("Phase8::Stability::Runtime::SubsystemStateNameTable",
          "[stability][phase8]") {
  using SS = PS5x::Runtime::SubsystemState;
  std::vector<SS> states = {
      PS5x::Runtime::SubsystemState::Unregistered,
      PS5x::Runtime::SubsystemState::Registered,
      PS5x::Runtime::SubsystemState::Initialising,
      PS5x::Runtime::SubsystemState::Running,
      PS5x::Runtime::SubsystemState::ShuttingDown,
      PS5x::Runtime::SubsystemState::Stopped,
      PS5x::Runtime::SubsystemState::Failed,
  };
  for (auto s : states) {
...
TEST_CASE("Phase8::Stability::Cpu::FaultCallbackFired", "[stability][phase8]") {
  ...
  // Note: faultCb is called in ExecuteOne only if a real fault handler calls
  // it; our check here is that Step returns Fault cleanly.
  Cpu::Shutdown();
}

TEST_CASE("Phase8::Stability::Cpu::FaultStatIncremented",
          "[stability][phase8]") {
  Cpu::Init();
  Cpu::ResetStats();
  // DIV by zero → fault
