...
struct HomebrewRun {
  ...
  static std::vector<uint8_t> MakeSyscallProgram(uint64_t nr,
                                                 uint64_t arg0 = 0) {
    std::vector<uint8_t> code;
    // mov rax, nr  (48 C7 C0 <nr32>)
    code.insert(code.end(),
                {0x48, 0xC7, 0xC0, static_cast<uint8_t>(nr),
                 static_cast<uint8_t>(nr >> 8), static_cast<uint8_t>(nr >> 16),
                 static_cast<uint8_t>(nr >> 24)});
    // mov rdi, arg0
    code.insert(code.end(), {0x48, 0xC7, 0xC7, static_cast<uint8_t>(arg0),
  ...
  Cpu::StepResult Run(const std::vector<uint8_t> &code, int maxSteps = 200) {
    ...
    Syscalls::RegisterSyscall(
        ...
        1);
    Syscalls::RegisterSyscall(
        Syscalls::Nr::ExitGrp, "exit_group",
        [&](const Syscalls::SyscallArgs &a) -> int64_t {
          exitCalled = true;
          exitCode = static_cast<int>(a.arg0);
          Cpu::Stop();
...
TEST_CASE("Phase8::Homebrew::HelloWorld::NonZeroExitCode",
          "[homebrew][phase8]") {
  HomebrewRun hw;
...
TEST_CASE("Phase8::Homebrew::ConsoleLog::WriteSyscallSucceeds",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::ConsoleLog::WriteBadFdReturnsError",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::FileIO::UnmountedPathEmpty",
          "[homebrew][phase8]") {
  Filesystem::Init();
...
TEST_CASE("Phase8::Homebrew::Input::RecordingAndPlayback",
          "[homebrew][phase8]") {
  Input::Init();
...
TEST_CASE("Phase8::Homebrew::Threading::GetTidReturnsPositive",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::Threading::SchedYieldSucceeds",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::Threading::ClockGettimeReturnsPositive",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::MemAlloc::AllocAndFreeStack",
          "[homebrew][phase8]") {
  Memory::Init();
...
TEST_CASE("Phase8::Homebrew::MemAlloc::AllocHeapAndFill",
          "[homebrew][phase8]") {
  Memory::Init();
...
TEST_CASE("Phase8::Homebrew::MemAlloc::BrkStubReturnsArg",
          "[homebrew][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
...
TEST_CASE("Phase8::Homebrew::Integration::ExitProgramStats",
          "[homebrew][phase8]") {
  HomebrewRun hw;
  auto code = HomebrewRun::MakeSyscallProgram(Syscalls::Nr::Exit, 0);
