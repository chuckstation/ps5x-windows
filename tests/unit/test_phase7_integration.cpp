...
static std::vector<uint8_t> MakeExitProgram(int exitCode = 0) {
  ...
  // mov rax, 60
  code.insert(code.end(), {0x48, 0xC7, 0xC0, 60, 0, 0, 0}); // mov rax, 60
  // mov rdi, exitCode
  code.insert(code.end(), {0x48, 0xC7, 0xC7, static_cast<uint8_t>(exitCode), 0,
                           0, 0}); // mov rdi, exitCode
  // SYSCALL
...
TEST_CASE("Phase7::Integration::SyscallStatsAfterRun",
          "[integration][phase7]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::ResetStats();
...
TEST_CASE("Phase7::Integration::BreakpointInterruptsRun",
          "[integration][phase7]") {
  Cpu::Init();
  Syscalls::Init();
...
TEST_CASE("Phase7::Integration::RuntimeEventsFromSyscall",
          "[integration][phase7]") {
  RuntimeEvents::Init();
  RuntimeEvents::Reset();
  ...
  auto subId =
      RuntimeEvents::Subscribe([&](const RuntimeEvents::RuntimeEvent &ev) {
        if (ev.type == RuntimeEvents::EventType::Custom)
          ++eventCount;
      });

  Syscalls::RegisterSyscall(
      39, "getpid", [](const Syscalls::SyscallArgs &) -> int64_t { return 1; });
