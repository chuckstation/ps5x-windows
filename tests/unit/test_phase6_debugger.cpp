// PS5x – Phase 6 Debugger tests (conditional BPs, watches, symbol browser,
...
TEST_CASE("Phase6::Debugger::ConditionalBP::AddReturnsId",
          "[debugger][phase6]") {
  uint32_t id = AddConditionalBreakpoint(
      0x1000, [](const CpuState &) { return true; }, "always");
  CHECK(id >= 10000u);
}

TEST_CASE("Phase6::Debugger::ConditionalBP::MultipleIds",
          "[debugger][phase6]") {
  auto id1 =
      AddConditionalBreakpoint(0x2000, [](const CpuState &) { return false; });
  auto id2 =
      AddConditionalBreakpoint(0x3000, [](const CpuState &) { return true; });
  CHECK(id1 != id2);
  CHECK(id2 == id1 + 1);
}

TEST_CASE("Phase6::Debugger::ConditionalBP::LabelStored",
          "[debugger][phase6]") {
  // Just verify no crash and id monotonically increases
  auto id = AddConditionalBreakpoint(
      0x5000, [](const CpuState &) { return true; }, "my_cond_bp");
  CHECK(id > 0);
}
...
TEST_CASE("Phase6::Debugger::SymbolBrowser::BrowseAll", "[debugger][phase6]") {
  // No modules loaded - result should be an empty (or small) vector without
  // crash
...
TEST_CASE("Phase6::Debugger::SymbolBrowser::FilterEmpty",
          "[debugger][phase6]") {
  auto syms = BrowseSymbols("");
  SUCCEED("BrowseSymbols(\"\") returned without crash");
  (void)syms;
...
TEST_CASE("Phase6::Debugger::SymbolBrowser::FilterNoMatch",
          "[debugger][phase6]") {
  auto syms = BrowseSymbols("zzz_no_such_symbol_xyz");
  CHECK(syms.empty());
}

TEST_CASE("Phase6::Debugger::SymbolBrowser::AddressToSymbol_Unknown",
          "[debugger][phase6]") {
...
TEST_CASE("Phase6::Debugger::EventHistory::ClearAndEmpty",
          "[debugger][phase6]") {
  ClearEventHistory();
...
TEST_CASE("Phase6::Debugger::EventHistory::PopulatedByConditionalBP",
          "[debugger][phase6]") {
  ClearEventHistory();
  AddConditionalBreakpoint(
      0xDEAD, [](const CpuState &) { return false; }, "hist_test");
  auto events = GetEventHistory();
  CHECK(!events.empty());
  bool found = false;
  for (auto &e : events) {
    if (e.type == "ConditionalBreakpoint")
      found = true;
  }
  CHECK(found);
}

TEST_CASE("Phase6::Debugger::EventHistory::LimitRespected",
          "[debugger][phase6]") {
  ClearEventHistory();
...
TEST_CASE("Phase6::Debugger::EventHistory::TimestampsNonZero",
          "[debugger][phase6]") {
  ClearEventHistory();
  AddConditionalBreakpoint(
      0x1111, [](const CpuState &) { return true; }, "ts_test");
  auto events = GetEventHistory();
  for (auto &e : events) {
