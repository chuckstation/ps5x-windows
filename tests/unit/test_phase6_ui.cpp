...
TEST_CASE("Phase6::UI::PanelName::AllPanels", "[ui][phase6]") {
  CHECK(std::string(PanelName(Panel::ExecutionDashboard)) ==
        "Execution Dashboard");
  ...
  CHECK(std::string(PanelName(Panel::ControllerMonitor)) ==
        "Controller Monitor");
  CHECK(std::string(PanelName(Panel::FilesystemBrowser)) ==
        "Filesystem Browser");
  CHECK(std::string(PanelName(Panel::PerformanceGraphs)) ==
        "Performance Graphs");
  ...
  CHECK(std::string(PanelName(Panel::ShaderCacheViewer)) ==
        "Shader Cache Viewer");
}
