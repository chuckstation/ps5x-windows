...
TEST_CASE("FS – Write then Read back", "[fs]") {
  ...
  auto wfd = Open("/savedata/test.sav",
...
TEST_CASE("FS – Exists, MakeDir, ReadDir, Remove, Rename", "[fs]") {
  ...
  auto fd =
      Open("/savedata/subdir/f.txt", OpenFlags::Write | OpenFlags::Create);
  const char *c = "hi";
  Write(fd, c, 2);
  Close(fd);
