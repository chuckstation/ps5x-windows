...
TEST_CASE("Debugger – Watchpoint add / clear", "[dbg]") {
  Setup();
  uint32_t id =
      PS5x::Debugger::AddWatchpoint(0x8000, 8, false, true, "wp-write");
...
TEST_CASE("Debugger – OnBreakpointHit fires callback and sets CPU state",
          "[dbg]") {
  Setup();
...
TEST_CASE("Debugger – WriteCrashDump creates file", "[dbg]") {
  Setup();
  auto dumpDir =
      (std::filesystem::temp_directory_path() / "ps5x_crash_test").string();
