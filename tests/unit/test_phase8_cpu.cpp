...
TEST_CASE("Phase8::Cpu::ImplementedOpcodes::ContainsPhase7Core",
          "[cpu][phase8]") {
  auto ops = ImplementedOpcodes();
  auto has = [&](uint8_t b) {
    return std::find(ops.begin(), ops.end(), b) != ops.end();
  };
  CHECK(has(0x90)); // NOP
  CHECK(has(0xC3)); // RET
  CHECK(has(0xE8)); // CALL
...
TEST_CASE("Phase8::Cpu::ImplementedOpcodes::ContainsPhase8Additions",
          "[cpu][phase8]") {
  auto ops = ImplementedOpcodes();
  auto has = [&](uint8_t b) {
    return std::find(ops.begin(), ops.end(), b) != ops.end();
...
TEST_CASE("Phase8::Cpu::Insn::TEST_sets_ZF", "[cpu][phase8]") {
  // TEST rax, rcx: 48 85 C8  (ModRM 11 001 000 = C8? No: TEST r/m64,r64; ModRM
  // rm=RAX, reg=RCX) TEST r/m64, r64: opcode 0x85, ModRM 11 reg rm. reg=RCX(1),
  // rm=RAX(0) → 0b11_001_000 = 0xC8
...
TEST_CASE("Phase8::Cpu::Insn::RET_imm16", "[cpu][phase8]") {
  ...
  CHECK(GetContext().gpr_get(Reg::RSP) ==
        reinterpret_cast<uint64_t>(stack.data()) + 8 +
            8); // 8 (ret) + 8 (imm16)
  Shutdown();
}
