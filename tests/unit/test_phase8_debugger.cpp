...
TEST_CASE("Phase8::Debugger::RegisterView::AllGPRsListed",
          "[debugger][phase8]") {
  Debugger::Init();
  Cpu::Init();
  ...
  for (auto &r : regs) {
    if (r.name == "rax" && r.value == 0xDEAD)
      found_rax = true;
    if (r.name == "rbx" && r.value == 0xBEEF)
      found_rbx = true;
  }
  CHECK(found_rax);
  CHECK(found_rbx);
...
TEST_CASE("Phase8::Debugger::RegisterView::RIPIncluded", "[debugger][phase8]") {
  ...
  bool found = false;
  for (auto &r : special) {
    if (r.name == "rip" && r.value == 0xCAFE'BABE) {
      found = true;
      break;
    }
  }
...
TEST_CASE("Phase8::Debugger::RegisterView::FlagsDecoded",
          "[debugger][phase8]") {
  Debugger::Init();
  Cpu::Init();
...
TEST_CASE("Phase8::Debugger::CallStack::MaxDepthRespected",
          "[debugger][phase8]") {
  Debugger::Init();
  Cpu::Init();
...
TEST_CASE("Phase8::Debugger::MemoryView::ReadReturnsBytes",
          "[debugger][phase8]") {
  Debugger::Init();
  Memory::Init();
...
TEST_CASE("Phase8::Debugger::MemoryView::WriteModifiesHost",
          "[debugger][phase8]") {
  Debugger::Init();
  Memory::Init();
...
TEST_CASE("Phase8::Debugger::MemoryView::HexDumpNotEmpty",
          "[debugger][phase8]") {
  Debugger::Init();
  Memory::Init();
...
TEST_CASE("Phase8::Debugger::ModuleBrowser::EmptyOnInit",
          "[debugger][phase8]") {
  Debugger::Init();
  ModuleRegistry::Init();
...
TEST_CASE("Phase8::Debugger::ModuleBrowser::RegistrationVisible",
          "[debugger][phase8]") {
  Debugger::Init();
  ModuleRegistry::Init();
...
TEST_CASE("Phase8::Debugger::SymbolBrowser::LookupUnknownReturnsEmpty",
          "[debugger][phase8]") {
  Debugger::Init();
  ModuleRegistry::Init();
...
TEST_CASE("Phase8::Debugger::SymbolBrowser::RegisterAndResolve",
          "[debugger][phase8]") {
  Debugger::Init();
  ModuleRegistry::Init();
...
TEST_CASE("Phase8::Debugger::SymbolBrowser::NearestSymbol",
          "[debugger][phase8]") {
  Debugger::Init();
  ModuleRegistry::Init();
...
TEST_CASE("Phase8::Debugger::Timeline::RecordAndRetrieve",
          "[debugger][phase8]") {
  Debugger::Init();
...
TEST_CASE("Phase8::Debugger::Timeline::MaxSizeRespected",
          "[debugger][phase8]") {
  Debugger::Init();
...
TEST_CASE("Phase8::Debugger::EventBrowser::CapturesRuntimeEvent",
          "[debugger][phase8]") {
  RuntimeEvents::Init();
  Debugger::Init();
  ...
  for (auto &e : log) {
    if (e.type == RuntimeEvents::EventType::FrameEnd)
      foundFrame = true;
    if (e.type == RuntimeEvents::EventType::ProcessStarted)
      foundProc = true;
  }
  CHECK(foundFrame);
  CHECK(foundProc);
...
TEST_CASE("Phase8::Debugger::EventBrowser::FilterByType",
          "[debugger][phase8]") {
  RuntimeEvents::Init();
  Debugger::Init();
  Debugger::AttachEventBrowser();
