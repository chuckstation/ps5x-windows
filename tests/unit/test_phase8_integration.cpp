...
TEST_CASE("Phase8::Integration::FullInit::AllSubsystems",
          "[integration][phase8]") {
  CHECK(Memory::Init());
  CHECK(Cpu::Init());
  CHECK(Syscalls::Init());
...
TEST_CASE("Phase8::Integration::CPU_Debugger::BreakpointHitTracked",
          "[integration][phase8]") {
  Cpu::Init();
  Debugger::Init();
...
TEST_CASE("Phase8::Integration::CPU_Debugger::RegisterViewAfterExec",
          "[integration][phase8]") {
  Cpu::Init();
  Debugger::Init();
...
TEST_CASE("Phase8::Integration::CPU_Syscall_Memory::GetPidAndBrk",
          "[integration][phase8]") {
  Memory::Init();
  Cpu::Init();
  Syscalls::Init();
...
TEST_CASE("Phase8::Integration::GPU_Events::FrameEndPublished",
          "[integration][phase8]") {
  RuntimeEvents::Init();
  GPU::Init(nullptr);
...
TEST_CASE("Phase8::Integration::PerfTools_CPU_GPU::FullFrame",
          "[integration][phase8]") {
  PerfTools::Init();
  Cpu::Init();
  GPU::Init(nullptr);
...
TEST_CASE("Phase8::Integration::CPU::ShiftAndCompare",
          "[integration][phase8]") {
  ...
  // hlt         (F4)
  alignas(16) uint8_t code[] = {0x48, 0xC7, 0xC0, 0x01, 0x00, 0x00, 0x00, 0x48,
                                0xC1, 0xE0, 0x03, 0x48, 0xC7, 0xC3, 0x08, 0x00,
                                0x00, 0x00, 0x48, 0x39, 0xD8, 0xF4};
...
TEST_CASE("Phase8::Integration::CPU::MovzxPipeline", "[integration][phase8]") {
  ...
  alignas(16) uint8_t code[] = {0x48, 0xC7, 0xC1, 0xAB,
                                0x00, 0x00, 0x00,       // mov rcx, 0xAB
...
TEST_CASE("Phase8::Integration::Homebrew::WriteAndExit",
          "[integration][phase8]") {
  Cpu::Init();
  Syscalls::Init();
  Syscalls::RegisterBuiltins();
  ...
  // exit:  rax=60, rdi=0
  alignas(16) uint8_t code[] = {// mov rax, 1
  ...
  for (int i = 0; i < 30; ++i) {
    ...
    if (r == Cpu::StepResult::Halt || exited)
      break;
  }
...
TEST_CASE("Phase8::Integration::Stats::AllModulesReport",
          "[integration][phase8]") {
  Memory::Init();
  Cpu::Init();
  Syscalls::Init();
  ...
  alignas(16) uint8_t code[] = {
      ...
      0x48, 0xC7, 0xC0, static_cast<uint8_t>(Syscalls::Nr::GetPid), 0x00, 0x00,
      0x00, 0x0F, 0x05, // syscall
      0xF4};
  Cpu::GetContext().rip = reinterpret_cast<uint64_t>(code);
  for (int i = 0; i < 20; ++i) {
    auto r = Cpu::Step();
