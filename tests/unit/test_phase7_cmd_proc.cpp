...
TEST_CASE("Phase7::CmdProc::OpName::AllOpcodes", "[cmd_proc][phase7]") {
  ...
  CHECK(std::string(OpcodeName(CommandOpcode::SetRenderTarget)) ==
        "SetRenderTarget");
  CHECK(std::string(OpcodeName(CommandOpcode::SetDepthTarget)) ==
        "SetDepthTarget");
  ...
  CHECK(std::string(OpcodeName(CommandOpcode::BeginRenderPass)) ==
        "BeginRenderPass");
  CHECK(std::string(OpcodeName(CommandOpcode::EndRenderPass)) ==
        "EndRenderPass");
  CHECK(std::string(OpcodeName(CommandOpcode::PushDebugLabel)) ==
        "PushDebugLabel");
  CHECK(std::string(OpcodeName(CommandOpcode::PopDebugLabel)) ==
        "PopDebugLabel");
...
TEST_CASE("Phase7::CmdProc::Builder::BarrierSize", "[cmd_proc][phase7]") {
  CommandList cl;
  PS5x::GPU::Barrier b{0x1000, PS5x::GPU::ResourceState::Undefined,
                       PS5x::GPU::ResourceState::RenderTarget};
  cl.Barrier(b);
...
TEST_CASE("Phase7::CmdProc::Builder::BeginEndRenderPass",
          "[cmd_proc][phase7]") {
  CommandList cl;
...
TEST_CASE("Phase7::CmdProc::Process::BarrierCounted", "[cmd_proc][phase7]") {
  ...
  ResetStats();
  CommandList cl;
  PS5x::GPU::Barrier b{0x2000, PS5x::GPU::ResourceState::Undefined,
                       PS5x::GPU::ResourceState::RenderTarget};
  cl.Barrier(b);
  cl.End();
  Process(cl);
...
TEST_CASE("Phase7::CmdProc::Process::CommandListsIncrement",
          "[cmd_proc][phase7]") {
  Init(nullptr);
  ResetStats();
  CommandList cl;
