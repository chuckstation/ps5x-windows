...
TEST_CASE("Perf – Multiple frames accumulate history", "[perf]")
{
    ...
    auto s = GetFrameStats();
    REQUIRE(s.frameIndex >= 4);   // 0-based: after 5 FrameEnd calls, last stored is 4
...
{
    Setup();
    for (int i = 0; i < 3; ++i) { FrameBegin(); FrameEnd(); }
    ResetFrameStats();
    auto s = GetFrameStats();
...
TEST_CASE("Perf – ProfileBegin/End records sample", "[perf]")
{
    ...
    for (const auto& s : report)
        if (s.label == "test-scope") { found = true; REQUIRE(s.calls == 1); }
    REQUIRE(found);
    Teardown();
}
...
TEST_CASE("Perf – Multiple calls accumulate stats", "[perf]")
{
    ...
    bool found = false;
    for (const auto& s : report)
        if (s.label == "multi") { found = true; REQUIRE(s.calls == 10); }
    REQUIRE(found);
    Teardown();
}
...
TEST_CASE("Perf – ScopeGuard records scope automatically", "[perf]")
{
    ...
    bool found = false;
    for (const auto& s : report)
        if (s.label == "raii-scope") found = true;
    REQUIRE(found);
    Teardown();
}
...
TEST_CASE("Perf – Report sorted by totalMs descending", "[perf]")
{
    ...
    for (size_t i = 0; i+1 < report.size(); ++i)
        if (report[i].totalMs >= report[i+1].totalMs) slowFirst = true;
    REQUIRE(slowFirst);
    Teardown();
}
...
{
    Setup();
    auto t = ProfileBegin("x"); ProfileEnd(t);
