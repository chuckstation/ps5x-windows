...
TEST_CASE("Phase6::FS::Trace::EventsRecorded", "[filesystem][phase6]")
{
    ...
    for (auto& e : entries) {
        if (e.event == FsEvent::Open) foundOpen = true;
    }
    CHECK(foundOpen);
    EnableTracing(false);
}

TEST_CASE("Phase6::FS::Trace::NotRecordedWhenDisabled", "[filesystem][phase6]")
{
    EnableTracing(false);
    ClearTrace();
...
TEST_CASE("Phase6::FS::TempFile::UnnamedGetsUniqueNames", "[filesystem][phase6]")
{
...
TEST_CASE("Phase6::FS::TempFile::MultipleCallsDoNotCrash", "[filesystem][phase6]")
{
    for (int i = 0; i < 10; ++i) {
...
TEST_CASE("Phase6::FS::ReadOnly::App0DefaultNotReadOnly", "[filesystem][phase6]")
{
    Init();
...
TEST_CASE("Phase6::FS::ReadOnly::SaveDataDefaultWritable", "[filesystem][phase6]")
{
    Init();
...
TEST_CASE("Phase6::FS::ReadOnly::InvalidMountReturnTrue", "[filesystem][phase6]")
{
