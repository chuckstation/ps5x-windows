...
TEST_CASE("Phase8::Fuzz::Loader::CorruptedSectionHeaders", "[fuzz][phase8]") {
  ...
  std::mt19937 rng(42);
  for (size_t i = 7; i < elf.size(); ++i)
    elf[i] = static_cast<uint8_t>(rng() & 0xFF);
...
TEST_CASE("Phase8::Fuzz::Loader::NonexistentPathReturnsError",
          "[fuzz][phase8]") {
  Loader::Init();
...
TEST_CASE("Phase8::Fuzz::CommandProcessor::EmptyCommandList",
          "[fuzz][phase8]") {
  GPU::Init(nullptr);
  ...
  for (int trial = 0; trial < 20; ++trial) {
    ...
    for (int i = 0; i < ops; ++i) {
      ...
      case 3:
        ...
        break;
      case 4:
        cl.Barrier({0, GPU::ResourceState::Undefined,
...
TEST_CASE("Phase8::Fuzz::Memory::AllocZeroSize", "[fuzz][phase8]") {
  ...
  if (p)
    Memory::FreeHost(p); // if returned, must be free-able
  CHECK(true);
  Memory::Shutdown();
}
...
TEST_CASE("Phase8::Fuzz::Filesystem::MalformedPathsDoNotCrash",
          "[fuzz][phase8]") {
  Filesystem::Init();
