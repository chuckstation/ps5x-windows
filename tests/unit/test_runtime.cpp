...
TEST_CASE("Runtime – SubsystemStateName coverage", "[runtime]") {
  REQUIRE(std::string(SubsystemStateName(SubsystemState::Running)) ==
          "Running");
  REQUIRE(std::string(SubsystemStateName(SubsystemState::Failed)) == "Failed");
  REQUIRE(std::string(SubsystemStateName(SubsystemState::Stopped)) ==
          "Stopped");
}
...
TEST_CASE("Runtime – dependency ordering enforced", "[runtime]") {
  ...
  Register({SubsystemId::Logger,
            "Logger",
            [&] {
              order.push_back("Logger");
              return true;
            },
            [] {},
            {},
            false});
  Register({SubsystemId::Config,
            "Config",
            [&] {
            ...
            {SubsystemId::Logger},
            false});
  Register({SubsystemId::Memory,
            "Memory",
            [&] {
              order.push_back("Memory");
              return true;
            },
            [] {},
            {SubsystemId::Logger},
...
TEST_CASE("Runtime – optional subsystem failure does not abort", "[runtime]") {
  Setup();
  Register(
      {SubsystemId::Logger, "Logger", [] { return true; }, [] {}, {}, false});
  Register({SubsystemId::Audio,
            "Audio",
            [] { return false; }, // fails
            [] {},
            {SubsystemId::Logger},
