...
static Cpu::CpuContext MakeCtx(uint64_t nr, uint64_t a0 = 0, uint64_t a1 = 0,
                               uint64_t a2 = 0) {
  Cpu::CpuContext ctx;
...
TEST_CASE("Phase7::Syscall::Register::Overwrite", "[syscalls][phase7]") {
  Init();
  RegisterSyscall(7777, "first",
                  [](const SyscallArgs &) -> int64_t { return 1; });
  RegisterSyscall(7777, "second",
                  [](const SyscallArgs &) -> int64_t { return 2; });
  auto d = Lookup(7777);
  REQUIRE(d.has_value());
  CHECK(d->name == "second");
  Shutdown();
...
TEST_CASE("Phase7::Syscall::Name::KnownReturnsName", "[syscalls][phase7]") {
  Init();
  RegisterSyscall(1234, "my_named_call",
                  [](const SyscallArgs &) -> int64_t { return 0; });
  CHECK(std::string(SyscallName(1234)) == "my_named_call");
  Shutdown();
}

TEST_CASE("Phase7::Syscall::Name::UnknownReturnsUnknown",
          "[syscalls][phase7]") {
  Init();
...
TEST_CASE("Phase7::Syscall::Dispatch::KnownHandler", "[syscalls][phase7]") {
  Init();
  bool fired = false;
  RegisterSyscall(5555, "test_dispatch", [&](const SyscallArgs &a) -> int64_t {
    fired = true;
    return 99;
  });
...
TEST_CASE("Phase7::Syscall::Dispatch::UnknownSyscallSafe",
          "[syscalls][phase7]") {
  Init();
