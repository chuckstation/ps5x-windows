...
TEST_CASE("Phase6::Perf::GpuTs::OrphanedBeginDoesNotCrash",
          "[perf_tools][phase6]") {
  Init();
  ResetGpuTimestamps();
...
TEST_CASE("Phase6::Perf::FrameHist::EmptyBeforeFrames",
          "[perf_tools][phase6]") {
  Init();
  ResetFrameStats();
...
TEST_CASE("Phase6::Perf::FrameHist::PopulatedAfterFrames",
          "[perf_tools][phase6]") {
  Init();
  ResetFrameStats();
  for (int i = 0; i < 5; ++i) {
