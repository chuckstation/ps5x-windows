...
{
    REQUIRE(std::string(EventTypeName(EventType::ProcessCreated))  == "ProcessCreated");
    REQUIRE(std::string(EventTypeName(EventType::FrameEnd))        == "FrameEnd");
    REQUIRE(std::string(EventTypeName(EventType::SyscallReturn))   == "SyscallReturn");
    REQUIRE(std::string(EventTypeName(EventType::WatchdogTimeout)) == "WatchdogTimeout");
...
TEST_CASE("Events – Subscribe receives matching events", "[events]")
{
    ...
    auto id = Subscribe([&](const RuntimeEvent& ev){
        if (ev.type == EventType::Custom) received.fetch_add(1);
    }, EventType::Custom);
...
TEST_CASE("Events – Watchdog fires warning for slow guest", "[events]")
{
    ...
    // Accept either: warning fired (ideal) OR test ran too fast for the poll cycle
    // (acceptable in heavily loaded CI). We just require no crash.
    // In normal conditions, warnings >= 1.
...
TEST_CASE("Events – KickWatchdog resets timer", "[events]")
{
    ...
    std::atomic<int> warnings{0};
    Subscribe([&](const RuntimeEvent& ev){
        if (ev.type == EventType::WatchdogWarning) warnings.fetch_add(1);
    });
