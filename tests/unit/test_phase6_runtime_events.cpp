...
{
    CHECK(std::string(EventCategoryName(EventCategory::Lifecycle))  == "Lifecycle");
    ...
    CHECK(std::string(EventCategoryName(EventCategory::Filesystem)) == "Filesystem");
    CHECK(std::string(EventCategoryName(EventCategory::Scheduler))  == "Scheduler");
...
{
    CHECK(GetEventCategory(EventType::ProcessCreated) == EventCategory::Lifecycle);
    CHECK(GetEventCategory(EventType::ProcessExited)  == EventCategory::Lifecycle);
    CHECK(GetEventCategory(EventType::ProcessFaulted) == EventCategory::Lifecycle);
}
...
TEST_CASE("Phase6::RE::SubscribeCategory::ThreadEvents", "[runtime_events][phase6]")
{
    Init();
    Reset();
    std::atomic<int> count{0};
    auto id = SubscribeCategory(EventCategory::Thread, [&](const RuntimeEvent&){
        count.fetch_add(1);
    });
...
TEST_CASE("Phase6::RE::SubscribeCategory::GPUEvents", "[runtime_events][phase6]")
{
    Init();
    Reset();
    std::atomic<int> count{0};
    auto id = SubscribeCategory(EventCategory::GPU, [&](const RuntimeEvent&){
        count.fetch_add(1);
    });
...
TEST_CASE("Phase6::RE::PublishThread::AppearsInRecent", "[runtime_events][phase6]")
{
    Init();
    Reset();
    ...
    for (auto& e : recent) {
        if (e.type == EventType::ThreadSpawned) {
            ...
            if (p && p->threadId == 100) found = true;
        }
    }
    CHECK(found);
    Shutdown();
...
TEST_CASE("Phase6::RE::PublishGpu::AppearsInRecent", "[runtime_events][phase6]")
{
    Init();
    Reset();
    ...
    for (auto& e : recent) {
        if (e.type == EventType::FrameEnd) {
            ...
            if (p && p->frameIndex == 77) found = true;
        }
    }
    CHECK(found);
    Shutdown();
...
TEST_CASE("Phase6::RE::PublishAudio::AppearsInRecent", "[runtime_events][phase6]")
{
    Init();
    Reset();
    ...
    for (auto& e : recent) {
        if (e.type == EventType::Custom) {
            ...
            if (p && p->tag.find("audio") != std::string::npos) found = true;
        }
    }
    CHECK(found);
    Shutdown();
...
TEST_CASE("Phase6::RE::PublishFilesystem::AppearsInRecent", "[runtime_events][phase6]")
{
    Init();
    Reset();
...
TEST_CASE("Phase6::RE::PublishScheduler::AppearsInRecent", "[runtime_events][phase6]")
{
    Init();
    Reset();
...
TEST_CASE("Phase6::RE::CategoryCount::ThreadIncremented", "[runtime_events][phase6]")
{
    Init();
    Reset();
...
TEST_CASE("Phase6::RE::CategoryCount::AudioIncremented", "[runtime_events][phase6]")
{
    Init();
    Reset();
